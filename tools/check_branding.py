...
# This checker itself and the generator's two output files are treated
# specially.  The generated files are validated by `generate_runtime.py`; the
# source templates and branding module are the places where the spelling is
        ...
        for pattern in patterns:
            for path in base.rglob(pattern):
                ...
                if relative in EXEMPT:
                    continue
                if relative in GENERATED_RUNTIME:
                    first_lines = "\n".join(path.read_text(encoding="utf-8").splitlines()[:3])
                    if "Generated by tools/generate_runtime.py; do not edit." in first_lines:
                        continue
                yield relative, path
