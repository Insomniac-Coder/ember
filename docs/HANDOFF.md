...
Four artifacts, one chain, and they are not interchangeable.
    ...
    docs/spec-source/Ember_v0.9.5_Hardened_10.md        frozen development target; NOT normative until explicitly adopted
...
| Ledgers | 58 defects, **none open**. **4 open deviations** (D1–D4). ODR-004 through ODR-009 are closed; ODR-003 is deferred editorial; **no owner semantic decision is open** |
...
**Implement H10's Arena initialization prerequisites and dependent surface.**
...
exact pickup is the real `Default`, `Zeroable`, and
...
1. `docs/spec-source/ember-spec.md` and the frozen H10 target — especially
   initialization, unsafe, `[TYP-15]`, `[LT-1]`, `[LT-4]`, and `[ARN-*]`;
2. `docs/MIGRATION-0.9.5.md`, then this §0 and §§0.26–0.27;
...
the remaining Phase 2 exit work. Do not begin 0.9.5 multi-region-view
implementation during Arena.
...
The next implementation work is `GEN-METHOD-1`, the H10 initialization
foundation, the remaining Arena surface, UnsafeCell, Phase 2 completion, and
the 0.9.5 multi-region work after the current target is explicitly adopted.
