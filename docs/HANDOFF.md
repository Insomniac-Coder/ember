...
## 0. Current state — the authoritative snapshot (updated 2026-09-23)
...
Four artifacts, one chain, and they are not interchangeable.
    ...
    docs/spec-source/Ember_v0.9.7_Hardened_3.md         immutable predecessor of 0.9.8_Hardened_1
    docs/spec-source/Ember_v0.9.8_Hardened_1.md         immutable predecessor in the 0.9.8 target lineage
    docs/spec-source/Ember_v0.9.8_Hardened_2.md         immutable immediate predecessor of the current 0.9.8_Hardened_3 target
    docs/spec-source/Ember_v0.9.8_Hardened_3.md         current frozen development target; NOT normative until explicitly adopted
...
### 0.169 `Shared[T]` / `Weak[T]` source-boundary discovery — 2026-09-20
...
ODR-017 was opened in `docs/OWNER-QUEUE.md` with the exact missing decisions. No
...
work and does not answer this library API gap. Implementation paused at this
owner boundary until a ruling defined the surface. Phase accounting remained
**1 of 9 complete**.

### 0.170 ODR-017 resolved; 0.9.8 target selected — 2026-09-20

The owner resolved ODR-017 and selected `0.9.8_Hardened_1`, not a 0.9.7
hardening, because the ruling supplies previously undefined public ownership
and borrowing semantics. `Shared(value)` is the strong-owner constructor;
`get()` returns `ref T`; `get_mut(mut self)` returns `ref mut T` under ordinary
borrowing plus the existing dynamic-exclusivity access state. `Weak[O]` is the
weak form of a counted owner: the existing `Weak[C]` remains valid, while the
companion of `Shared[T]` is `Weak[Shared[T]]`. The weak constructor, empty,
copy/drop, upgrade, no-resurrection, and foreign non-interoperation behavior
reuse the object-header model rather than adding an aliasing or ownership system.

The ruling is preserved under `docs/spec-source/as-received/`, ADR-037 records
the decision, and the frozen 0.9.8 target is the implementation authority.
...
### 0.213 `[TYP-16]` generic-interface `dyn` materialization — 2026-09-21

`interface Inspect[T]` was syntactically accepted but its owner parameter was
not in scope while member signatures were collected; the checker also tracked
only bare interface names. Consequently `Inspect[T]` could not be implemented
and `dyn Inspect[i32]` could not form. This was a compiler defect against the
existing generic-interface grammar and `[TYP-22]`, not a missing semantic rule.

The checker now materializes a concrete interface contract from its generic
declaration, substitutes its method signatures, and binds a generic nominal's
owner arguments while registering its `implements` clause. The intentionally
red imported `Signal[T] implements Inspect[T], Child[T]` probe now forms
`Box[dyn Child[i32]]`, dispatches an `i32` argument through inherited and child
...
### 0.296 `[DSP-1]` borrowed generic virtual weak-field arguments — 2026-09-22

The default borrowed parameter mode survives a generic virtual dispatch. Passing
a `Weak[Shared[Token]]` field from another class to a base-typed virtual call
reuses that field's weak observer; the derived override upgrades it against the
independent live `Shared` owner without a retain/release pair at the dispatch
boundary.

`tests/conformance/TYP-16/accept_generic_class_virtual_borrowed_weak_field_argument.em`
prints `7` in debug, release, and shipping. Its generated-C checks require the
derived virtual slot and base-typed `->slot0` call, one weak retain/release pair
for the Holder field only, and the weak upgrade. This extends `[TYP-16]`,
...
### 0.300 `[TYP-22]` borrowed dynamic-interface returns of generic weak fields — 2026-09-22

A `ref dyn Observe` carrier borrows a generic class handle while its object-safe
slot returns a copied `Weak[Shared[Token]]` field. The dynamic call does not
copy the class owner; the returned weak observer alone gains the independent
control-block ownership needed for the caller's later `upgrade()`.

`tests/conformance/TYP-22/accept_ref_dyn_generic_class_weak_field_return.em`
prints `7` in debug, release, and shipping. Generated-C assertions pin the
concrete adapter slot and require weak retain/release/upgrade operations. This
...
### 0.303 `[TYP-22]` expired borrowed dynamic returns of generic weak fields — 2026-09-22

A `ref dyn Observe` call may return a copied `Weak[Shared[Token]]` field from a
generic class without extending the class or payload lifetime. After the helper
destroys both the class and its final `Shared` owner, the returned observer
still owns only the control-block reference and `upgrade()` correctly takes
`None`.

`tests/conformance/TYP-22/accept_expired_ref_dyn_generic_class_weak_field_return.em`
prints `7` in debug, release, and shipping. Generated-C assertions require the
...
### 0.304 `[TYP-22]` expired mutable borrowed dynamic returns of generic weak fields — 2026-09-22

A `ref mut dyn Revise` call may update a generic class under the normal dynamic
write interval and return its copied `Weak[Shared[Token]]` field. The mutation
does not extend either the class or payload lifetime: after the helper tears
down both objects, the returned weak observer still correctly upgrades to
`None`.

`tests/conformance/TYP-22/accept_expired_ref_mut_dyn_generic_class_weak_field_return.em`
prints `7` in debug, release, and shipping. Generated-C assertions require the
...
### 0.306 `[TYP-22]` expired dynamic returns of generic class-owned weak fields — 2026-09-22

A `Weak[Token]` returned from a generic class through `Box[dyn Observe]` must
remain a valid but non-owning control-block handle after the box payload and the
final `Token` owner leave the helper. Its later `upgrade()` correctly takes
`None`; the adapter must not retain either class owner or reconstruct a token
handle from the expired observer.

`tests/conformance/TYP-22/accept_expired_dyn_interface_box_generic_class_weak_class_field_return.em`
prints `7` in debug, release, and shipping. Generated-C assertions require the
concrete dynamic adapter and weak retain/release/upgrade operations while
rejecting `ember_box_new_copy`. This extends `[TYP-16]`, `[TYP-22]`, `[IFC-1]`,
`[CLS-4]`, `[DRP-6]`, `[HEAP-3]`–`[HEAP-7]`, `[WK-11]`–`[WK-14]`, and
...
### 0.308 `[TYP-22]` expired borrowed dynamic returns of generic class-owned weak fields — 2026-09-22

A `ref dyn Observe` helper may copy a generic class's `Weak[Token]` field for
its caller without extending the class-owner lifetime. Once the helper tears
down both the borrowed class and its final `Token` owner, the returned observer
remains valid solely as an expired weak control-block handle and `upgrade()`
takes `None`.

`tests/conformance/TYP-22/accept_expired_ref_dyn_generic_class_weak_class_field_return.em`
prints `7` in debug, release, and shipping. Generated-C assertions require the
concrete dynamic adapter and weak retain/release/upgrade operations. This
extends `[TYP-16]`, `[TYP-22]`, `[IFC-1]`, `[CLS-4]`, `[OBJ-3]`, `[BRW-1]`,
`[HEAP-3]`–`[HEAP-7]`, `[WK-11]`–`[WK-14]`, and `[TST-26]` coverage without an
...
`Weak[Token]` field without preserving the final class owner. When the helper
ends, the write access is already closed, the class and token are destroyed,
and the returned observer remains valid only as an expired weak handle whose
`upgrade()` takes `None`.

`tests/conformance/TYP-22/accept_expired_ref_mut_dyn_generic_class_weak_class_field_return.em`
prints `7` in debug, release, and shipping. Generated-C assertions require the
concrete mutable adapter, the write-access operation, and weak
retain/release/upgrade operations. This extends `[TYP-16]`, `[TYP-22]`,
`[IFC-1]`, `[CLS-4]`, `[OBJ-3]`, `[BRW-1]`, `[EXC-1]`, `[HEAP-3]`–`[HEAP-7]`,
...
### 0.311 `[TYP-22]` multi-interface dynamic returns of generic class-owned weak fields — 2026-09-22

One `Box[dyn Observe + Bump]` carrier may return a generic class's
`Weak[Token]` field through its shared slot and subsequently invoke its
separate mutable slot. The returned observer is independent of the box and the
write access remains confined to `bump()`, so the later weak upgrade observes
only the distinct live class owner.

`tests/conformance/TYP-22/accept_dyn_multi_interface_box_generic_class_weak_class_field_return.em`
prints `1` and `7` in debug, release, and shipping. Generated-C assertions pin
both ordered multi-interface adapter slots, the write-access operation, and the
weak retain/release/upgrade paths while rejecting `ember_box_new_copy`. This
extends `[TYP-16]`, `[TYP-22]`, `[IFC-1]`, `[CLS-4]`, `[OBJ-3]`, `[DRP-6]`,
`[EXC-1]`, `[HEAP-3]`–`[HEAP-7]`, `[WK-11]`–`[WK-14]`, and `[TST-26]`
coverage without an ODR or specification change.

### 0.312 `[TYP-22]` dynamic returns of inherited generic class-owned weak fields — 2026-09-22

A generic derived class carried in `Box[dyn Observe]` may return a
`Weak[Token]` field inherited from its generic base. The dynamic adapter sees
the derived payload, while base initialization and teardown retain and release
the same class-owner observer normally; the result remains independently
upgradeable only while the separate token owner is live.

`tests/conformance/TYP-22/accept_dyn_interface_box_generic_base_weak_class_field_return.em`
prints `7` in debug, release, and shipping. Generated-C assertions require the
concrete dynamic adapter and weak retain/release/upgrade operations while
rejecting `ember_box_new_copy`. This extends `[TYP-16]`, `[TYP-22]`, `[IFC-1]`,
...
### 0.313 `[TYP-22]` mutable dynamic returns of inherited generic class-owned weak fields — 2026-09-22

A `mut self` interface slot on a generic derived class may update one inherited
base field and return another inherited `Weak[Token]` field through
`Box[dyn Revise]`. The dynamic write interval closes with that call, while the
returned observer retains only the independent weak control-block lifetime
needed to upgrade against the live token owner.

`tests/conformance/TYP-22/accept_dyn_interface_box_generic_base_mut_weak_class_field_return.em`
prints `7` in debug, release, and shipping. Generated-C assertions require the
concrete mutable adapter, the write-access operation, and weak
retain/release/upgrade operations while rejecting `ember_box_new_copy`. This
...
### 0.314 `[TYP-22]` expired dynamic returns of inherited generic class-owned weak fields — 2026-09-22

A `Weak[Token]` copied through `Box[dyn Observe]` from a generic base field
outlives both its derived payload and the final token owner, but not as a
strong edge. After the helper tears down the base and derived objects, the
returned observer is valid only for the expired `upgrade()` path and cannot
reconstruct a class owner.

`tests/conformance/TYP-22/accept_expired_dyn_interface_box_generic_base_weak_class_field_return.em`
prints `7` in debug, release, and shipping. Generated-C assertions require the
concrete dynamic adapter and weak retain/release/upgrade operations while
rejecting `ember_box_new_copy`. This extends `[TYP-16]`, `[TYP-22]`, `[IFC-1]`,
`[CLS-2]`, `[CLS-4]`, `[OBJ-3]`, `[DRP-6]`, `[HEAP-3]`–`[HEAP-7]`,
...
### 0.316 `[TYP-22]` multi-interface dynamic returns of inherited generic class-owned weak fields — 2026-09-22

A `Box[dyn Observe + Bump]` carrier may return an inherited generic-base
`Weak[Token]` field through its shared slot and mutate a separate inherited
counter through its mutable slot. Both ordered adapters use the same derived
payload; the returned observer does not copy the box or class owner and
upgrades only against the separate live token owner.

`tests/conformance/TYP-22/accept_dyn_multi_interface_box_generic_base_weak_class_field_return.em`
prints `1` and `7` in debug, release, and shipping. Generated-C assertions pin
both ordered multi-interface adapter slots, the write-access operation, and the
weak retain/release/upgrade paths while rejecting `ember_box_new_copy`. This
extends `[TYP-16]`, `[TYP-22]`, `[IFC-1]`, `[CLS-2]`, `[CLS-4]`, `[OBJ-3]`,
...
### 0.317 `[TYP-22]` borrowed dynamic returns of inherited generic class-owned weak fields — 2026-09-22

A `ref dyn Observe` carrier may borrow a generic derived class and return its
inherited `Weak[Token]` base field. The derived class remains borrowed; the
returned observer alone gains independent control-block lifetime and may
upgrade only while the distinct token owner remains live.

`tests/conformance/TYP-22/accept_ref_dyn_generic_base_weak_class_field_return.em`
prints `7` in debug, release, and shipping. Generated-C assertions require the
concrete dynamic adapter and weak retain/release/upgrade operations. This
extends `[TYP-16]`, `[TYP-22]`, `[IFC-1]`, `[CLS-2]`, `[CLS-4]`, `[OBJ-3]`,
...
### 0.319 `[TYP-22]` mutable borrowed dynamic returns of inherited generic class-owned weak fields — 2026-09-22

A `ref mut dyn Revise` carrier may update a generic-base counter and return an
inherited `Weak[Token]` field. The derived handle remains borrowed, the write
interval ends with the dynamic call, and the copied observer alone has the
independent control-block lifetime needed to upgrade against the live token
owner.

`tests/conformance/TYP-22/accept_ref_mut_dyn_generic_base_weak_class_field_return.em`
prints `7` in debug, release, and shipping. Generated-C assertions require the
concrete mutable adapter, the write-access operation, and weak
retain/release/upgrade operations. This extends `[TYP-16]`, `[TYP-22]`,
`[IFC-1]`, `[CLS-2]`, `[CLS-4]`, `[OBJ-3]`, `[BRW-1]`, `[EXC-1]`,
`[HEAP-3]`–`[HEAP-7]`, `[WK-11]`–`[WK-14]`, and `[TST-26]` coverage without an
ODR or specification change.

### 0.320 `[TYP-22]` expired mutable borrowed dynamic returns of inherited generic class-owned weak fields — 2026-09-22

A `ref mut dyn Revise` helper may update an inherited generic-base counter and
return its `Weak[Token]` field without preserving the final class owner. After
the helper ends, the dynamic write interval is closed, the derived class and
token are gone, and the returned observer correctly follows the expired
`upgrade()` path.

`tests/conformance/TYP-22/accept_expired_ref_mut_dyn_generic_base_weak_class_field_return.em`
prints `7` in debug, release, and shipping. Generated-C assertions require the
concrete mutable adapter, the write-access operation, and weak
retain/release/upgrade operations. This extends `[TYP-16]`, `[TYP-22]`,
`[IFC-1]`, `[CLS-2]`, `[CLS-4]`, `[OBJ-3]`, `[BRW-1]`, `[EXC-1]`,
...
### 0.321 `[TYP-22]` borrowed multi-interface dynamic returns of inherited generic class-owned weak fields — 2026-09-22

A `ref mut dyn Observe + Bump` carrier may mutate an inherited generic-base
counter through its mutable slot and then return the inherited `Weak[Token]`
field through its separate shared slot. The derived class stays borrowed across
both calls; only the weak observer is copied for the caller's later upgrade.

`tests/conformance/TYP-22/accept_ref_mut_dyn_multi_interface_generic_base_weak_class_field_return.em`
prints `7` in debug, release, and shipping. Generated-C assertions pin both
ordered multi-interface adapter slots, the write-access operation, and weak
retain/release/upgrade operations. This extends `[TYP-16]`, `[TYP-22]`,
`[IFC-1]`, `[CLS-2]`, `[CLS-4]`, `[OBJ-3]`, `[BRW-1]`, `[EXC-1]`,
`[HEAP-3]`–`[HEAP-7]`, `[WK-11]`–`[WK-14]`, and `[TST-26]` coverage without an
ODR or specification change.

### 0.322 `[DSP-1]` generic virtual returns of class-owned weak fields — 2026-09-22

A generic base-typed virtual call may return an inherited `Weak[Token]` field
from a derived override. The returned observer is separate from the virtual
receiver and owns only its weak control-block lifetime; it upgrades to the
class handle only while the distinct token owner remains live.

`tests/conformance/TYP-16/accept_generic_class_virtual_weak_class_field_return.em`
prints `7` in debug, release, and shipping. Generated-C assertions require the
derived virtual slot, base-typed `->slot0` dispatch, and weak
retain/release/upgrade operations. This extends `[TYP-16]`, `[CLS-2]`,
`[CLS-4]`, `[OBJ-3]`, `[DSP-1]`, `[HEAP-3]`–`[HEAP-7]`, `[WK-11]`–`[WK-14]`,
and `[TST-26]` coverage without an ODR or specification change.

### 0.323 `[DSP-1]` expired generic virtual returns of class-owned weak fields — 2026-09-22

A `Weak[Token]` returned through a generic virtual slot may outlive the derived
receiver and its final token owner only as a control-block reference. After the
helper exits, the observer remains valid but expired: the base-typed virtual
dispatch must not reconstruct or retain the class owner during its later
`upgrade()`.

`tests/conformance/TYP-16/accept_expired_generic_class_virtual_weak_class_field_return.em`
prints `7` in debug, release, and shipping. Generated-C assertions require the
derived virtual slot, base-typed `->slot0` dispatch, and weak
retain/release/upgrade operations. This extends `[TYP-16]`, `[CLS-2]`,
`[CLS-4]`, `[OBJ-3]`, `[DSP-1]`, `[HEAP-3]`–`[HEAP-7]`, `[WK-11]`–`[WK-14]`,
...
### 0.324 `[DSP-1]` mutable generic virtual returns of class-owned weak fields — 2026-09-22

A base-typed mutable generic virtual call may update an inherited counter in a
derived override and return an inherited `Weak[Token]` field. The normal write
interval ends with the virtual call, while the returned observer has independent
weak control-block lifetime and upgrades only against the separate live token
owner.

`tests/conformance/TYP-16/accept_generic_class_virtual_mut_weak_class_field_return.em`
prints `7` in debug, release, and shipping. Generated-C assertions require the
derived virtual slot, base-typed `->slot0` dispatch, the write-access operation,
and weak retain/release/upgrade operations. This extends `[TYP-16]`, `[CLS-2]`,
`[CLS-4]`, `[OBJ-3]`, `[DSP-1]`, `[EXC-1]`, `[HEAP-3]`–`[HEAP-7]`,
`[WK-11]`–`[WK-14]`, and `[TST-26]` coverage without an ODR or specification
change.

### 0.325 `[DSP-1]` expired mutable generic virtual returns of class-owned weak fields — 2026-09-22

A mutable generic virtual call may update inherited derived state and return an
inherited `Weak[Token]` field without retaining the final token owner. When the
helper ends, the virtual write interval has closed and the returned observer is
valid only for the expired `upgrade()` path; it cannot reconstruct the class
owner.

`tests/conformance/TYP-16/accept_expired_generic_class_virtual_mut_weak_class_field_return.em`
prints `7` in debug, release, and shipping. Generated-C assertions require the
derived virtual slot, base-typed `->slot0` dispatch, the write-access operation,
and weak retain/release/upgrade operations. This extends `[TYP-16]`, `[CLS-2]`,
`[CLS-4]`, `[OBJ-3]`, `[DSP-1]`, `[EXC-1]`, `[HEAP-3]`–`[HEAP-7]`,
...
### 0.327 `[DSP-1]` mutable generic virtual replacement of class-owned weak fields — 2026-09-22

A base-typed virtual call may transfer a `Weak[Token]` argument into a derived
generic override. The override replaces its inherited field, drops the prior
empty observer, retains the replacement, and upgrades only that replacement
while the separate token owner remains live; the receiver's normal dynamic
write interval surrounds the whole transition.

`tests/conformance/TYP-16/accept_generic_class_virtual_mut_weak_class_field_reassignment.em`
prints `8` in debug, release, and shipping. Generated-C assertions require the
derived virtual slot, base-typed `->slot0` dispatch, the write-access operation,
and weak retain/release/upgrade paths. This extends `[TYP-16]`, `[CLS-2]`,
...
### 0.351 ODR-019 option 1 and N1 cross-file ranking — 2026-09-23

This entry supersedes 0.350. The owner supplied an ODR-019 ruling that named
`0.9.8_Hardened_2`, then explicitly clarified that the new artifact must be
`_3` and that the existing `_2` must not be edited. `_2` remains untouched;
`Ember_v0.9.8_Hardened_3.md` is a copy of `_2` with immediate-predecessor
lineage and is now the frozen, not-yet-adopted development target. The received
ruling is archived verbatim at
`docs/spec-source/as-received/ODR-019_N1_cross_file_suggestion_ranking.md`.

ODR-019 is closed in `docs/OWNER-QUEUE.md`; `_3` has one normative algorithm
in XX.6.2 and one H3 change-log section. Damerau–Levenshtein remains primary;
same-file locality wins only on equal distance; existing same-file proximity
is unchanged; cross-file ties use canonical qualified declaration identity;
top-three truncation follows complete sorting. Candidate discovery, visibility,
language acceptance, runtime, ABI, ownership, lifetime, and type semantics do
not change. The summary row references the algorithm without duplicating it.

The implementation carries declaration spans and canonical identities into
the existing N1 ranking helper. Added conformance cases cover locality vs
distance, canonical cross-file top-three ordering, aliased imports, inherited
fields, prelude names, and a compiling correction. A driver UI test repeats
the diagnostic with reversed helper-file creation and import order and checks
actual ordered help output. The type-checker unit tests also vary candidate
and file IDs to prove cross-file offsets do not affect the result.

Verification is complete: `cargo build --workspace --locked` and
`cargo test --workspace --locked` pass (35 milestone tests, including the
multi-profile conformance runner; 6 UI tests; 8 N1 ranking unit tests). The
repository document gates pass: hardening, spec split, rule index, branding,
runtime generation/regression, spec parsing, and error pages. Targeted
`rustfmt --check`, development-target SHA verification, and `git diff --check`
also pass. The first full run caught an invalid positive fixture that nested
calls unsupported by this current C lowering; it was simplified to a valid
