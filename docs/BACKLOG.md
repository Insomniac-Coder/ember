...
| **ARN-INIT-1** | Initialization types/interfaces required by Arena bulk allocation | 2 | **Done when** H10 `[ARN-3]`/`[ARN-8]`–`[ARN-13]` are implemented exactly: `Zeroable` then `Default`, E2040 fallback, unconditional `!needs_drop`, transactional rollback, compiler-proven all-zero validity, canonical `MaybeUninit` value/span APIs, and Phase 2 core support. ODR-009 is closed; there is currently no `Default`, `Zeroable`, or `MaybeUninit` compiler surface, so this is ordinary implementation work rather than permission for ad-hoc zeroing or fake initialized bytes |
| **ARN-COLL-1** | `[ARN-5]` `ArenaArray` and `ArenaMap` | 2 | both are genuine arena-backed `@view` collections with `[TYP-15]` provenance and ordinary collection behavior. This waits for the corresponding collection/generic machinery and must not be represented as an owning `Array`/`Map` with a hidden arena pointer |
...
| **GEN-METHOD-1** | Explicit type arguments on method calls | 2 | a specified call such as `arena.alloc_array[Pixel](2)` reaches method syntax and type checking instead of E1010 “only direct calls are supported in this phase”; method generic arguments participate in inference, bounds, monomorphisation, and diagnostics by the same rules as direct generic calls. This is a compiler mechanism gap, not permission to special-case Arena |
