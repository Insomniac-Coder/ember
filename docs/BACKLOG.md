...
| **ARN-INIT-1** | Initialization types/interfaces required by Arena bulk allocation | 2 + ODR-009 | `[ARN-3]`'s `alloc_array` is implemented with its exact `T: Zeroable` / `T: Default` behavior and `alloc_uninit` uses the real `MaybeUninit[u8]` contract. There is currently no `Default`, `Zeroable`, or `MaybeUninit` compiler surface, and H9 does not define the unsafe initialization boundary completely; no ad-hoc zeroing or fake uninitialized type may substitute for the owner ruling |
