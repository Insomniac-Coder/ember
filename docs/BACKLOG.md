...
| Completed | 8 compiler-debt items (`LT-REG-1`, `RIDX-1`, `CELL-DEF-1`, `ARN-INIT-1`, `ARN-COLL-1`, `GEN-METHOD-1`, `VER-096-1`, `MEM-API-1`) |
...
| **SPN-API-1** | Span/MutSpan view-method surface (`split_at`, `chunks`, `iter_mut`, `reborrow`, `as_ptr`, …) | 2 | **Partially implemented in `352ea64` and `1aaa98f`.** `Array[T].split_at_mut(index)` and shared/mutable `Span.split_at(index)` now share one checked MIR split and one C result constructor. Mutable Span splitting reborrows named parents, accepts the normative direct view-producing expression, preserves source provenance transitively, returns disjoint same-kind halves, permits end boundaries, and rejects out-of-range boundaries through the ordinary bounds path. `chunks`, `iter`/`iter_mut`, `[SPN-3]`'s explicit `reborrow`, `as_ptr`, and the rest of the rule-named surface remain absent. Done when that remaining surface exists and its sanctioned-way cases compile |
