...
| **ARCH-096-1** | Implement 0.9.6's canonical semantic-fact architecture | 2/3 | **Started 2026-09-13; verified boundaries landed in `66d0d43` and `c913fbd`.** `TypeIdentity` and `LayoutDescriptor` name the existing canonical representations; `BorrowCapability`, `AccessContract`, storage/provenance, permission, representation, ownership, checking, unsafe-authority, synchronization, validity, escape, and shared `InitializationState` facts exist, and Arena loans carry distinct provenance/storage facts. Definite initialization retains one verified `InitializationFacts` fixpoint, and the C backend accepts only an exact `VerifiedMir` body/type pairing. The region solver now retains compile-time-only per-field slots for direct multi-view aggregates and uses field-sensitive NLL, with conservative fixed-array provenance and runtime erasure. Callable field/provenance summaries, field replacement, `E3065/B14`, summary invalidation, enum/generic/escape matrices, MIR verification of slots/summaries, `OwnershipGraph`, `EffectSet`, complete borrow/ownership producer-consumer migration, and the equivalence matrix remain. Preserve all accepted/rejected 0.9.5 behavior and the three distinct write orderings; do not create placeholder facts with no real producer and consumer |
