...
| | |
|---|---|
| Total | 43 tracked items — 20 must-have, 12 nice-to-have, 11 compiler-debt items |
...
| **ARN-INIT-1** | Initialization types/interfaces required by Arena bulk allocation | 2 | `[ARN-3]`'s `alloc_array` is implemented with its exact `T: Zeroable` / `T: Default` behavior and `alloc_uninit` uses the real `MaybeUninit[u8]` contract. There is currently no `Default`, `Zeroable`, or `MaybeUninit` compiler surface; no ad-hoc zeroing or fake uninitialized type may substitute for them |
