...
| Completed | 9 compiler-debt items (`LT-REG-1`, `RIDX-1`, `CELL-DEF-1`, `ARN-INIT-1`, `ARN-COLL-1`, `GEN-METHOD-1`, `VER-096-1`, `MEM-API-1`, `TUP-DST-1`) |
...
| **SPN-API-1** | Span/MutSpan view-method surface (`split_at`, `chunks`, `iter_mut`, `reborrow`, `as_ptr`, …) | 2 | **Partially implemented in `352ea64`, `1aaa98f`, and `24d8fbc`.** `Array[T].split_at_mut(index)` and shared/mutable `Span.split_at(index)` share one checked MIR split and one C result constructor. `[SPN-3]`'s explicit `MutSpan.reborrow()` now forms the same mutable receiver loan, works through generic and direct view-producing forms, reconstructs only the existing pointer/length representation, retains transitive owner provenance, and leaves the parent usable after the child loan ends. `chunks`, `iter`/`iter_mut`, `as_ptr`, and the rest of the rule-named surface remain absent. Done when that remaining surface exists and its sanctioned-way cases compile |
