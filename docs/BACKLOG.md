...
`ARCH-096-1` continuation checkpoint: `d35e94f` routes Arena allocation-return
escape checking through canonical `StorageIdentity::ArenaAllocation` ownership
while retaining the source-type fallback for ordinary Arena place borrows. This
preserves behavior and is an implementation-architecture migration under
`[IMP-7]`; no new semantic fact or placeholder consumer was introduced.
