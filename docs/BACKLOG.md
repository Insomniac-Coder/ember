...
| Completed | 6 compiler-debt items (`LT-REG-1`, `RIDX-1`, `CELL-DEF-1`, `ARN-INIT-1`, `GEN-METHOD-1`, `VER-096-1`) |
| Started | `ARCH-096-1`, `ARN-COLL-1` |
...
| **ARN-COLL-1** | `[ARN-5]` `ArenaArray` and `ArenaMap` | 2 | **Started 2026-09-13; active against H4.** Both are genuine fixed-capacity, single-allocation arena-backed `@view` collections with `[TYP-15]` provenance, not owning wrappers with hidden Arena pointers. ODR-011 is closed in H2. Array, built-in-key Map, all minimum operations, `CapacityError.Full`, `!needs_drop`, named iterator, provenance/borrow, zero/full/reuse, and exact allocation-count paths have executable tests. ODR-012 is closed in H3 with public `Hash`/`Hasher`, `DefaultHasher`, Eq/hash coherence, read-only resident keys, and no frozen mixing algorithm. ODR-013 is closed in H4 with static `fn hash[H: Hasher](self, mut h: H)`, inferred concrete `H`, and no mandatory dynamic dispatch. Remaining: public Hasher/DefaultHasher implementation, user-defined `Eq + Hash` key dispatch, and final `[TST-24]`/`[HASH-*]` evidence |
...
| **ARCH-096-1** | Implement 0.9.6's canonical semantic-fact architecture | 2/3 | **Started 2026-09-13.** `TypeIdentity` and `LayoutDescriptor` now name the existing canonical representations; `BorrowCapability`, `AccessContract`, storage/provenance, permission, representation, ownership, checking, unsafe-authority, synchronization, validity, escape, and shared `InitializationState` facts exist, and Arena loans carry distinct provenance/storage facts. `OwnershipGraph`, `EffectSet`, complete producer/consumer migration, callable summaries, verification, runtime erasure proof, and the equivalence matrix remain. Preserve all accepted/rejected 0.9.5 behavior and the three distinct write orderings; do not create placeholder facts with no real producer and consumer |
