...
| | |
|---|---|
| Total | 46 tracked items — 20 must-have, 12 nice-to-have, 14 compiler-debt items |
...
| **DIA-UI-1** | `[DIA-7..10]`, `[DIA-12]`, `[DIA-13]`, and `[PHIL-8a]` rendered diagnostic catalogue | 2 | **Started 2026-09-13.** `compiler/ember_driver/tests/ui.rs` checks exact `.stderr` output, verifies the emitted code maps to the directory's §XX.6.1 shape, checks a shape-specific primary-help construct, rejects orphan companions, and compiles every `.fixed.em`. Fourteen of the 25 code-keyed ownership shapes have executable cases, including B1 with a compiling `Array.split_at_mut` repair. Remaining ownership shapes, the R1 overlay, all N1–N12 basic shapes, `ember explain --borrow`, and the final `rule_index.py` completeness gate remain. D-060/D-061/D-062 were exposed and fixed by these cases rather than normalized into snapshots |
