...
| D-040 | **Method-call conflicts have no code: `E3025` is registered, shape-mapped, and emitted by nothing.** A method call defeating disjoint-field access (`[BRW-4]`'s parenthetical) reports `E3022`, where `[DIA-7a]` keys shape B8 to `E3025`. Keying it needs call provenance at the report point — the conflicting access is recorded as a plain borrow — which is design work, not a lookup | `[BRW-4]`, `[DIA-7a]` | **open** | thread the method-call origin to the reporter; until then no case pins the method shape since the emitted code is wrong |
...
| D-030 | `[DRP-5]` — a `drop` body moving a field out of `mut self` is accepted, where the rule says it may not (`[EXP-6]`). The field is then dropped again after `drop` returns | `[DRP-5]`, `[EXP-6]` | **open** | found by the same sweep; not yet fixed. Lower severity than D-029 only because reaching it needs a `drop` body that moves, which nothing in the corpus does |
