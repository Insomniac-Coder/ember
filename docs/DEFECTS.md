...
| D-018 | `[RNG-4]`'s range tracking derived a range for constants only, so `[RNG-10]`(d) — "a value whose `[RNG-4]` range is contained in the target's" — admitted a constant and nothing else | `[RNG-4]`, `[RNG-4a]`, `[RNG-10]` | **fixed** | `range_of` returns an **interval**, not a point. The fact carrying most of the weight is not arithmetic: a value at a range type is in its declared range, which `[RNG-9]` makes an invariant and `[RNG-4]` may assume. Intervals propagate through `+`, `-`, and `*`, survive bindings, and are dropped by later writes. `[RNG-4a]` is honoured: a derived fact is kept only where the operation provably cannot overflow its representation in any profile. The remaining `[RNG-4]` transfers were completed by D-141's conditional refinement and D-142's canonical `std.math` helpers; strict floating-point inequalities, disjunctions, and bit-operation facts remain conservative later work. |
