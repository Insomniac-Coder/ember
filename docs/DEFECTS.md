...
| D-025 | `RangeError` is synthesised on demand by the compiler and is not a name a program can write, so `[RNG-3]`'s own signature — `T.checked(v) -> Result[T, RangeError]` — cannot be written by a user. Found by `tools/error_pages.py` refusing a page whose "fix" did not compile | `[RNG-3]` | **open** | the type exists and `match` on the result works; only the name is unavailable. It belongs in `std.core` beside `Ordering`, which is written in Ember and needs no compiler support |
