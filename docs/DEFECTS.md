...
| D-190 | **Returning a view of a local reports two errors.** `fn leak() -> Span[int]: tmp = [41]; return view(tmp)` gives `E3021 tmp cannot be written while it is borrowed` (nothing writes `tmp`: the end-of-function drop is counted as a write) and then the right `E3060 tmp does not live long enough`. Present before the 0.9.9 work (same at `a9294b9`). | `[DIA-14]`, `[LT-3]` | **open** | — |
