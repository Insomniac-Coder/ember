...
| D-135 | **The user-defined class-constructor path rejected named arguments even though `[TYP-25]` requires source arguments to bind to the declared parameter names.** `Point(y=23, x=19)` was therefore rejected with `E1010`, and the constructor path did not carry `[EXP-1]` source evaluation order through its declaration-order ABI arguments. | `[CLS-1]`, `[CLS-2]`, `[CLS-3]`, `[TYP-25]`, `[EXP-1]` | **fixed** | The class-constructor path now reuses the direct-call binder for the `init` parameters, validates unknown/duplicate/positional-after-named arguments, checks expressions in source order, stores them in constructor-parameter order, and carries explicit evaluation-order metadata into MIR. `tests/run-pass/class_named_init_construct.em` covers out-of-order binding and side-effect order; `tests/compile-fail/class_named_init_argument_errors.em` covers all three rejection boundaries in debug, release, and shipping. No specification or owner decision changed. |
