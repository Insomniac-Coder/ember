...
|---|---|---|---|---|
| D-062 | **Shape B1 was emitted with shape B3's repair.** A second mutable indexed borrow correctly reports E3022, but its primary help says only to shorten the first borrow. `[DIA-7]`/`[DIA-10]` require a concrete structural API such as `split_at_mut`, `iter_mut`, `columns_mut`, or `mem.assert_disjoint`; none of the Span/Array disjointness APIs needed to make that prescribed repair compile exists yet | `[DIA-7]`, `[DIA-10]`, `[BRW-5]`, `[PHIL-8a]` | **open** | assigned to `SPN-API-1` and the B1 `tests/ui` case. Do not bless the current B3-shaped output as B1 merely because E3022 itself is correct |
