...
| D-038 | **Implicit `String`→`str` coercion is rejected.** `[SPN-1]` lists "String to `str`" among the coercions and Part XIX §5 records it as one the checker applies, but `v: str = s` is `E2020`. The explicit `as_str()` spelling works (now soundly, D-037), so this fails closed | `[SPN-1]` | **open** | needs the coercion arm in typeck mirroring the `Array`→`Span` one; no case pins the current rejection since the rule admits the program |
...
| D-042 | **Partial moves out of owned places double-destroy at scope end.** `[EXP-6]` allows moving a field out of a plain struct ("leaves the struct partially moved"), but drop elaboration tracks whole-local movedness only — `moved_by_operand` ignores field moves ("Phase 2's later work handles with per-field paths", not built) — so the scope-end drop still destroys the moved-from field. `x = o.inner` with no further use of `o` emits both `em_Inner_drop(&_3)` and `em_Inner_drop(&_1.inner)` for one value (asserted on the emitted C; the program prints the destructor line twice). `E3042` ("partial move then use of the whole value") is registered, shape-mapped (O4), and emitted by nothing, and would not cover the implicit scope-end drop anyway | `[EXP-6]`, `[OWN-2]`, `[OWN-3]` | **open** | needs per-field movedness in drop elaboration; until then no case pins it — a reject cannot fail to compile (that is the defect) and an accept would enshrine the double drop. Found while building D-041's accept case (which therefore covers owned reads and dropless moves only); left open and unfixed here |
