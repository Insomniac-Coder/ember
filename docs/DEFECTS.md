...
| D-018 | `[RNG-4]`'s range tracking derived a range for constants only, so `[RNG-10]`(d) — "a value whose `[RNG-4]` range is contained in the target's" — admitted a constant and nothing else | `[RNG-4]`, `[RNG-4a]`, `[RNG-10]` | **fixed** | `range_of` returns an **interval**, not a point. The fact carrying most of the weight is not arithmetic: a value at a range type is in its declared range, which `[RNG-9]` makes an invariant and `[RNG-4]` may assume. Intervals propagate through `+`, `-`, `*`, proven-safe division, proven-safe remainder, proven-safe shifts, exact bitwise constants, and proven-safe non-negative `&` masks, survive bindings, and are dropped by later writes. `[RNG-4a]` is honoured: a derived fact is kept only where the operation provably cannot overflow its representation in any profile. The remaining `[RNG-4]` transfers were completed by D-141's conditional refinement, D-142's canonical `std.math` helpers, D-143's non-zero division, D-144's non-zero remainder, D-145's in-range shifts, and D-146's conservative bitwise slice; strict floating-point inequalities, disjunctions, and general bitwise facts remain conservative later work. |
