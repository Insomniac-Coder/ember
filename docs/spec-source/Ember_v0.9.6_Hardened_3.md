...
**Immediate predecessor:** 0.9.6_Hardened_2
...
conformance result is asserted merely because the specification requires one. This H3 successor
...
> **REMAINING CALLABLE-TYPE BOUNDARY — OWNER DECISION REQUIRED:** the approved
> `Hash.hash(self, mut h: Hasher)` spelling does not state how a concrete, move-only
> `DefaultHasher` becomes the interface-typed parameter under `[TYP-22]`. That rule defines `dyn I`
> as unsized behind an explicit indirection and permits bare `I` only in class-handle position;
> `DefaultHasher` is not specified as a class handle. ODR-013/ERR-054 preserve the unresolved choice
> between a generic `H: Hasher` method and an explicit dynamic-interface representation. H3 does not
> choose an ABI or dispatch model by inference, and custom-key Hash conformance is blocked until the
> owner resolves this boundary.
...
| AUD-096-18 | Owner decision required | The approved `mut h: Hasher` spelling does not determine how a concrete move-only hasher crosses `[TYP-22]`'s interface-value boundary. ODR-013/ERR-054 block custom-key implementation from inventing generic versus dynamic dispatch. |
...
| AUD-096-01 | Source authority | Preserved the owner-supplied Revision 5 RFC and ODR-011 ruling byte-for-byte, preserved H1 and H2 unchanged, and recorded the ODR-012 authority separately; H2 is the immutable immediate predecessor, while `ember-spec.md` remains the adopted 0.8.5 contract until H3 passes explicit adoption gates. |
...
hashing-contract completion in H3. ADR-030 through ADR-032 preserve the three decisions.
H1 and H2 remain unchanged; H2 is H3's immediate predecessor.

### 0.9.6_Hardened_3 language-conformance condition

`0.9.6_Hardened_3` MUST NOT be declared conformant until all H1 and H2 conditions remain satisfied and
...
12. ODR-013 is owner-resolved and the selected generic or dynamic interface representation is
    reflected consistently in signatures, implementation, ABI, and conformance without a
    compiler-only coercion.

### 0.9.6_Hardened_3 acceptance condition

H3 MUST NOT replace `docs/spec-source/ember-spec.md` unless it has one version identity and predecessor,
...
source observation is insufficient, and every difference from H2 is classified and traceable to the
owner-approved ODR-012 ruling.

## 0.9.6_Hardened_3 Evidence Status — No Implementation Claim

This H3 file is a frozen development target. The adopted repository specification remains
`docs/spec-source/ember-spec.md` (`0.8.5_Hardened_1`). H3's architecture, 0.9.5 multi-region semantics,
...
source does not constitute H3 adoption or H3 conformance.
...
## 0.9.6_Hardened_3 consolidation record
...
completes ODR-011's Arena-backed collection contract and ODR-012's hashing
protocol/API contract. It is normative within
...
* `[HASH-1]` **Hashing contract.** `std.collections` publicly exports the `Hash` and `Hasher`
  ...
  canonical interfaces are the declarations above. `Hasher` is a stateful, move-only hashing
...
* `[HASH-2]` **Canonical hasher.** `std.collections` publicly exports the default concrete hasher
  `DefaultHasher`, with constructor `DefaultHasher.new() -> DefaultHasher`. `DefaultHasher` is not a
...
# Appendix B — 0.9.6_Hardened_3 Consistency Checklist

The following checks are mandatory CI checks for the consolidated specification itself:

1. Exactly one current version declaration exists and names `0.9.6_Hardened_3`.
2. The immediate predecessor is frozen `0.9.6_Hardened_2`, which remains byte-for-byte unchanged.
3. `[MOD-6]` and `[MOD-6a]` name the supported `0.9.6`, `0.9.5`, `0.9`, and 0.8.x source-language contracts explicitly; `0.9` does not silently enable later semantics, and H3 changes H2 only through the owner-approved ODR-012 completion.
...
41. ODR-013/ERR-054 remain explicit until the owner determines whether a concrete
    `DefaultHasher` reaches `Hash.hash` through a generic `H: Hasher` method or an
    explicit `dyn Hasher` representation. No compiler-only interface conversion is inferred.
