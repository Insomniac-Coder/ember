...
**Version:** 0.9.7_Hardened_4
**Immediate predecessor:** 0.9.7_Hardened_3 (immutable; preserved as the diff base)
**Status:** Frozen development target. Full implementation and conformance are **not** claimed; ODR-020's bounded diagnostic slice has implementation and regression evidence, not full DIA-12 or target conformance. Normative repository adoption still requires the recorded adoption gates.

    Specification    frozen for this target — 873 rules, every one carrying a change-log
                     row. ODR-020 is closed here; ODR-016 remains closed from H3, and the
                     two withdrawn H3 gap reports remain withdrawn. 13 rule ids are
                     cited and defined by no rule: the inherited ERR-042 set, unchanged
                     since 0.8.4 and not this revision's to write.
    ...
    Conformance      NOT CLAIMED — 140 0.9-series rules remain in the [TST-4c] baseline
                     after the four H3 removals; H4 does not alter the baseline
...
> **Authority boundary.** This revision records the owner-selected 0.9.7 development target.
...
**What Hardened_4 changed.** It closes ODR-020 by defining qualified-path N1 diagnostics:
resolve the namespace prefix first, search only legally visible members using the unresolved
final identifier, display the source-qualified suggestion, and edit only that final token.
This is diagnostic/tooling hardening: no source-language program changes validity, and no
module/import, visibility, overload, type, ownership, lifetime, ABI or runtime semantics change.
The ruling is archived at `docs/spec-source/as-received/ODR-020_qualified_path_N1.md`.
This H4 is authored from immutable `0.9.7_Hardened_3`; it does not modify H3 or the separate
0.9.8 target. H4 adds no diagnostic code; this source revision's existing unknown-name code is
`E1010`.
...
multi-region views; 0.9.7 moved for the `@latebound` callable boundary; and **this file is
`0.9.7_Hardened_4` because it changes no rule's meaning.**

**Compatibility.** Additive. `0.9.7` ⊃ `0.9.6` ⊃ `0.9.5` ⊃ `0.9` ⊃ `0.8.5` ⊃ `0.8.4` ⊃ `0.8.3`.
No program valid under an earlier contract becomes invalid, and a source file may declare
`#! language "0.8.3"`, `"0.8.4"`, `"0.8.5"`, `"0.9"`, `"0.9.5"`, `"0.9.6"` or `"0.9.7"`. An older
selector never silently acquires later semantics (`[MOD-6]`, `[MOD-6a]`).

**Lineage.** `0.8.5 → 0.9 → 0.9.5 → 0.9.6 → 0.9.7`, each authored from the one before it. 0.8.5 was
...
| **Appendix B** | the 0.9.7_Hardened_1 consistency checklist |
...
## Change log — 0.9.7_Hardened_4

**Hardened_4 closes ODR-020.** It defines how the existing N1 diagnostic applies when
a namespace prefix resolves and the final member does not. This changes diagnostic
selection, presentation, span and fix-it only. No Ember source program changes validity;
no module/import, visibility, overload, type, ownership, lifetime, ABI or runtime behavior
changes.

Every normative edit below is **DIAGNOSTIC / TOOLING HARDENING**. It does not add a
diagnostic code or a second suggestion/ranking system.

| # | Change | Class | Where |
|---|---|---|---|
| H4.1 | **Qualified paths are a specialization of N1.** Resolve the namespace prefix first; enumerate only member names legally visible through that namespace under `[MOD-2]` and the existing resolver; compare spelling against the unresolved final identifier only; keep the existing N1 rank and top-three limit; display the original source-qualified prefix and candidate; highlight and replace only the final identifier. A failed prefix gets the ordinary prefix-resolution diagnostic and never triggers a final-member search. | DIAGNOSTIC / TOOLING HARDENING | XX.6.2, N1 |
| H4.2 | **Conformance and regression evidence.** Add qualified-alias and nested-path suggestions, token-local source spans and edits, alias preservation, public and same-package candidates, private and cross-package `pub(package)` exclusion, ranked top-three candidates, deterministic output, failed-prefix handling, no-candidate handling, and the unchanged direct-`from` import behavior. | DIAGNOSTIC / TOOLING HARDENING | `tests/conformance/DIA-12/`, `compiler/ember_driver/tests/ui.rs` |
| H4.3 | **No language or diagnostic identity change.** The existing `E1010` unknown-name family remains in use in this revision; no `E1060` or other code is introduced. This change adds no accepted program, import behavior, visibility rule, ranking key, runtime operation, or ABI surface. | DIAGNOSTIC / TOOLING HARDENING | XX.6.2 |

**ODR-019 dependency.** This H4 is authored from `0.9.7_Hardened_3`, which does not
incorporate ODR-019's cross-file tie policy. H4 therefore does not redefine or duplicate
cross-file ranking. A source revision that already carries ODR-019 MUST continue applying
its same-file/cross-file ordering to the qualified candidate set.

The complete owner ruling is preserved byte-for-byte at
`docs/spec-source/as-received/ODR-020_qualified_path_N1.md`. H4 is a frozen development
target, not a change to the repository's adopted `0.8.5_Hardened_1` source.

**Implementation checklist.**

- [x] Keep invalid qualified names rejected and leave successful path resolution unchanged.
- [x] Resolve namespace prefixes before final-member candidate discovery.
- [x] Use the existing namespace/member and visibility facts; exclude inaccessible names.
- [x] Compare only the unresolved final segment and preserve the source alias/prefix in help.
- [x] Attach the diagnostic primary span and machine-applicable edit to the final token only.
- [x] Reuse N1 ranking, threshold, deterministic ordering and top-three cap; add no ranking key.
- [x] Keep `from` imports, module/import semantics, language validity, ABI and runtime unchanged.
- [x] Cover the specified behavior with DIA-12 conformance and UI regressions.

**This change log is exhaustive for the H4 normative delta.**
...
* `[MOD-6]` A module MAY declare a supported language version with `#! language "X.Y"` on its first line; the package's `ember.toml` `language` key is the default. Mismatch with the compiler's supported set is `E0006`. The compiler's supported set MUST include every language version whose source it still accepts. For this revision the supported set MUST include `"0.9.7"`, `"0.9.6"`, `"0.9.5"`, `"0.9"`, `"0.8.5"`, `"0.8.4"`, and `"0.8.3"`. Selecting an older version selects that version's accepted-program and semantic rules; it does not silently opt the module into later language semantics. *(updated 2026-09-14; ODR-015)*
* `[MOD-6a]` **Patch-level language revisions.** A language version MAY be written as `X.Y` or `X.Y.Z`. `0.9` remains the pre-0.9.5 0.9 language contract; `0.9.5` selects inferred multi-region-view semantics; `0.9.6` preserves the complete 0.9.5 source-language accepted/rejected program sets and observable semantics while selecting that revision's reference-implementation and conformance contract; and `0.9.7` adds the explicit `@latebound` callable-type boundary of `[FN-6b]`/`[LT-7]`. The compiler MUST NOT silently enable `0.9.5`, `0.9.6`, or `0.9.7` semantics for a module that explicitly selects `0.9`. The supported set is exactly the set stated by `[MOD-6]`; the package manifest and module directive MUST resolve to the same exact language contract after normalization.
...
| Shape | Trigger | Required primary suggestion |
|---|---|---|
| **N1** unknown name | a path resolves to nothing, including a qualified path with a resolved namespace prefix and unresolved final member | the in-scope binding, field, method or item within Damerau–Levenshtein distance ≤ 2 (or ≤ ⅓ of the name's length), named exactly; at most 3, ranked by the existing N1 ordering |
...
For N1 on a qualified path of the form `<prefix>::<name>`, the compiler MUST
resolve `<prefix>` first using the existing module/namespace resolver. If that
prefix does not resolve, the ordinary path-resolution diagnostic MUST identify
the unresolved prefix component; the compiler MUST NOT search for suggestions
for `<name>`. Only after the prefix resolves may the compiler enumerate the
items eligible for lookup through that namespace under `[MOD-2]` and the
existing import/visibility rules. Private or otherwise inaccessible items MUST
NOT enter this candidate set; `pub(package)` items are eligible only where the
existing package-visibility rule permits access. Candidate discovery MUST reuse
the resolver's namespace/member facts, not a diagnostic-only symbol table or a
scan of unrelated modules.

The spelling-distance comparison MUST use only the unresolved final identifier
`<name>` and the candidate's final identifier. It MUST use the existing N1
distance threshold, ranking and maximum suggestion count; qualified names add
no ranking key or separate algorithm. For each candidate, help MUST render the
original source-spelled prefix followed by `::` and the candidate's final name.
This preserves aliases exactly: `io::pritn` suggests `io::print`, not a
canonical declaration path. For nested paths, preserve every source prefix
component in the same way.

The primary unresolved-name span and the machine-applicable replacement MUST
cover only the unresolved final identifier token. For `io::pritn`, the help
renders `io::print`, while the edit replaces only `pritn` with `print`; it MUST
NOT rewrite `io::pritn` as one whole-path edit. The diagnostic remains in the
existing unknown-name family and MUST NOT introduce a diagnostic code or alter
which programs are accepted. A direct `from a.b import x` binding continues to
use ordinary, unqualified N1.
