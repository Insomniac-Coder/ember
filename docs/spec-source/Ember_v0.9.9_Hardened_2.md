...
**Version:** 0.9.9_Hardened_2
**Supersedes:** 0.9.9_Hardened_1, 0.9.8_Hardened_3 (development target) and 0.8.5_Hardened_1 (adopted). This document is
...
| Appendix H | Changes from 0.9.8_Hardened_3, and from 0.9.9_Hardened_1 (§H.4) |
...
* `[LEX-15]` *(changed in 0.9.9)* The table above is the complete reserved set. **Contextual
  ...
  before `class`; `from` at the start of an import; `gen` and `once` immediately before `fn`; `safe` before `fn` in an extern block; `some`
...
* `[HASH-3]` *(changed in 0.9.9)* `Map` and `Set` MUST NOT weaken equality to compensate for an
  ...
  undefined behaviour, an out-of-bounds access or an operation that does not terminate.
...
* `[TYP-23]` *(changed in 0.9.9)* Inference is local to a function body and bidirectional. Function
  ...
  later assignment converts by `[TYP-5]` rule 11. Expected types flow into literals, lambdas, `None`,
  `Ok(…)`, `Err(…)` and generic calls. A lambda with unannotated parameters and no expected callable
  type is `E2061`. Untyped literals are resolved last.
...
* `[FN-10]` *(new in 0.9.9)* A function whose return type is `Result[void, E]` returns `Ok(())` when
  ...
  still allowed. No other return type has an implicit value.
...
* `[CLS-4]` *(changed in 0.9.9)* A class is final unless declared `open` or `abstract`. Methods are
  ...
  `E2110`. A derived `init` MUST call `super.init(…)` exactly once (`[CLS-11]`). Fields cannot be
...
* `[CTL-1]` *(changed in 0.9.9)* `for pattern in e:` iterates:
  ...
  * an `Iterator` value: calls `next` until `None`.
  ...
  values); a `str` yields its `char`s. `for x in
...
* `[CLO-7]` *(changed in 0.9.9)* Standard-library APIs that store or send a callback (`thread.spawn`,
  `jobs.submit`, `Option.map` returning a value computed later, event registries) take it as a
...
* `[STD-15]` *(new in 0.9.9)* **`Array[T]`** is a growable contiguous list (Python's `list`).
  ...
  `dedup`, `reverse`, `sort` (stable; `T: Ord`; an inconsistent order permutes but never corrupts, `[HASH-3]`), `sort_by_key(f)`, `sort_by(cmp)`, `sorted() ->
...
* `[STD-16]` *(new in 0.9.9)* **`Map` operations.** `m[k]` reads the value and panics when the key is
  ...
  `entry(k).or_insert(v) -> ref mut V`, `k in m` and `for k in m:` (by key, as Python's `dict`), `keys`,
  `values`, `values_mut`, `items` (`(ref K, ref V)` pairs: `for k, v in m.items():`), `len`, `is_empty`, `clear`, `retain`.
