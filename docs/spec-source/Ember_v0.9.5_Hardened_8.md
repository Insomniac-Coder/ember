...
**Immediate predecessor:** 0.9.5_Hardened_7
...
their mutable API family, callable-type mode syntax, helper input modes, and `Callable` bridge by
owner decision, and records evidence status honestly. No implementation or conformance result is
...
> preserves the full mode signature as compiler-known type metadata (this H8). H4 through H7 remain
...
> throughout the language. ODR-004 through ODR-007 are closed. No helper consumes its input view,
...
### 0.9.5_Hardened_8 language-conformance condition

`0.9.5_Hardened_8` MUST NOT be declared conformant unless `[LT-8]`–`[LT-13]`,
...
`0.9.5_Hardened_8` MUST NOT be declared clean unless:
...
3. the active revision lineage ends at `0.9.5_Hardened_8`;
4. the current consistency checklist targets `0.9.5_Hardened_8`;
...
## 0.9.5_Hardened_8 Evidence Status — No Implementation Claim
...
## 0.9.5_Hardened_8 consolidation record
...
# Appendix B — 0.9.5_Hardened_8 Consistency Checklist

The following checks are mandatory CI checks for the consolidated specification itself:

1. Exactly one current version declaration exists and names `0.9.5_Hardened_8`.
2. The immediate predecessor is `0.9.5_Hardened_7`.
