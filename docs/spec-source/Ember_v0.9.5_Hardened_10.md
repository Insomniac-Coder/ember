...
**Immediate predecessor:** 0.9.5_Hardened_9
...
their mutable API family, callable-type mode syntax, helper input modes, `Callable` bridge, the
narrow Arena-backed return-provenance boundary, and the Arena bulk-initialization safety contract
by owner decision, and records evidence status honestly. No implementation or conformance result is
...
* `[TST-23]` Conformance MUST cover the Arena initialization boundary:
  ...
  10. `alloc_array`'s `Zeroable`-before-`Default` precedence, `E2040` fallback,
      `!needs_drop` rejection, and transactional cursor rollback.
...
## 0.9.5_Hardened_10 Audit Record
...
| AUD-095-52 | Failure and phase ordering | Made `Default` initialization transactional and required the core `Zeroable` predicate/built-ins during the Arena phase while leaving general derive generation in its later phase. |
...
Because H9 was already frozen, ADR-023 requires this clarification to be issued as H10. The adopted
0.8.5 source remains unchanged; H10 is the new frozen, not-yet-adopted development target and makes
no blanket implementation or conformance claim.

### 0.9.5_Hardened_10 language-conformance condition
...
3. the active revision lineage ends at `0.9.5_Hardened_10`;
4. the current consistency checklist targets `0.9.5_Hardened_10`;
...
## 0.9.5_Hardened_10 Evidence Status — No Implementation Claim
...
## 0.9.5_Hardened_10 consolidation record
...
* `[ARN-10]` **Transactional `Default` initialization.** `alloc_array[T](count)` is transactional with respect to the Arena's allocation cursor. If any `T.default()` construction fails or panics, the cursor MUST be restored to its state immediately before the call and no partial result becomes reachable. Because `[ARN-3]` requires `!needs_drop(T)`, rollback requires no element destructor. `alloc_uninit` has no constructor-failure phase.
...
# Appendix B — 0.9.5_Hardened_10 Consistency Checklist

The following checks are mandatory CI checks for the consolidated specification itself:

1. Exactly one current version declaration exists and names `0.9.5_Hardened_10`.
2. The immediate predecessor is `0.9.5_Hardened_9`.
...
29. `[TST-23]` covers safe writes, unsafe assertions, full-span initialization, ownership transfer, layout, zero validity, capability precedence, and rollback without turning unsafe preconditions into implementation-defined runtime tracking.
