...
**Immediate predecessor:** 0.9.6_Hardened_5
...
The 0.9.5 language revision introduced inferred multi-region view structs. This owner-selected
0.9.6 architecture-line revision preserves the 0.9.5 accepted/rejected program sets and observable
semantics while consolidating the reference compiler around shared semantic facts, explicit MIR
analysis/verification boundaries, and one capability model. It also incorporates the owner-approved
clarification of `[ARN-10]` under v1's abort-only panic policy. The new `0.9.6` version selector is
additive; it does not silently change a module that selected an older contract. No implementation or
conformance result is asserted merely because the specification requires one. The H5 predecessor
incorporated the owner-approved ODR-011 completion of the previously named but under-specified
Arena-backed collection surface, the owner-approved ODR-012 completion of the `Hash`/`Hasher`
boundary on which the Map contracts already depended, and the owner-approved post-H4 simplicity
consolidation. This H6 successor completes the already advertised `Span[T]`/`MutSpan[T]` iterator,
chunk, and raw-pointer surface through the owner-approved ODR-014 contract.

> **Authority boundary:** This frozen revision records the owner-selected 0.9.6 development target, but
...
of the current H6 document, which is identified in the front matter.
...
* `[MOD-6]` A module MAY declare a supported language version with `#! language "X.Y"` on its first line; the package's `ember.toml` `language` key is the default. Mismatch with the compiler's supported set is `E0006`. The compiler's supported set MUST include every language version whose source it still accepts. For this revision the supported set MUST include `"0.9.6"`, `"0.9.5"`, `"0.9"`, `"0.8.5"`, `"0.8.4"`, and `"0.8.3"`. Selecting an older version selects that version's accepted-program and semantic rules; it does not silently opt the module into later language semantics. *(hardened 2026-09-13; HC-096-01)*
* `[MOD-6a]` **Patch-level language revisions.** A language version MAY be written as `X.Y` or `X.Y.Z`. `0.9` remains the pre-0.9.5 0.9 language contract; `0.9.5` selects inferred multi-region-view semantics; and `0.9.6` preserves the complete 0.9.5 source-language accepted/rejected program sets and observable semantics while selecting this revision's reference-implementation and conformance contract. The compiler MUST NOT silently enable `0.9.5` or `0.9.6` semantics for a module that explicitly selects `0.9`. The supported set is exactly the set stated by `[MOD-6]`; this rule explains why `"0.9"`, `"0.9.5"`, and `"0.9.6"` remain distinct selectable contracts even where the latter two accept and interpret ordinary source identically. The package manifest and module directive MUST resolve to the same exact language contract after normalization.
...
# Appendix B — 0.9.6_Hardened_6 Consistency Checklist

The following checks are mandatory CI checks for the consolidated specification itself:

1. Exactly one current version declaration exists and names `0.9.6_Hardened_6`.
2. The immediate predecessor is frozen `0.9.6_Hardened_5`, which remains byte-for-byte unchanged.
3. `[MOD-6]` and `[MOD-6a]` name the supported `0.9.6`, `0.9.5`, `0.9`, and 0.8.x source-language contracts explicitly; `0.9` does not silently enable later semantics, H5 changes H4 only through the owner-approved post-H4 simplicity architecture/process contract, and H6 changes H5 only through the owner-approved ODR-014 completion.
