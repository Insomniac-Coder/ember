...
**Version:** 0.9.5_Hardened_2  
**Status:** Iteratively hardened implementation-ready specification  
**Immediate predecessor:** 0.9.5_Hardened_1  
**Versioning model:** A hardening increments `Hardened_N` when it only adds implementation detail, invariants, tooling requirements, or editorial clarification without changing accepted language semantics. When an iteration adds, completes, or materially improves a language feature, the hardening counter resets to `1` and the language version increments. Under this model, the simulated implementation cycle crossed the language boundary when the previously specified-but-unimplemented interior-mutability primitives were completed and conformance-pinned, producing `0.9_Hardened_1`; subsequent implementation hardenings produced `0.9_Hardened_2` through `0.9_Hardened_14`. This revision crosses the language boundary because inferred multi-region view structs materially improve the accepted borrow/type semantics. It is named `0.9.5_Hardened_1` rather than `0.10`; the current document is a subsequent hardening, `0.9.5_Hardened_2`. The hardening adds compiler-contract detail and does not change the accepted source semantics of the feature.
...
7. `[WK-5]`–`[WK-10]` are implemented as diagnostic-only ownership-cycle analysis and do not alter ARC semantics;
8. `[EXC-8]`–`[EXC-14]` are implemented and generated-code tests demonstrate safe loop hoisting without weakening checked semantics;
9. `[LT-8]`–`[LT-13]` are implemented through ordinary late-bound callback regions; the former multi-region storable-view restriction is now superseded by `0.9.5_Hardened_1`;
...
This revision is the result of a simulated compiler/runtime/toolchain implementation pass over `Ember 0.8.5_Hardened_1`. The process was intentionally iterative:
...
## [IMP-8] Active-versus-historical specification boundary

Only text outside `HISTORICAL_BASELINE_BEGIN` / `HISTORICAL_BASELINE_END` blocks is eligible to define current normative semantics. Historical material MUST NOT be consumed by rule-index, grammar, version-selection, owner-decision, or conformance extraction unless an extractor explicitly requests historical lineage.
...
| 0.9_Hardened_1 | **Language revision** | completed and conformance-pinned `UnsafeCell` / `RefCell` implementation contract |
...
## HISTORICAL_BASELINE_BEGIN — 0.8.5_Hardened_1 (NON-NORMATIVE)

**Historical baseline integrated below:** 0.8.5_Hardened_1. Everything inside the historical-baseline block is NON-NORMATIVE and retained for lineage/compatibility reference only. The normative current revision is 0.9.5_Hardened_1.
...
# Appendix B — 0.9.5_Hardened_2 Consistency Checklist

The following checks are mandatory CI checks for the consolidated specification itself:

1. Exactly one current version declaration exists and names `0.9.5_Hardened_2`.
2. The immediate predecessor is `0.9.5_Hardened_1`.
...
9. No historical change-log entry is treated as a current normative rule.
10. Every new `[HC-095-*]` amendment, including `HC-095-02`, is covered by at least one specification-consistency test.
11. `[LT-14]`–`[LT-43]` have both accept and reject conformance coverage where applicable, including per-field expiration, callable access summaries, and escape tests.
12. Multi-region view layouts and generated code are region-erased and match the equivalent separate-`Span` representation.
13. The MIR verifier checks multi-region provenance and callable access summaries after every relevant transformation.
14. Rule IDs are unique and historical IDs are never reused for new meanings.
15. Every canonical artifact named by `[IMP-3]` has exactly one authoritative owner.
