...
**Version:** 0.9.5_Hardened_3  
**Status:** Consolidated, mechanically audited implementation-ready specification; implementation/conformance evidence is not claimed  
**Immediate predecessor:** 0.9.5_Hardened_2  
...
semantics. When an iteration adds, completes, or materially improves a language feature, the
hardening counter resets to `1` and the language version increments. The 0.9.5 language revision
...
extraction defects, and records evidence status honestly. No implementation or conformance result
is asserted merely because the specification requires one.

> **Authority:** This document remains the sole normative source for Ember. Historical text is retained for lineage, but only the rules in this revision govern the implementation.
...
9. `[LT-8]`–`[LT-13]` are specified through ordinary late-bound callback regions; the former multi-region storable-view restriction is now superseded by `0.9.5_Hardened_1`;
...
exclusivity. Its `std.borrow.with_views2/3/4` helpers remain valid and useful, but the multi-view
composition limitation they worked around is superseded by the source-language feature in
`0.9.5_Hardened_1`. The ARC and exclusivity hardenings remain normative unchanged.
...
The `std.borrow.with_views2/3/4` helpers introduced here remain conforming library APIs. They
are no longer the only way to compose independently-lived views. `0.9.5_Hardened_1` adds
inferred multi-region `@view struct` semantics, specified below in the active language revision.
Implementations MAY continue to lower these helpers directly through `[LT-7]`; they MUST NOT
use the helpers as a reason to reject a conforming multi-region `@view struct`.

The original `[LT-8]`–`[LT-13]` obligations remain as compatibility tests for the helper APIs,
with their old no-escape and no-hidden-allocation guarantees unchanged.
...
* `[CLI-15]` `ember inspect --cycle <path>` MUST report the same ownership graph used by `[WK-5]`,
...
## [IMP-8] Inherited-baseline and supersession extraction

The consolidated specification has one inherited normative baseline delimited by
`INHERITED_NORMATIVE_BASELINE_BEGIN` / `INHERITED_NORMATIVE_BASELINE_END`. Rule definitions inside
that block are eligible for normative extraction. Historical change-log prose inside the block is
descriptive only.

When an inherited rule ID is superseded by the current revision, the supersession manifest is
authoritative and the current definition is selected. A rule ID MUST have exactly one selected
definition after applying the manifest. Rule-index, grammar, version-selection, owner-decision and
conformance extraction MUST apply this selection before building their active indexes.
...
## [RT-1] Canonical runtime ABI name
...
## [RT-4] Handle generation exhaustion
...
| 0.9_Hardened_1 | **Language revision** | specified `UnsafeCell` / `RefCell` implementation contract; implementation evidence remains external |
...
| AUD-014-03 | High | Independent view lifetimes remain forbidden in storable `@view struct`s, but canonical `std.borrow.with_views` helpers provide allocation-free late-bound callback composition for common multi-view APIs. |
...
### 0.9.5_Hardened_2 acceptance condition

`0.9.5_Hardened_2` MUST NOT be declared conformant unless `[LT-14]`–`[LT-43]`, `[MIR-REG-1]`, `[TYP-15]`, `[TYP-15a]`, `[VERIFY-3]`, `[TST-17]`–`[TST-19]`, and `[BUD-8]` all have passing evidence; callable access summaries are verified against actual MIR field accesses; unknown calls conservatively require all slots; stale summaries invalidate dependents; and generated C/LLVM contains no runtime region bookkeeping or summary-dispatch machinery.

### 0.9.5 acceptance condition

`0.9.5_Hardened_2` MUST NOT be declared conformant unless `[LT-14]`–`[LT-43]`, `[LT-31a]`, `[MIR-REG-1]`, `[TYP-15]`, `[TYP-15a]`, `[VERIFY-3]`, `[TST-17]`, `[TST-18]`, `[TST-19]`, and `[BUD-8]` all have passing conformance evidence; the MIR verifier proves region-slot preservation; the generated C/LLVM contains no runtime region bookkeeping; and the RageV workload demonstrates that multi-region ECS/SoA views do not introduce measurable overhead versus equivalent separate `Span` parameters after equivalent optimisation.


This revision incorporates the complete Hardened_14 hardening record, the accepted 0.9.5 multi-region-view language revision, and the callable-provenance hardening of Hardened_2. The current revision identity, historical-baseline boundary, feature lineage, owner-decision guidance, standard-library ownership, hot-reload transaction contract, and multi-region view safety contract are explicit.

### Acceptance condition

`0.9.5_Hardened_2` MUST NOT be declared clean unless:
1. the document has one current version identity;
2. historical material is machine-delimited and excluded from normative extraction;
3. the active revision lineage ends at `0.9.5_Hardened_2`;
4. the current consistency checklist targets `0.9.5_Hardened_2`;
...
## 0.9.5_Hardened_3 Evidence Status — No Implementation Claim
...
## 0.9.5_Hardened_3 Consolidation and Supersession Manifest

This section is normative and is applied before rule-index generation.

### Consolidation rule

The 0.8.5_Hardened_1 body is the inherited normative baseline. The 0.9/0.9.5 hardening
contracts and accepted feature revisions amend that baseline by stable rule ID. A later definition
wins only for the exact rule ID it explicitly supersedes; unrelated inherited rules remain intact.
No inherited rule is deleted merely because it is old.

### Explicit supersessions

| Inherited rule | Current authority | Reason |
|---|---|---|
| `[LT-2]` one-region `@view struct` restriction | `[LT-14]`–`[LT-18]`, `[LT-25]`–`[LT-34]` | 0.9.5 makes multiple inferred region slots part of the language |
| `[TYP-15]` single-region storage formulation | current `[TYP-15]` | generalized to require the destination to outlive every region slot |
| `[TYP-15a]` one-region `BorrowList`/`ViewList` limitation | current `[TYP-15a]` | generalized to multi-region elements bounded by one container region |
| `[LT-6]` named lifetime reservation | current `[LT-6]` | retained; 0.9.5 does not introduce source-level lifetime names |
| `[LT-7]` late-bound callback regions | current `[LT-7]` plus `[LT-8]`–`[LT-13]` | retained and used by canonical `with_views` helpers |
| `[LT-8]`–`[LT-13]` callback composition | current `[LT-8]`–`[LT-13]` | recovered from Hardened 14 and retained as valid library APIs |
| `[MIR-REG-1]` inherited placement | canonical current definition in the active MIR contract | callable provenance is current normative semantics |
| `[CLI-15]` duplicate historical/current meanings | current `[CLI-15]` cycle inspection; `[CLI-17]` CLI-surface completeness | one stable meaning per rule ID |
| `E3064` legacy B13 | B13 explicitly reserved/non-emitted | 0.9.5 uses `E3065` for B14 |
| `E3065` B14 | diagnostic registry + B14 catalogue + required artifact paths | current provenance diagnostic |

### No semantic invention

If a rule ID, diagnostic, grammar production, attribute, ABI field, or standard-library API is
referenced but has no recoverable definition in the complete lineage, it is recorded as a
`SPEC_GAP` and blocks conformance. It MUST NOT be fabricated from another language's behavior.

## Canonical Rule-Index Registry

This registry is normative machine-readable metadata. Markdown headings are editorial structure
and are not, by themselves, rule definitions for `tools/rule_index.py`. Every heading-defined
rule is mirrored here exactly once. The registry points to the authoritative section and does not
create a second semantic definition.

* `[OQ8-1]` **ECS is library-owned** — authoritative definition in its heading section.
* `[OQ8-2]` **RageV-compatible sparse-set layout** — authoritative definition in its heading section.
* `[OQ8-3]` **Stable query iteration** — authoritative definition in its heading section.
* `[OQ8-4]` **Access-set enforcement** — authoritative definition in its heading section.
* `[OQ8-5]` **Comptime specialization without language specialization** — authoritative definition in its heading section.
* `[OQ8-6]` **FFI interoperability** — authoritative definition in its heading section.
* `[OQ8-7]` **No ECS compiler magic** — authoritative definition in its heading section.
* `[IMP-1]` **Specification-to-implementation traceability** — authoritative definition in its heading section.
* `[IMP-2]` **No semantic invention** — authoritative definition in its heading section.
* `[IMP-3]` **Canonical artifacts** — authoritative definition in its heading section.
* `[IMP-4]` **Implementation completeness** — authoritative definition in its heading section.
* `[IMP-5]` **Canonical type identity** — authoritative definition in its heading section.
* `[IMP-6]` **MIR source provenance** — authoritative definition in its heading section.
* `[IMP-7]` **Compiler pass contracts** — authoritative definition in its heading section.
* `[IMP-8]` **Inherited-baseline and supersession extraction** — authoritative definition in its heading section.
* `[IMP-9]` **Canonical standard-library definition ownership** — authoritative definition in its heading section.
* `[COMP-1]` **Drop-check phase** — authoritative definition in its heading section.
* `[GEN-COH-1]` **Concrete implementation ownership** — authoritative definition in its heading section.
* `[GEN-COH-2]` **Blanket overlap** — authoritative definition in its heading section.
* `[CLO-ABI-1]` **Closure environment** — authoritative definition in its heading section.
* `[CLO-ABI-2]` **Capture-free closures** — authoritative definition in its heading section.
* `[COR-ABI-1]` **Coroutine frame** — authoritative definition in its heading section.
* `[COR-ABI-2]` **Destruction** — authoritative definition in its heading section.
* `[RT-1]` **Canonical runtime ABI name** — authoritative definition in its heading section.
* `[RT-2]` **Reference-count overflow** — authoritative definition in its heading section.
* `[RT-3]` **Atomic ordering matrix** — authoritative definition in its heading section.
* `[RT-4]` **Handle generation exhaustion** — authoritative definition in its heading section.
* `[LAY-1]` **Canonical layout descriptor** — authoritative definition in its heading section.
* `[FFI-IMPL-1]` **Canonical ABI descriptor** — authoritative definition in its heading section.
* `[FFI-IMPL-2]` **C++ ABI fingerprint** — authoritative definition in its heading section.
* `[FFI-IMPL-3]` **Generated thunk tests** — authoritative definition in its heading section.
* `[FFI-CB-1]` **Callback generation identity** — authoritative definition in its heading section.
* `[HR-IMPL-1]` **Reload state machine** — authoritative definition in its heading section.
* `[HR-IMPL-2]` **Epoch publication** — authoritative definition in its heading section.
* `[HR-IMPL-3]` **Transaction failure** — authoritative definition in its heading section.
* `[DET-IMPL-1]` **Floating-point environment** — authoritative definition in its heading section.
* `[DET-IMPL-2]` **Differential determinism oracle** — authoritative definition in its heading section.
* `[STD-IMPL-1]` **Normative API/source correspondence** — authoritative definition in its heading section.
* `[STD-IMPL-2]` **Standard-library conformance** — authoritative definition in its heading section.
* `[VERIFY-1]` **MIR verification** — authoritative definition in its heading section.
* `[VERIFY-2]` **Pass preservation** — authoritative definition in its heading section.

`tools/rule_index.py` MUST consume this registry together with bullet-form rule definitions and
the inherited-baseline supersession manifest. The selected active index MUST contain exactly one
definition per rule ID.
## INHERITED_NORMATIVE_BASELINE_BEGIN — 0.8.5_Hardened_1

**Inherited normative baseline:** the still-valid 0.8.5_Hardened_1 language contract is incorporated below as normative inherited material. Its explanatory history remains historical prose, but every inherited rule not superseded by the current revision remains normative. The current revision's supersession manifest determines which inherited rule definition is authoritative when an ID has changed.
**Versioning:** two numbers move independently. The **language version** moves when the set of accepted programs changes — 0.8.4 exists because of one such change, S1, the owner's resolution of ERR-044, which admits a static-region view into storage that has no bounding region. The **hardening number** moves when the document gains implementation detail and no rule changes meaning; it resets to 1 with each language version. A hardening may never carry a semantic change: that is what forces the language number instead. That is why 0.8.4 was cut as `0.8.4_Hardened_1` rather than `0.8.3_Hardened_2` when S1 landed, and why **this file is `0.8.5_Hardened_1`** — S2, S3 and S4, the owner's rulings of 2026-09-10, are three more such changes, so the language number moved again and the hardening number reset.
...
**Hardening:** A hardening adds implementation detail that the revision left out and changes no rule's meaning. It never moves the language version — a change that does is not a hardening, which is why S1 made 0.8.4 rather than `0.8.3_Hardened_2`, and why S2/S3/S4 made 0.8.5 rather than `0.8.4_Hardened_3`. Source files may still declare `#! language "0.8.3"` or `#! language "0.8.4"` for source compatibility; `#! language "0.8.5"` selects the 0.8.5 contract, and **all three are accepted** (`[MOD-6]`). What a hardening contains is its own change-log section; every edit is marked in place and recorded with its justification in `docs/spec-amendments.md`. The owner's file is preserved untouched at `docs/spec-source/as-received/`.
**Lineage:** the immediate predecessor of this revision is **0.8.4**, which was authored from **0.8.3**. 0.8.3 in turn carries the ancestry below, and every decision in it is retained here — immediate predecessor and historical ancestry are different things and the chain `0.8.3 → 0.8.4 → 0.8.5` is the one a reader needs. The ancestry: 0.8.3 was authored from **0.8.2c**, which was authored from 0.8.2b, which was authored from 0.8.1, which was authored from 0.8, which was authored from 0.7.2, which was authored from 0.7.1, which was authored from 0.6.3 taking the design — not the text — of the 0.7 draft for hot reload, the compile-time budget and the C++ boundary. The 0.7 draft was itself authored from 0.3 and silently reverted 239 rules settled in 0.4 through 0.6.2; every one of those is retained here. **A revision of this document MUST be authored from the immediately preceding revision.**
...
**Status:** Historical baseline text incorporated into the 0.9_Hardened_11 consolidated specification
...
* `[MOD-6a]` **Patch-level language revisions.** A language version MAY be written as `X.Y` or `X.Y.Z`. `0.9` remains the pre-0.9.5 0.9 language contract; `0.9.5` selects the inferred multi-region-view semantics of this revision. The compiler MUST NOT silently enable `0.9.5` semantics for a module that explicitly selects `0.9`. The supported set for this revision MUST include `"0.9.5"` in addition to the versions listed by `[MOD-6]`. The package manifest and module directive MUST resolve to the same exact language contract after normalization.
...
* `[CLI-17]` `--report=instantiations` (`[MONO-4]`), `--build-id` (`[BLD-13]`), `--reload` and `--reload --explain` (`[HR-18b]`), `--hot` (`[HR-33]`), `--timings` (`[BUD-6]`) and `ember inspect --deterministic` (`[DET-9]`) are the surfaces 0.6.3 adds. Each MUST appear in `ember --help` and in XXIII §3's construct list, per `[DIA-6a]`'s completeness check.
...
## [DIA-18] E3065 diagnostic artifact completeness
...
## INHERITED_NORMATIVE_BASELINE_END — 0.8.5_Hardened_1

Everything between the inherited-baseline markers is part of the consolidated normative baseline, subject to the supersession manifest and current-revision amendments above. No text after this marker is treated as inherited 0.8.5 language.

# Appendix B — 0.9.5_Hardened_3 Consistency Checklist

The following checks are mandatory CI checks for the consolidated specification itself:

1. Exactly one current version declaration exists and names `0.9.5_Hardened_3`.
2. The immediate predecessor is `0.9.5_Hardened_2`.
...
9. The inherited baseline has exactly one machine-delimited begin marker and one end marker.
10. The inherited baseline is normative by default; only explicitly historical prose is excluded.
11. The supersession manifest produces exactly one selected definition for every rule ID.
12. `[LT-8]`–`[LT-13]` are present as recovered canonical library rules and have regression coverage obligations.
13. `[LT-14]`–`[LT-43]` have both accept and reject conformance coverage where applicable.
14. `[MIR-REG-1]` is in the selected active rule index and is not lost through baseline filtering.
15. `[CLI-15]` has exactly one definition and `[CLI-17]` owns the historical CLI-surface completeness rule.
...
19. Every heading-defined rule is mirrored by the canonical machine-readable rule-index registry.
20. Rule IDs are unique after supersession selection and historical IDs are never silently reused for a new meaning.
