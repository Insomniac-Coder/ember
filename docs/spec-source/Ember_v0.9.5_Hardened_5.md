...
**Immediate predecessor:** 0.9.5_Hardened_4
...
extraction defects, recovers the owner-supplied Hardened 14 callback-composition rules, and records
evidence status honestly. No implementation or conformance result is asserted merely because the
...
> **SOURCE RECOVERY COMPLETED — `[LT-8]`–`[LT-13]`:** on 2026-09-12 the owner supplied the
> missing Hardened 14 definitions reproduced in §H14.3. The transport-only Markdown escaping and
> broken code fences were normalized; the rule substance was not inferred. H4 remains frozen as the
> audit record of the gap, and this H5 successor is the recovered development target. The supplied
> shared-`Span` signatures do not by themselves define a `MutSpan` overload surface; that distinct
> accepted-program question is recorded rather than guessed in `docs/OWNER-QUEUE.md` ODR-005.
...
* `[LT-8]` **Canonical `with_views` helpers.** `std.borrow` MUST provide canonical,
  allocation-free helpers for two, three and four borrowed views. The normative signatures are
  structurally:
  ...
  The exact public spelling MAY use the module's overload convention, but the arity and type
  relationships MUST remain these. Their callback boundary uses `[LT-7]` late-bound regions, so
  each supplied view receives its own invocation-local region. The callback MUST complete before
...
* `[LT-11]` **Mutability follows ordinary borrowing.** `with_views` does not grant overlapping
  mutable access. Two mutable inputs whose sources may alias are rejected by the ordinary borrow
  checker; structurally disjoint inputs remain expressible through `split_at_mut`, `columns_mut`,
  `SoA`, or an established disjointness fact.
...
> **API-surface boundary:** the recovered structural signatures spell shared `Span` inputs, while
> `[LT-11]` also states the safety rule for mutable inputs. This recovery does not invent unprovided
> `MutSpan` overload signatures or combinations. Their exact public surface, if intended, remains
> ODR-005; the ordinary borrow rule itself is not in doubt.
...
### 0.9.5_Hardened_5 language-conformance condition

`0.9.5_Hardened_5` MUST NOT be declared conformant unless `[LT-8]`–`[LT-13]`,
...
`0.9.5_Hardened_5` MUST NOT be declared clean unless:
...
3. the active revision lineage ends at `0.9.5_Hardened_5`;
4. the current consistency checklist targets `0.9.5_Hardened_5`;
...
11. ODR-005 is resolved before conformance claims require a mutable `with_views` overload surface.

---

## 0.9.5_Hardened_5 Evidence Status — No Implementation Claim
...
## 0.9.5_Hardened_5 consolidation record
...
LT-8-through-LT-13 source gap is closed by the owner-supplied definitions in §H14.3; ODR-005
separately records the mutable-overload surface that those definitions do not spell.
...
# Appendix B — 0.9.5_Hardened_5 Consistency Checklist

The following checks are mandatory CI checks for the consolidated specification itself:

1. Exactly one current version declaration exists and names `0.9.5_Hardened_5`.
2. The immediate predecessor is `0.9.5_Hardened_4`.
...
12. `[LT-8]`–`[LT-13]` are present as recovered canonical library rules, their source and 0.9.5 supersession boundary are explicit, and they have regression coverage obligations.
