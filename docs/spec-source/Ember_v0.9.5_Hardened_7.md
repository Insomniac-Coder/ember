...
**Immediate predecessor:** 0.9.5_Hardened_6
...
their mutable API family and callable-type mode syntax by owner decision, and records evidence
status honestly. No implementation or conformance result is asserted merely because the
...
> **SOURCE RECOVERY AND CALLABLE-MODE BOUNDARY:** on 2026-09-12 the owner supplied the missing
> Hardened 14 definitions (H5), selected explicit all-mutable `_mut` variants (H6), and then extended
> callable types with borrowed/default, `mut`, and `owned` parameter modes (this H7). H4, H5, and H6
> remain frozen predecessor records. This revision consistently uses Ember's canonical type name
> `MutSpan[T]`; the rulings' `SpanMut[T]` spelling was normalized to the type already defined
> throughout the language. ODR-004 and ODR-005 are closed. ODR-006 is narrowed to the helper
> functions' own still-unmarked mutable input parameters, and ODR-007 records the unresolved bridge
> from mode-bearing `fn(...)` types to the existing `Callable[Args, R]` abstraction.
...
> type. Mixed shared/mutable overloads are not specified. Both families use the same ordinary region
> and borrow machinery; the mutable family creates no new aliasing rule.
...
### 0.9.5_Hardened_7 language-conformance condition

`0.9.5_Hardened_7` MUST NOT be declared conformant unless `[LT-8]`–`[LT-13]`,
`[LT-11a]`, `[TST-20]`,
...
`0.9.5_Hardened_7` MUST NOT be declared clean unless:
...
3. the active revision lineage ends at `0.9.5_Hardened_7`;
4. the current consistency checklist targets `0.9.5_Hardened_7`;
...
12. ODR-006 is resolved before the mutable helper functions' own input modes are claimed conformant;
13. ODR-007 is resolved before mode-bearing callable types are mapped to `Callable` or claimed conformant.

---

## 0.9.5_Hardened_7 Evidence Status — No Implementation Claim
...
## 0.9.5_Hardened_7 consolidation record
...
# Appendix B — 0.9.5_Hardened_7 Consistency Checklist

The following checks are mandatory CI checks for the consolidated specification itself:

1. Exactly one current version declaration exists and names `0.9.5_Hardened_7`.
2. The immediate predecessor is `0.9.5_Hardened_6`.
...
24. ODR-006 is resolved before the helper functions' own unmarked mutable inputs are treated as implementation-ready or conformant.
25. ODR-007 is resolved before mode-bearing `fn(...)` types are lowered through `Callable[Args, R]` or claimed conformant.
