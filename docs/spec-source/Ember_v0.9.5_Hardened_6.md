...
**Immediate predecessor:** 0.9.5_Hardened_5
...
their mutable API boundary by owner decision, and records evidence status honestly. No
implementation or conformance result is asserted merely because the specification requires one.
...
> **SOURCE RECOVERY AND API BOUNDARY COMPLETED — `[LT-8]`–`[LT-13]`:** on 2026-09-12 the
> owner first supplied the missing Hardened 14 definitions, frozen in H5, and then ruled that the
> helpers also have explicit all-mutable `_mut` variants. H4 remains the frozen source-gap record and
> H5 remains the frozen recovered-but-ambiguous predecessor. This H6 successor uses Ember's canonical
> type name `MutSpan[T]` for the mutable signatures; the ruling's `SpanMut[T]` spelling was normalized
> to the type already defined throughout the language. ODR-004 and ODR-005 are closed. The separate
> question of how mutable callback parameter modes are represented by Ember's currently mode-free
> `fn(...)` type grammar is recorded as ODR-006 rather than answered by inference.
...
* `[LT-8]` **Canonical `with_views` helpers.** `std.borrow` MUST provide canonical,
  ...
  variants. The normative signatures are structurally:
  ...
  with_views2_mut[A, B, R](a: MutSpan[A], b: MutSpan[B],
                           f: fn(MutSpan[A], MutSpan[B]) -> R) -> R
  with_views3_mut[A, B, C, R](a: MutSpan[A], b: MutSpan[B], c: MutSpan[C],
                              f: fn(MutSpan[A], MutSpan[B], MutSpan[C]) -> R) -> R
  with_views4_mut[A, B, C, D, R](a: MutSpan[A], b: MutSpan[B],
                                 c: MutSpan[C], d: MutSpan[D],
                                 f: fn(MutSpan[A], MutSpan[B], MutSpan[C], MutSpan[D]) -> R) -> R
  ```
...
* `[LT-11]` **Mutability follows ordinary borrowing.** `with_views` does not grant overlapping
  mutable access; the `_mut` variants accept `MutSpan[T]` and therefore remain subject to the
  ordinary exclusive-borrow rules. Mutable inputs whose sources may alias are rejected by the
...
> `MutSpan`. Mixed shared/mutable overloads are not specified. Both families use the same ordinary
> region and borrow machinery; the mutable family creates no new aliasing rule.
...
### 0.9.5_Hardened_6 language-conformance condition

`0.9.5_Hardened_6` MUST NOT be declared conformant unless `[LT-8]`–`[LT-13]`,
...
`0.9.5_Hardened_6` MUST NOT be declared clean unless:
...
3. the active revision lineage ends at `0.9.5_Hardened_6`;
4. the current consistency checklist targets `0.9.5_Hardened_6`;
...
12. ODR-006 is resolved before the mutable helper family is claimed conformant.

---

## 0.9.5_Hardened_6 Evidence Status — No Implementation Claim
...
## 0.9.5_Hardened_6 consolidation record
...
# Appendix B — 0.9.5_Hardened_6 Consistency Checklist

The following checks are mandatory CI checks for the consolidated specification itself:

1. Exactly one current version declaration exists and names `0.9.5_Hardened_6`.
2. The immediate predecessor is `0.9.5_Hardened_5`.
...
23. ODR-006 is resolved before the `_mut` helper family is treated as implementation-ready or conformant.
