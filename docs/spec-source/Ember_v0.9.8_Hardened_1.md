...
**Version:** 0.9.8_Hardened_1
**Immediate predecessor:** 0.9.7_Hardened_3 (immutable; preserved as the diff base)
**Status:** Frozen development target. Implementation and conformance evidence is **not** claimed; normative repository adoption still requires the recorded adoption gates.
    ...
    Specification    frozen for this target. ODR-017 is closed here by the
                     owner-selected `Shared[T]` and generalized weak-owner API.
                     The prior H3 diagnostic-identity work remains unchanged.
...
**this file is `0.9.8_Hardened_1` because ODR-017 supplies previously undefined
public ownership and borrowing semantics.**
...
## 0.9.8_Hardened_1 authority and evidence status

This file is the frozen development target selected by the owner ruling that
closes ODR-017.  It is authoritative for the `Shared[T]`/`Weak[O]` work only
after its recorded source hash is accepted through
...
`as-received/ODR-017_Shared_Weak_API_completion.md` and ADR-037 records its
application here.
...
ember explain --cycle <Class[.field]>   why that field is cycle-prone, and the Weak[T] edge to break ([WK-9])
...
| Cycle inspection | `ember inspect --cycle` reports the whole ownership graph, `ember explain --cycle` answers for one field, and `[WK-4]`'s leak reporter is the run-time view of the same data |
