...
guarantee, reopens no confirmed ADR, and changes no existing rule's meaning except
where a row below says so. Every feature is opt-in: a v0.5 program compiles
unchanged and acquires none of it by accident.
...
* `[RNG-5]` Arithmetic on a range type yields its **representation**, not the
  range type: `r * 2.0` is `f32`. Producing a range type again is a construction
  and goes through `[RNG-3]`. The alternative — a range-arithmetic result type
  per operator — has no honest answer for `Roughness * Roughness` and is
  rejected.
...
* `[EFF-18]` The effect set gains **`Io`** (file, socket, console) and
  **`Lock`** (acquiring a synchronisation primitive that may wait), separate
  from `Block` and `Sync`, so that "this function touches the disk" and "this
  function may wait" are different questions — which is what a frame budget
  needs. `@noio` and `@nolock` are the corresponding contracts and behave under
  `[EFF-6]`, `[EFF-8]` and `[EFF-2]` exactly as `@noalloc` does. The full set is
...
* `[CTR-7]` Contracts are part of a function's public interface. `[EFF-4]`'s
  ...
  for effects (`E4052`).
* `[CTR-8]` `--contracts=off|debug|all` selects where checks are emitted,
  defaulting to `debug` under `debug` and `release`, and `off` under `shipping`.
  This is a **checking policy, not a semantic profile**: a contract that fails
  describes a program with a bug in every setting, so `[PRF-1]` is not weakened.
  A contract discharged under `[PRV-1]` emits no check in any setting.
...
on something that is neither a loop nor a recursive function.
...
* `[PRV-8]` `--verify=off|check|prove`. **Verification is not a profile axis.**
  It is orthogonal to `debug`/`release`/`shipping` and `[PRF-1]` is untouched:
  verification never changes which branch is taken nor what any expression
  evaluates to, only whether a check the program would have executed is still
  present.
...
* `[FFI-36]` **Ownership transfer is written at the call site.** A foreign call
  yielding an object Ember must destroy returns `Foreign[T]` — an unsafe handle —
  and becomes an owned Ember value only through `adopt(handle)`. The overlay
  declares whether adoption is permitted and what destroys the object; it does
  not perform the transfer invisibly. In review, the moment Ember became
  responsible for freeing something is a line that can be pointed at. Adopting a
  handle the overlay does not permit adopting, or adopting twice, is `E5052`.
...
* `[BLD-11]` A package declares which `[STD-6]` layers it links. Omitting
  `alloc` removes the allocating types from the build, so a reference to one is
  a name-resolution error (`E9030`) rather than a lint. This is stronger than a
  coding rule that says do not allocate, and it is checkable by a person reading
  the manifest.
