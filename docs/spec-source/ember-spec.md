...
* `[EFF-18]` **Effects are orthogonal.** `Io` (file, socket, console) and `Lock`
  ...
  effect model so a single operation may report all applicable effects. `RuntimeCheck(k)`'s kinds are `{Bounds, Overflow, Aliasing, Stale, Contract}`.
