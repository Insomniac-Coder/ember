...
`[TYP-15]` A view-typed value MUST NOT be stored in a place whose region is not outlived by the view's region. **This is the whole of the rule, and it is a condition on the view's region rather than on the kind of place.** Class fields, non-view struct fields, `static`s, `Box[T]` and `Shared[T]` contents, container elements and `owned fn` captures have **no bounding region**, so the only view they may hold is one whose region is `static` (`[LT-3]`) — a static region outlives everything, the destination included, so the condition above is met rather than waived. Any other view stored in one of them is `E3063 stored view may not outlive its source`, shape B12. So `class Foo: greeting: str = "hello"` is admitted and `foo.greeting = s` for a parameter `s` is not, because `s` may carry a caller's region that does not outlive the field. **This does not relax `[TYP-15a]`**: an owning container instantiated at a view type — `Array[str]`, `Map[str, V]`, `Array[MutSpan[T]]` — stays rejected whatever the region, because that rejection is at the type and not at the region. *(owner decision, ERR-044, 2026-09-09; see `docs/spec-amendments.md`)*  Views MAY live in locals, parameters, return values, and in tuple, enum, `Option` and `Result` payloads. Whether a generic container instantiated at a view type may itself become a view type is reserved (OQ-19).
...
## XXII.3 Stage 2 — Systems in Ember over the engine ECS (Phase 6)
