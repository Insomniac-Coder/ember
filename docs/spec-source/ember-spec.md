...
**Version:** 0.8.4_Hardened_1 **+ E5, pending a version decision** (supersedes 0.8.3; see the change log at the end of Part 0)
**Pending:** this file carries one editorial repair beyond the frozen cut — E5, which adds the missing `Nondet` member to `[EFF-18]`'s enumeration per the already-decided ERR-028. It changes no rule's meaning and moves no implementation, so it forces neither a language bump nor a new frozen cut by itself — but the header may not claim this file *is* the frozen `0.8.4_Hardened_1` artifact while differing from it. Whether E5 folds into the next hardening pass or cuts **Hardened_2** is the owner's call. Until then this file is "0.8.4_Hardened_1 plus E5" and says so.
