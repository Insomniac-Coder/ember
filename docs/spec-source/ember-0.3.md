...
**Version:** 0.3 (supersedes 0.2; see the change log at the end of Part 0)
...
* `[LEX-11]` A `##` comment that is not immediately followed (ignoring blank lines) by a declaration is a warning `W0001 dangling doc comment`.
...
| 3 | `and` | |
| 4 | `not` (prefix) | |
| 5 | `==` `!=` `<` `>` `<=` `>=` `is` `is not` `in` `not in` | non-associative; chaining `a < b < c` is `E0010` (no Python chaining) |
| 6 | `..` `..=` | non-associative |
| 7 | `\|` | |
| 8 | `^` | |
| 9 | `&` | |
...
`[GRM-10]` **Statement vs expression `match`.** `match e:` followed by arms using `pattern: block` is a statement; arms using `pattern => expr` make it an expression. Mixing is `E0011`.
...
Unknown attributes are `E0020` unless prefixed with a registered plugin namespace (`@ragev.field`). Recognised v1 attributes (full semantics where referenced):
...
* `[OWN-6]` `mem.take(mut place) -> T` (replaces with `Default`), `mem.replace(mut place, new) -> T`, `mem.swap(mut a, mut b)`, `mem.forget(owned x)` (skips drop, safe: leaking is not unsafe).
...
`[LT-6]` Explicit named lifetimes (`fn f['a, 'b](…)`) are **reserved for v2**; the grammar reserves the `'ident` token form (`E0007` in v1 with the message "named lifetimes are not supported in this version; restructure using @borrows or a view struct").
    ...
    match assert_disjoint(dst, src):
        Some(d, s):                       # d, s are *known* disjoint from here on
            for i in 0..d.len():
                d[i] = d[i] * 0.5 + s[i] * 0.5
        None:
            fallback_overlapping_blend(mut dst, src)
```
...
* `[DSJ-5]` `assert_disjoint_or_panic(a, b) -> (MutSpan[T], Span[T])` is the panicking form for code that treats overlap as a bug. Both forms carry `RuntimeCheck(Aliasing)` in the profile where the comparison is emitted; the comparison is elided when the compiler already knows the ranges are disjoint (the common `split_at_mut` case), in which case the effect is absent.
...
* `[EFF-9]` `RuntimeCheck(k)` enters a function's effect set when the function's own body emits a check of kind `k` **after** all elision passes have run (`[EXC-3]`, `[RC-2/3]`, bounds-check elimination), and propagates through the call graph like every other effect (`[EFF-1..3]`). It is therefore a statement about generated code, not about source syntax: a function whose checks are all elided does not carry the effect.
...
* `[TST-2]` `run-pass` files may contain `#! stdout:` followed by `#!` lines of expected output, and `#! exit: N`.
...
10. **`@static_safe` excludes only `RuntimeCheck(Aliasing)` (`[EFF-12]`)** — confirm; including `Bounds` would make the contract unsatisfiable for most real code, but a project that wants the stricter form has to wait for v2's `@no_runtime_checks`.
11. **Name of the language/CLI** — `ember`/`emberc` conflict check against existing packages on crates.io/PyPI/npm before publishing anything.

## XXII.2 Non-goals (v1)

No tracing GC; no exceptions; no implicit numeric narrowing; no function overloading; no macros; no named lifetimes; no specialisation/HKT; no C++ subclassing from Ember; no shader/kernel compilation; no dynamic typing; no whole-program-required optimisations for correctness; no stable Ember-to-Ember ABI (C ABI is the stable boundary); **no relaxed or unchecked safety mode** — `Cell`/`RefCell` (Part IX §7) and classes (Part VIII) are the sanctioned ways to express aliased mutation, and each keeps its check rather than removing it; **no assumption that is checked in one profile and assumed in another** (`[DSJ-8]`); **no cycle collector** — reclaiming reference cycles requires root identification, graph traversal and mutator cooperation, which is a tracing collector over the RC subgraph and reintroduces exactly what Part 0 removed; `Weak`, the leak report (`[WK-1]`) and `L3001` are the whole answer.
