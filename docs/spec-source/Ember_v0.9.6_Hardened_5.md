...
**Immediate predecessor:** 0.9.6_Hardened_4
...
conformance result is asserted merely because the specification requires one. This H5 successor
incorporates the owner-approved ODR-011 completion of the previously named but under-specified
Arena-backed collection surface and the owner-approved ODR-012 completion of the `Hash`/`Hasher`
boundary on which the Map contracts already depended, and materializes the owner-approved post-H4
simplicity consolidation as binding reference-compiler, tooling, documentation, and development
requirements without changing source-language semantics.
...
## 0.9.6_Hardened_5 consolidation record
...
protocol/API contract, ODR-013's static Hasher call boundary, and the owner-approved post-H4
simplicity architecture/process contract. It is normative within
...
the H5 target, not a verbatim historical snapshot. The front-matter authority boundary controls
...
of the current H5 document, which is identified in the front matter.
...
| `std.collections` | `Array`, `SmallArray`, `Deque`, `Map`, `Set`, `BitSet`, `Pool`, `Handle`, `SoA` (derive support), `Hash`, `Hasher`, `DefaultHasher`, `ArenaArray`, `ArenaMap`, `CapacityError`, `ArenaArrayIter`, `ArenaArrayIterMut`, `ArenaMapIter`; `Hash` retains its `[MOD-5]` prelude export, while `Hasher`, `DefaultHasher`, and the Arena-backed names are public but not prelude exports |
...
# Appendix B — 0.9.6_Hardened_5 Consistency Checklist

The following checks are mandatory CI checks for the consolidated specification itself:

1. Exactly one current version declaration exists and names `0.9.6_Hardened_5`.
2. The immediate predecessor is frozen `0.9.6_Hardened_4`, which remains byte-for-byte unchanged.
3. `[MOD-6]` and `[MOD-6a]` name the supported `0.9.6`, `0.9.5`, `0.9`, and 0.8.x source-language contracts explicitly; `0.9` does not silently enable later semantics, and H5 changes H4 only through the owner-approved post-H4 simplicity architecture/process contract.
...
49. Specification, implementation, owner-decision, and conformance ledgers remain separate, and no
    implementation status is inferred merely from H5's requirements.
