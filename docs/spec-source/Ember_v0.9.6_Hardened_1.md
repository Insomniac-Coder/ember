...
**Immediate predecessor:** 0.9.5_Hardened_10
...
conformance result is asserted merely because the specification requires one.
...
## 0.9.6_Hardened_1 Audit Record

| ID | Classification | Resolution |
|---|---|---|
| AUD-096-01 | Source authority | Preserved the owner-supplied Revision 5 RFC byte-for-byte; H10 is the immutable construction source, while `ember-spec.md` remains the adopted 0.8.5 contract until H1 passes explicit adoption gates. |
...
**Amendment:** `HC-096-01` — owner-approved simplicity consolidation and `[ARN-10]`
clarification. ADR-030 records the decision. This is the first 0.9.6 cut, so the hardening counter
resets to 1. H10 remains unchanged and becomes the historical immediate predecessor.

### 0.9.6_Hardened_1 language-conformance condition

`0.9.6_Hardened_1` MUST NOT be declared conformant until all H10 conditions remain satisfied and
...
8. existing H10 safety regressions remain present and green; implementation-path consolidation is not
   used to delete an adversarial test.

### 0.9.6_Hardened_1 acceptance condition

H1 MUST NOT replace `docs/spec-source/ember-spec.md` unless it has one version identity and predecessor,
...
source observation is insufficient, and every difference from H10 is classified and traceable to the
owner-approved RFC or final `[ARN-10]` ruling.

## 0.9.6_Hardened_1 Evidence Status — No Implementation Claim

This H1 file is a frozen development target. The adopted repository specification remains
`docs/spec-source/ember-spec.md` (`0.8.5_Hardened_1`). H1's architecture, 0.9.5 multi-region semantics,
remaining H10 Arena initialization surface, and later phases are `SPECIFIED` unless the repository's
implementation matrix and executable evidence say otherwise. Passing the six gates against the adopted
source does not constitute H1 adoption or H1 conformance.
...
## 0.9.6_Hardened_1 consolidation record

The body below originated in `0.8.5_Hardened_1`, incorporates the accepted 0.9/0.9.5 changes in
place, and carries the owner-selected 0.9.6 reference-compiler consolidation. It is normative within
...
# Appendix B — 0.9.6_Hardened_1 Consistency Checklist

The following checks are mandatory CI checks for the consolidated specification itself:

1. Exactly one current version declaration exists and names `0.9.6_Hardened_1`.
2. The immediate predecessor is frozen `0.9.5_Hardened_10`, which remains byte-for-byte unchanged.
3. `[MOD-6]` and `[MOD-6a]` name the supported `0.9.6`, `0.9.5`, `0.9`, and 0.8.x source-language contracts explicitly; `0.9` does not silently enable later semantics, and 0.9.6 preserves the ordinary-source accepted/rejected sets and observable semantics of 0.9.5.
