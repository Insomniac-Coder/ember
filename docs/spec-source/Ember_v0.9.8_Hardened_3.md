...
**Version:** 0.9.8_Hardened_3
**Immediate predecessor:** 0.9.8_Hardened_2 (immutable; preserved as the diff base)
**Status:** Frozen development target. Implementation and conformance evidence is **not** claimed; normative repository adoption still requires the recorded adoption gates.

    Specification    frozen for this target. ODR-019 is closed here with one
                     deterministic N1 ranking algorithm. ODR-018 remains closed
                     by the shared cycle-analysis root for `inspect`/`explain`;
                     ODR-017's ownership API remains unchanged.
                     Both figures are measurements, not constants — 0.8.4_Hardened_1
                     records 826 against 833 for the same document — and XXIII.4 requires
                     an extractor to tell a definition from a citation without saying how
    Implementation   NOT CLAIMED
    Conformance      NOT CLAIMED — 144 rules from the 0.9 series sit in the [TST-4c]
                     baseline; 4 of them leave it with this revision

These three move independently. A complete specification is not a built compiler, and a
built compiler is not a passing conformance suite; this revision advances only the first.

> **Authority boundary.** This revision records the owner-selected 0.9.8 development target.
> `docs/spec-source/ember-spec.md` (`0.8.5_Hardened_1`) remains the repository's sole normative
> source until this target passes the adoption gates and is explicitly installed. The immutable
> received files are preserved under `docs/spec-source/as-received/`.

**What 0.9.8_Hardened_1 changed.** It closes ODR-017 with one strong-owner
surface for `Shared[T]` and one generalized weak-owner family: `Shared(value)`,
`get() -> ref T`, `get_mut(mut self) -> ref mut T`, and `Weak[O]` where `O` is
either a class handle or `Shared[T]`. `Weak[C]` is preserved unchanged, while
the companion of `Shared[T]` is `Weak[Shared[T]]`; no second weak-pointer
meaning or aliasing model is introduced. This is an owner-approved semantic
change and therefore begins language revision 0.9.8. The received ruling is
preserved byte-for-byte under `docs/spec-source/as-received/`.

**What 0.9.8_Hardened_2 changed.** It closes ODR-018 as a tooling hardening:
`ember inspect --cycle <path>` and `ember explain --cycle <path>
<Class[.field]>` resolve the same explicit analysis root and its ordinary
package/module/import closure. `[CLI-18]` forbids cwd scanning, stale-build
fallback, prior-build inference, and arbitrary package selection. This changes
neither Ember source-language acceptance nor ownership or lifetime semantics,
and introduces no ABI, runtime, metadata, or ownership-graph mechanism.

**What 0.9.8_Hardened_3 changes.** It closes ODR-019 by specifying
deterministic N1 suggestion ordering: Damerau–Levenshtein distance first,
same-file candidates before cross-file candidates only at equal distance,
same-file declaration proximity only within that file, and canonical qualified
identity for cross-file ties. This changes diagnostic suggestion ordering
only; candidate discovery and Ember source-program validity are unchanged.

**What 0.9.7_Hardened_3 changed.** It closes ODR-016 by giving the three rules that stated a
rejection without naming one a diagnostic code, and it withdraws two gap reports Hardened_2
made in error: `[MAN-7]` was defined all along, and `§XIX.4.7` resolves. Both were artifacts
of a rule detector that only recognised an identifier at the start of a line. No rule changes
meaning and no Ember source program changes validity. What Hardened_2 changed, below, stands.

**What Hardened_2 changed.** Nothing about what Ember means. Hardened_1 carried 144 rules added
across the 0.9 series and 23 amended in place with no change-log row for any of them, while itself
requiring CI to fail on exactly that (`[DIA-6a]`). Hardened_2 writes those change logs, completes
the rule-ID prefix map, the CLI listing, the error-code registry and the attribute table, records
what has no conformance test and why, and moves the 0.9 material behind the language it amends —
repairing, in the same pass, the marker note and the inverted *above*/*below* references that the
move itself would otherwise have left wrong.
Every edit is classified in **Change log — 0.9.7_Hardened_2** and none is an owner-approved
semantic change; a hardening that made one would be a language revision instead.

**Versioning.** Two numbers move independently. The **language version** moves when the set of
accepted programs changes. The **hardening number** moves when the document gains implementation
detail, invariants, tooling requirements or editorial repair and no rule changes meaning; it resets
to 1 with each language version. A hardening may never carry a semantic change: that is what forces
the language number instead. Implementation completion by itself moves neither. So 0.8.4 was cut as
`0.8.4_Hardened_1` rather than `0.8.3_Hardened_2` when S1 landed; 0.9.5 moved for inferred
multi-region views; 0.9.7 moved for the `@latebound` callable boundary; and
**`0.9.8_Hardened_1` supplied the public ownership and borrowing semantics for
this language version; 0.9.8_Hardened_2 completes ODR-018's explicit CLI/tooling
boundary, and this file is `0.9.8_Hardened_3` because ODR-019 completes N1's
diagnostic-ranking boundary.**

**Compatibility.** Additive. `0.9.8` ⊃ `0.9.7` ⊃ `0.9.6` ⊃ `0.9.5` ⊃ `0.9` ⊃ `0.8.5` ⊃ `0.8.4` ⊃ `0.8.3`.
No program valid under an earlier contract becomes invalid, and a source file may declare
`#! language "0.8.3"`, `"0.8.4"`, `"0.8.5"`, `"0.9"`, `"0.9.5"`, `"0.9.6"`, `"0.9.7"` or `"0.9.8"`. An older
selector never silently acquires later semantics (`[MOD-6]`, `[MOD-6a]`).

**Lineage.** `0.8.5 → 0.9 → 0.9.5 → 0.9.6 → 0.9.7 → 0.9.8`, each authored from the one before it. 0.8.5 was
authored from 0.8.4, which was authored from 0.8.3; the ancestry behind 0.8.3 runs 0.8.2c → 0.8.2b →
0.8.1 → 0.8 → 0.7.2 → 0.7.1 → 0.6.3, taking the *design* — not the text — of the 0.7 draft for hot
reload, the compile-time budget and the C++ boundary. The 0.7 draft was itself authored from 0.3 and
silently reverted 239 rules settled in 0.4 through 0.6.2; **every one of those is retained here**, and
so is every rule of 0.8.3 and 0.8.5. **A revision of this document MUST be authored from the
...
## 0.9.8_Hardened_3 authority and evidence status

This file is the frozen development target selected by the owner rulings that
close ODR-018 and ODR-019. It carries the inherited `Shared[T]`/`Weak[O]` work
from 0.9.8_Hardened_1, the canonical cycle-analysis-root tooling contract from
0.9.8_Hardened_2, and the N1 ranking hardening after its recorded source hash
is accepted through
`docs/spec-source/development-target.json`; the adopted source remains
`ember-spec.md` at `0.8.5_Hardened_1` until the repository's adoption gates
are met.  The ruling is preserved byte-for-byte in
`as-received/ODR-018_Cycle_explanation_analysis_root.md` and
`as-received/ODR-019_N1_cross_file_suggestion_ranking.md`; ADR-038 records
ODR-018's application here.

The rules added for this language revision are specified, not yet evidence of
an implementation: adoption requires the `[TST-26]` matrix, implementation
tests for its accepted and rejected programs, and the ordinary compiler and
documentation gates.  No claim in this status section weakens or replaces the
inherited rules in the consolidated body.

## Where things are

| | |
|---|---|
| **Part 0** | foundational decisions, rejected alternatives, and **the change logs** — read it first |
| **Parts I–XVII** | the language |
| **Part XVIII** | hot reload |
| **Part XIX** | compiler internals (IRs, passes, ABI, mangling, backends) |
| **Part XX** | toolchain (CLI, manifest, build graph, tests, diagnostics, error-code registry) |
| **Part XXI** | implementation plan |
| **Part XXII** | RageV integration |
| **Part XXIII** | owner decisions, glossary, rule-ID registry, rule categories |
| **Appendix A** | syntax quick reference |
| **Part XXIV** | **0.9-series normative addenda** — multi-region views, the implementation contract, the ABI/runtime/reload/determinism addenda, the 1.0 readiness gate |
| **Appendix B** | the preserved 0.9.7_Hardened_1 consistency checklist |
| **Appendix H** | 0.9-series provenance: completion records, audit records, evidence statements, the hardening cycle log |

Part XXIV follows Appendix A rather than preceding it because the `CONSOLIDATED_NORMATIVE_BODY`
markers delimit the text inherited from the 0.8.5 lineage — Parts 0–XXIII and Appendix A — and
Part XXIV was authored by the 0.9 series. `[TST-7]` extracts its `ember` blocks from Parts I–XVII
and Appendix A, and that scope is unchanged.

Parts I–XXIV are one normative contract (`[IMP-8]`). Appendix H is history: it records how the
contract got here and binds nobody. Part XXIV is placed after the body it amends because the
language comes first in a language specification; nothing in it depends on its position, and
position-based supersession is forbidden by `[IMP-8]`, so relocating a Part changes nothing.
...
8. **Part XXIV** carries the 0.9-series normative addenda: inferred multi-region views, the implementation contract, the ABI, runtime, hot-reload and determinism addenda, and the 1.0 readiness gate. It is normative with Parts I–XXIII (`[IMP-8]`) and sits after them because a language specification states the language first.
9. **Appendix H** is the 0.9-series provenance record — completion records, audit records, evidence statements and the hardening cycle log. It is history and binds nobody.
...
## Change log — 0.9.8_Hardened_3

**0.9.8_Hardened_3 closes ODR-019.** Option 1 is adopted as diagnostic/tooling
hardening only. Declaration proximity applies only within the same source
file; for equal spelling distance, same-file candidates rank before cross-file
candidates; cross-file ties use canonical qualified-name ordering. No
project-wide file/module proximity model is introduced. Candidate discovery
semantics are unchanged, no source program changes validity, and no runtime,
ABI, ownership, lifetime, overload, import, or type-system semantics change.

| # | Change | Class | Where |
|---|---|---|---|
| H3.1 | `[DIA-12]` defines one N1 ranking algorithm: spelling distance, locality class on distance ties, unchanged same-file proximity, canonical qualified identity for cross-file ties, and top-N selection after complete ordering. | DIAGNOSTIC / TOOLING HARDENING | XX.6.2, N1 ranking algorithm |
| H3.2 | Cross-file identity is independent of unrelated offsets, import/alias positions, filesystem/source discovery, hash iteration, import order, operating-system directory order, and build/cache order; visible suggestion spelling remains unchanged. | DETERMINISTIC DIAGNOSTIC ORDERING | XX.6.2, N1 ranking algorithm |
| H3.3 | Unit, conformance, and UI regression coverage verifies suggestion order, aliases/imports, inherited fields, prelude candidates, top-three truncation, and repeated runs under reversed file/import discovery order. | IMPLEMENTATION / CONFORMANCE EVIDENCE | `compiler/ember_typeck/src/name_suggestions.rs`, `compiler/ember_driver/tests/ui.rs`, `tests/conformance/DIA-12/` |

**This change log is exhaustive for the 0.9.8_Hardened_3 delta.** The H1
source-language revision remains the complete 0.9.8 language delta; H2 and H3
are tooling/diagnostic hardenings.

---

## Change log — 0.9.8_Hardened_2

**0.9.8_Hardened_2 closes ODR-018.** It supplies one explicit analysis-root
contract for the two existing cycle tools. It changes no Ember source-language
accepted/rejected program set, ownership or lifetime mechanism, ABI, runtime
object semantics, or static ownership-cycle analysis.

| # | Change | Class | Where |
|---|---|---|---|
| H2.1 | `[CLI-18]` defines a package-directory or standalone-file analysis root, and forbids cwd scanning, stale-build fallback, prior-build inference, or arbitrary package selection. | TOOLING HARDENING | XX.1, `[CLI-18]` |
| H2.2 | `[CLI-17]` and `[WK-9]` use the canonical root; `explain --cycle` gains its required `<path>` argument and the target-resolution/ambiguity contract. | TOOLING HARDENING | XX.1, XXIV H14 |
| H2.3 | Help, conformance mapping, and decision records identify the one resolver and verify package, standalone, target, invalid-root, and non-fallback behavior. | IMPLEMENTATION EVIDENCE BOUNDARY | XX.1, XX.5 |

**This change log is exhaustive for the 0.9.8_Hardened_2 delta.** The H1
source-language revision remains the complete 0.9.8 language delta.

---

## Change log — 0.9.8_Hardened_1

**0.9.8_Hardened_1 closes ODR-017.** It selects one public `Shared[T]`
strong-owner API and generalizes `Weak` over the existing counted owners. This
changes the set of well-defined source programs, so it is an owner-approved
language revision rather than a hardening of 0.9.7.

| # | Change | Class | Where |
|---|---|---|---|
| H1.1 | `Shared(value)` constructs one counted allocation. `get()` returns `ref T`; `get_mut(mut self)` returns `ref mut T` and participates in the existing dynamic-exclusivity machinery without requiring unique ownership. | OWNER-APPROVED SEMANTIC CHANGE | IX.1, `[HEAP-3]`–`[HEAP-6]` |
| H1.2 | `Weak[O]` denotes the weak form of a counted owner. The supported owner forms are a class handle `C` and `Shared[T]`; the latter's companion is `Weak[Shared[T]]`. Construction, empty, copy/drop, upgrade, no-resurrection, and non-interoperation rules reuse the existing weak-count model. | OWNER-APPROVED SEMANTIC CHANGE | VIII.5, `[HEAP-7]`, `[WK-11]`–`[WK-14]` |
| H1.3 | The storage table distinguishes static borrowing from dynamic exclusivity for `Shared[T]`, the version selector admits `0.9.8`, and `[TST-26]` records the conformance matrix. | OWNER-APPROVED SEMANTIC CHANGE | V, IX, XX |

**This change log is exhaustive for the 0.9.8 delta.** Every later implementation
or conformance record is evidence, not a further source-semantic change.

---

## Change log — 0.9.7_Hardened_3

**Hardened_3 closes ODR-016 and withdraws two gap reports Hardened_2 made in error.
No rule changes meaning and no Ember source program changes validity relative to
Hardened_2.**

Giving a rule a diagnostic code does not change which programs Ember accepts — the rule
already rejected them — so this remains a hardening. What it changes is conformance
obligation: four rules lose `[TST-4b]`'s automatic waiver and owe a reject case each.

**Every edit declares its class**, and a hardening admits only four of the five
(`0.8.4_Hardened_1`). Every edit below is **EDITORIAL REPAIR** except H3.3, H3.4 and H3.5,
which are **IMPLEMENTATION INVARIANT**: they complete the diagnostic identity of behaviour
the language already specifies. None is an owner-approved semantic change.

| # | Change | Class | Where |
|---|---|---|---|
| H3.1 | **Two gap reports are withdrawn.** Hardened_2 recorded `MAN-7` as referenced-but-undefined and `§XIX.4.7` as a dangling section reference. Both were wrong. The manifest rule is defined in Part XX, mid-bullet after `[MAN-6]`, with exactly the values, defaults, `E9031`, `E9033` and package-agreement clause it was reported to be missing; and `## XIX.4 MIR` carries fourteen subsections of which `### 4.7` is *Borrow checking (NLL)*, the reference's referent. Both reports came from one defect in the sweeping tool — it recognised a rule identifier only at the start of a line, and resolved no subsection paths. The same miss had already surfaced once, on `[HR-10a]`, and was recorded without being generalised. | EDITORIAL REPAIR | Part 0, front matter |
| H3.2 | **The manifest reload rule `[MAN-7]` gets its own bullet**, out of `[MAN-6]`'s. Its text is unchanged and moves not one word; it simply stops sharing a bullet with `[MAN-6]`, where no line-start extractor could see it. This is the defect underneath H3.1 — not a missing rule, an unfindable one. | EDITORIAL REPAIR | XX manifest |
| H3.3 | **ODR-016 is closed.** `[LT-23]` and `[LT-39]` name `E3065`, shape **B14** — the code `[LT-22]` already carries, because all three describe one observable class: multi-region provenance that cannot be established soundly. `[FN-6a]` and `[LT-11a]` name `E2228`, shape **B15**, a new code for callable parameter-mode mismatch. One new code, not three. | IMPLEMENTATION INVARIANT | VII, V |
| H3.4 | **Shape B15** is added to the XX.6.1 catalogue with its meaning and its required fixes, and its `help` MUST print the expected and supplied signatures rather than a category, as `[DIA-10]` requires of the shapes people meet most. | IMPLEMENTATION INVARIANT | XX.6.1 |
| H3.5 | **Registry.** `E2228` is registered with kind, subsystem, title, shape and enforcing rules, plus its error page and fixture. `E3065`'s entry gains `[LT-23]` and `[LT-39]` as enforcing rules and no new code. | IMPLEMENTATION INVARIANT | XX.6 |
| H3.6 | **The completeness pass gains a section-reference check** (`[DIA-6a]`). It must resolve compound subsection notation — `§XIX.4.7`, `§XIX.4 subsection 4.7` and `XIX.4 → 4.7` all denote `### 4.7` inside `## XIX.4` — and fail CI only on a reference with no referent. A checker that cannot follow a subsection path reports a correct reference as dangling, which is half of what H3.1 withdraws. | EDITORIAL REPAIR | XX.6 |
| H3.7 | **Four rules leave the `[TST-4c]` baseline.** `[LT-23]`, `[LT-39]`, `[FN-6a]` and `[LT-11a]` now name codes, so `[TST-4b]` withdraws their automatic waiver in the same pass that adds the code, and each owes a reject case per code. The baseline shrinks by four, which is the direction `[GATE-1]` requires. | EDITORIAL REPAIR | XX.5 |

**Four things in the reviewed resolutions were not taken, each for a stated reason.**

The proposed new manifest rule is **not added**: the rule it would restore already exists, and
adding it would put two definitions of one identifier in the document, which XXIII.4 makes a hard
CI failure and which the proposal's own acceptance criterion — *"exactly one active definition"* —
forbids. The reconstruction was accurate almost to the word, which is why H3.2 only makes the
existing rule findable.

The proposed rewrite of the section reference to *"§XIX.4, subsection 4.7"* is **not made**: the
reference resolves as written, and rewriting a correct citation into a longer one to satisfy a
broken checker fixes the wrong end. H3.6 fixes the checker. The proposal's own conclusion — that no
new section may be created — is right and is why none is.

The proposed code number `E3066` is **not used**, and is **registered as reserved** rather than
left unmentioned — naming a code in this document obliges `[DIA-6a]` to find a registry entry for
it, and the document already reserves `E4071`, `E4073` and `E9032` this way so that a retired or
declined number is never silently reused. The resolution assigns `E3066` subsystem `typeck` while
numbering it into `E3000–E3499`, which the range table gives to ownership, moves, borrows, regions,
exclusivity and drops; `typeck` is `E2000–E2499`. The resolution is right about the subsystem and
wrong about the band, so the code is `E2228`. This also keeps it out of `[DIA-7a]`'s
shape table, which is scoped to the `E3xxx` band, and under `[DIA-12]`, which covers `E1xxx` and
`E2xxx`.

The proposed worked example is **not included**. Its body is an ellipsis and its call sits at file
scope, which `[GRM-2]` forbids — *file admits only items* — so `[TST-7]` would reject the block on
sight. The B15 row states the shape's meaning and fixes in the form the other fourteen rows use,
and needs no example to do it.

**One thing the resolutions did not cover is covered here.** `[LT-11a]` already required a
callback parameter-mode-mismatch diagnostic and named no code. Closing `[FN-6a]` without it would
have left the identical gap one rule away, so both name `E2228`.

**Corrections applied before freezing (2026-09-15).** Three stale numbers in the front
matter, fixed in response to review and changing no rule, so the hardening number does not
move: the versioning paragraph said *"this file is `0.9.7_Hardened_2`"*, carried over verbatim
from Hardened_2 and not updated when this revision was cut; the status block said **872**
rules where this revision has **873**, the manifest reload rule having become visible to the
extractor under H3.2; and it said *"~14"* cited-but-undefined ids where the count is **13**,
approximate where it did not need to be and wrong as well. The block now states both figures
exactly and says what they are measurements of, because a count that moves when the extractor
improves is not a constant and this document has twice been misled by treating one as though
it were.
...
## Change log — 0.9.7_Hardened_2

**Hardened_2 is a completeness repair on Hardened_1. No rule changes meaning, no
rule is added or removed, and the accepted program set is identical to
Hardened_1's.**

Hardened_1 carried 144 rules added across the 0.9 series and 23 amended in place
with **no change-log row for any of them**, while itself carrying `[DIA-6a]`'s
requirement that `tools/rule_index.py` *"MUST additionally fail CI when a rule
defined in the active index has no row in the current revision's change log."*
The document failed its own gate 167 times. **Six further defects of the same class**
were found in the same pass, and Hardened_2 repairs all seven — H2.1 through H2.7 below.
It then restructures the file so that the language is at the front of it (H2.10), and
repairs the two things that restructuring broke in passing: a marker note that would have
become false (H2.8) and every *above* and *below* the move inverted (H2.9). Ten rows, of
which seven answer Hardened_1 and three answer Hardened_2 itself.
...
Every edit below is **EDITORIAL REPAIR** except H2.6, which is an **IMPLEMENTATION
INVARIANT**. H2.6 affects **conformance-tool acceptance only** — what
`tools/rule_index.py` admits into the `[TST-4c]` baseline. It does not change Ember
source-program validity, any language semantics, or any compile-time or runtime check
the language specifies. No edit below answers *what Ember means*: **no Ember source
program changes validity relative to Hardened_1**, and each edit records, indexes or
relocates a decision already taken.

| # | Change | Class | Where |
|---|---|---|---|
| H2.1 | **The 0.9 series gains change logs.** Four sections beside this one — 0.9 (which carries the Hardened_1..14 cycle, H12's and H14's owner rulings included), 0.9.5, 0.9.6 and 0.9.7_Hardened_1 — each carrying a row for every rule that revision added or amended, reconstructed from the audit records, the owner rulings they cite, and a mechanical diff of the rule set against `0.8.5_Hardened_1`. This is what `[DIA-6a]` requires and what every revision from 0.3 to 0.8.5 provided. | EDITORIAL REPAIR | Part 0 |
| H2.2 | **The rule-ID prefix map gains the 15 families 0.9 added** — `IMP`, `COMP`, `GEN-COH`, `CLO-ABI`, `COR-ABI`, `LAY`, `FFI-IMPL`, `FFI-CB`, `HR-IMPL`, `DET-IMPL`, `STD-IMPL`, `VERIFY`, `MIR-REG`, `HASH`, `OQ8` — 44 rules that were defined and then not declared anywhere `tools/rule_index.py` reads. The Non-normative heading index of H1 named ten of them as a navigation aid and said of itself that it is *"not a registry"*; the registry is XXIII.4, and it is now complete. That index is also extended, with three lines pointing at `[IMP-10]`, `[VERIFY-3]`, `[MIR-REG-1]`, `[HASH-1]`..`[HASH-4]` and the `OQ8` family, so that the navigation aid and the registry no longer disagree about what exists. XXIII.4 additionally now requires the extractor to ignore rule ids inside Appendix H, so that a provenance record quoting a rule at the start of a line cannot register a second definition. | EDITORIAL REPAIR | XXIII.4 |
| H2.3 | **`ember inspect --cycle` and `ember explain --cycle` enter the CLI listing.** `[CLI-17]` and `[WK-9]` named two commands that appeared in no `ember --help` listing and in no construct list — the exact defect `[CLI-15]` exists to catch, and the one 0.7.1 committed with `--reload` and `--timings`. Their overlap is also now stated: `explain --cycle` answers for one field, `inspect --cycle` reports the whole graph, and `[WK-4]`'s leak reporter is the run-time view of the same data. | EDITORIAL REPAIR | XX.1, XXIII.3 |
| H2.4 | **`E3105` and `E3065` enter the error-code registry.** `E3105` was added by 0.8.5 for `UnsafeCell` and `E3065` by 0.9.5 for multi-region provenance; both were named in rules, `[DIA-7a]`'s shape table and `[DIA-19]`, and neither had a registry paragraph. `[DIA-6a]` makes every code named anywhere a registry entry, in both directions. | EDITORIAL REPAIR | XX.6 |
| H2.5 | **`@latebound` enters the attribute table and the glossary**, marked as what `[FN-6b]` already says it is — a callable-**type** modifier, not an item attribute, admissible in no statement or item position and therefore outside `[ATT-2]`'s enumeration. It was in Part III's `fn_type` production and nowhere a reader looks an attribute up. | EDITORIAL REPAIR | III §7, XXIII.3 |
| H2.6 | **`[TST-4c]` states the permitted reason its own predecessor already relied on.** *(The one edit in H2 that widens anything — read it first.)* 0.8.5's change log placed four rules in `tools/rule_index_baseline.json` under *"`[TST-4c]`'s one permitted reason: a new specification revision opened the gap"* — a reason stated in neither `[TST-4c]` nor `[TST-7]`, whose four permitted reasons are a `std` signature sketch, foreign-language source, a deliberate error example, and overlay-language source. The rule is brought in line with the practice it governs, in the manner of `0.8.4_Hardened_2`'s `[EFF-18]` repair: the enumeration was stale, not the practice. The baseline's direction of travel is unchanged — it shrinks, `[GATE-1]` requires it empty for 1.0, and a rule may enter it only in the revision that creates it. | IMPLEMENTATION INVARIANT | XX.5 |
| H2.7 | **What has no tests yet, and why** — the 144 rules of the 0.9 series are recorded against `[TST-4c]`'s reason above, rather than left to fail CI silently or pass it falsely. H1's own Evidence Status sections already said these rules are `SPECIFIED`, not implemented; this is that statement made machine-readable. | EDITORIAL REPAIR | Part 0, XX.5 |
| H2.8 | **The consolidated-body END marker's note is corrected.** It read *"material after this marker is specification tooling guidance"*, which H2.9's reordering would have made false, since Part XXIV follows it and is normative. It now says what the markers actually distinguish — text inherited from the 0.8.5 lineage from text the 0.9 series authored — and names Part XXIV, Appendix B and Appendix H by what each is. `[TST-7]`'s extraction scope, Parts I–XVII and Appendix A, sits inside the markers and is unchanged. | EDITORIAL REPAIR | end of Appendix A |
| H2.9 | **Directional references are corrected or annotated.** Moving 0.9 material from the front to the back inverted every *above* and *below* in it. Two occur in normative Part XXIV text — *"appears in Part IV §4 below"* and *"supplements the existing Parts I–XXIII below"* — and are corrected in place, each marked with what H1 said. Four occur in Appendix H, whose text is left exactly as written; the appendix header instead tells the reader to read its directions against H1's layout, because editing a historical record to suit a later file is how it stops being a record. | EDITORIAL REPAIR | XXIV, Appendix H |
| H2.10 | **The document is reordered.** H1 opened with 2,528 lines of banners, completion records, addenda and audit records before *How to use this document*, so 29% of the file preceded the language. The normative 0.9 material is now **Part XXIV**, after the consolidated body it amends; the historical record is **Appendix H**, at the end. **Nothing is deleted and no sentence is rewritten in the move** — every line of H1 survives in one of the two. Two mechanical changes accompany it: every moved heading drops one level so that it nests under its new Part or Appendix, and one new heading, *Reference-compiler model and boundaries*, parents three normative sections whose former parent — the 0.9.6_Hardened_1 record — is provenance and went to Appendix H. Beyond those, `[IMP-8]`'s one-consolidated-source rule is unaffected because it never depended on position. | EDITORIAL REPAIR | whole file |

**What has no tests yet, and why.** **All 144 rules the 0.9 series added have no
conformance test**, and none of the 23 it amended has a reject case for behaviour
the amendment introduced. This is not a new condition — H1's five Evidence Status
sections already stated that the 0.9, 0.9.5, 0.9.6 and 0.9.7 contracts are
`SPECIFIED`, not implemented and not conformant — but H1 recorded it in prose that
`tools/rule_index.py` does not read, so `[TST-4]` and `[TST-4a]` would have failed
CI on 144 rules with no explanation available to the tool. They are entered in
`tools/rule_index_baseline.json` under `[TST-4c]`'s permitted reason as stated by
H2.6, each tagged with the revision that created it: 7 under `0.9_Hardened_12`,
31 under `0.9_Hardened_14`, 37 under `0.9.5_Hardened_1`, 34 under the 0.9
implementation-contract hardenings, and 35 under the 0.9.6 and 0.9.7 completions.

This is the largest baseline entry in the document's history and it is meant to be
uncomfortable. `[GATE-1]` requires the baseline **empty** for 1.0, `[TST-4c]`
forbids a later commit from adding to it, and the counts above are therefore a
work list with a fixed ceiling, not an exemption. `E3065` is registered in
`ember_diag` ahead of its emitter so that `[DIA-6a]` holds, as `E3105` was for
0.8.5.

**What H2 deliberately does not do.** It does not assign diagnostic codes to the
0.9 rules that state a program-level rejection without naming one — among them the
rules `[LT-23]`, `[LT-39]` and `[FN-6a]`. Choosing which rejections are diagnosable and
under which code is a determination about what Ember reports, not about how to
implement what it already says, and `[TST-4b]` derives reject-case obligations
mechanically from that choice. Assigning them here would be this hardening
answering a question that belongs to the owner. They are recorded as **ODR-016**
and the affected rules keep `[TST-4b]`'s automatic waiver until it is answered —
a waiver that is now visible in the baseline rather than invisible in its absence.

It does not define `MAN-7` — written unbracketed here so that this note adds no further citation
of it — which is referenced seven times, by `[HR-25]`, `[PRF-2]`, the manifest example and the
0.6.3 and 0.7.1 registry paragraphs, and defined by no rule. It is one of the nine
ids `0.8.4_Hardened_1` recorded as genuinely undefined under ERR-042, and writing the missing rule
would be this hardening deciding what the `reload` manifest key means. It stays ERR-042's. The same
applies to the one dangling *section* reference the sweep found, `§XIX.4.7` in Part XIX's pass list:
XIX.4 has no numbered subsections, so the reference has no referent to repair, and inventing one
would be writing structure rather than recording it. Both are inherited from `0.9.7_Hardened_1` and
neither was introduced by the reordering.

It also does not resolve the two `--cycle` commands into one. Both are specified,
both are reachable, and collapsing them would remove a surface an implementation
may already have built.

**Corrections applied before freezing (2026-09-15).** Four changes made in response to
review — three wording repairs and one addition — none of which touches a rule: the class
sentence above no longer says H2.6
"changes what any check accepts" — in a document where *check* means a bounds,
exclusivity or runtime check, that phrasing collided with the language's own vocabulary,
so it now names the conformance tool it actually affects; the phrase *"the accepted-program
set is byte-identical"* is replaced by *"no Ember source program changes validity"*,
because a program set is semantic and has no bytes (the term survives where it is correct,
in Part XXIV, of rule text and code blocks that genuinely are byte-identical); and the
front matter gains the three-state status block. Two colloquial uses of *safe* were also
reworded, in the front matter and the Part XXIV header: **Safe Ember** is a defined term
(`[PHIL-10]`, `[PHIL-11]`) and "moving it is safe" borrowed it for something unrelated —
the same class of looseness as H2.6's "any check". Nothing else moved.
...
## Change log — 0.9.7_Hardened_1

**0.9.7 is one owner-approved language change: the `@latebound` callable-type
boundary, closing ODR-015.** It changes which callback-return and escape programs
are accepted, which is why it is a language revision and not a hardening.

*(Reconstructed by `0.9.7_Hardened_2` from the 0.9.7 Audit Record, the ODR-015
ruling it cites, and a mechanical diff of the rule set. H1 recorded this work in
audit rows and banner blocks and in no change log; the rows below are what
the completeness rule `[DIA-6a]` requires.)*

| # | Change | Where |
|---|---|---|
| 1 | **`@latebound` is admitted on a callable type** (`[FN-6b]`). `@latebound fn(Span[T]) -> R` supplies fresh invocation-local regions for its borrowed and view parameters. It is permitted only immediately before a callable type: it is not an item attribute, names no region, and appears in no runtime value, ABI byte, reload schema or source-visible lifetime identity. | V, III `fn_type` |
| 2 | **The boundary's region rules** (`[LT-7]` amended, `[LT-8]`, `[LT-8a]`, `[LT-10]`). Each invocation binds fresh regions; the invocation's result MUST be free of them, so a callback cannot return or publish a view of what it was lent. Ordinary region and escape diagnostics remain authoritative — the boundary introduces no bespoke diagnostic and no compiler intrinsic. | XXIV §H14.3, VII |
| 3 | **The fact is part of compile-time callable identity** (`[FN-6a]`, `[FN-6]` amended). `@latebound` participates in canonical type identity, generic substitution, expected-callable checking, interface/EMIF compatibility and incremental invalidation, and `Callable[Args, R]` preserves it rather than erasing it. | V |
| 4 | **`std.borrow.with_views*` stays an ordinary library API.** No compiler recognition of those names is permitted; the helpers are ordinary functions whose callback parameters are spelled `@latebound`. This is what keeps the feature reusable rather than a special case for one module. | XXIV §H14.3 |
| 5 | **Version selection** (`[MOD-6]` amended, `[MOD-6a]`). `0.9.7` selects the `@latebound` contract; `0.9`, `0.9.5` and `0.9.6` do not silently acquire it. | V |
| 6 | **Conformance** (`[TST-20]`, `[TST-21]` amended): adversarial escape, storage, owned-capture, FFI-publication, freshness and nested-boundary cases. | XXIV §MRV-8 |
| 7 | No diagnostic code is added. `@latebound` violations are reported by the existing region and escape diagnostics, which is the design. | — |

**Evidence.** `@latebound` is **SPECIFIED**, not implemented or conformant. H1's
Evidence Status section states the condition and it is unchanged.
...
**0.9.6 preserves 0.9.5's accepted-program set exactly and completes four
standard-library and reference-compiler contracts the owner had left open.** Six
hardenings, of which four were owner-approved API completions.

*(Reconstructed by `0.9.7_Hardened_2` from the Hardening Cycle Log, the H1–H6
audit records and the ODR rulings they cite.)*

| # | Change | Where |
|---|---|---|
| 1 | **The reference compiler is consolidated onto reusable facts** (H1). `[IMP-1]`..`[IMP-9]`, `[COMP-1]`, `[GEN-COH-1]` and `[GEN-COH-2]` — coined by 0.9 — are **revised in place** rather than paralleled by a second rule family, and `[MIR-REG-1]` is added for the MIR region boundary. The facts are: `TypeIdentity`, `BorrowCapability`, `OwnershipGraph`, `AccessContract`, `InitializationState`, `LayoutDescriptor`, `EffectSet`. Existing contracts are revised in place rather than paralleled by a second rule family. `[ARN-10]` is clarified to v1 abort-only panic — no unwinder, no observable post-panic Arena state. | XXIV |
| 2 | **Fixed-capacity Arena-backed collections** (H2, ODR-011: `[ARN-3]`, `[ARN-5]` amended, `[ARN-5a]`, `[ARN-5b]`, `[ARN-5c]`, `[ARN-5d]`, `[ARN-5e]`, `[ARN-5f]`, `[ARN-5g]`, `[ARN-11]`, `[ARN-12]`, `[ARN-13]`). `ArenaArray[T]`, `ArenaMap[K,V]` and their named iterator view types, with exact provenance, drop and failure behaviour and `CapacityError.Full`. `[ARN-2]` is amended to match. | IX |
| 3 | **The hashing protocol** (H3, ODR-012: `[HASH-1]`..`[HASH-4]`). `Hash`/`Hasher`, `DefaultHasher`, Eq/hash coherence, `Map` key bounds and the read-only safe key-access boundary. | XV |
| 4 | **`Hash.hash` is statically generic over `H: Hasher`** (H4, ODR-013). The signature becomes `fn hash[H: Hasher](self, mut h: H)`, `DefaultHasher implements Hasher` is confirmed, and implicit dynamic dispatch at the hashing boundary is excluded. | XV |
| 5 | **Simplicity consolidation** (H5, Revision 5 RFC): derived-only compiler aggregates, conservative unknown handling, reuse and inference gates, and the semantic-preservation contract. `[DOC-2]`, `[DOC-3]`, `[VER-1]`, `[BEN-7]`, `[DIA-2]`, `[TST-11]` are amended to match. | XXIV, XX, XXI |
| 6 | **Span iterator, chunk and raw-pointer boundary** (H6, ODR-014: `[SPN-4]`..`[SPN-10]`). Named iterator and chunk view types, exact shared and mutable reborrow contracts, all-profile zero-chunk behaviour, and safe `.as_ptr()`/`.as_mut_ptr()` whose *use* remains governed by `[UNS-1]`. `[CLO-3]`, `[CLO-6]`, `[UNS-5]`, `[BLD-2]` are amended to match. | VII, IX |
| 7 | **`[MOD-6a]`** admits `X.Y.Z` language versions and pins what each of `0.9`, `0.9.5`, `0.9.6` selects. | V |
| 8 | `[TST-24]`, `[TST-25]` carry the conformance obligations for items 2–6. No diagnostic code is added: every rejection above is reported by an existing code. | XX.5 |
...
**0.9.5 is one language change — inferred multi-region `@view` structs — plus ten
hardenings that completed the callback-composition and Arena-initialisation
contracts around it.**

*(Reconstructed by `0.9.7_Hardened_2` from the 0.9.5_Hardened_1–10 audit records
and the owner rulings they cite.)*

| # | Change | Where |
|---|---|---|
| 1 | **Inferred multi-region views** (`[LT-14]`..`[LT-43]`, `[TYP-15]`, `[TYP-15a]`, `[LT-2]` amended). A `@view struct` may carry any finite number of independently inferred borrow regions in a compiler-internal **region vector**, one slot per borrowed field. `[LT-16]` preserves each field's provenance instead of intersecting; `[LT-17]` makes validity **conjunctive** — a view is usable only while every slot a reachable field needs is valid — and that is the safety core of the feature. `[LT-15]` forbids requiring any named-lifetime syntax; `[LT-30]` keeps region slots out of the runtime representation, so two instantiations differing only in region have identical size, alignment, offsets, padding and calling convention. `[LT-31a]` keeps region identity out of *nominal* type identity — two values of one `@view struct` with different region vectors share a type, an ABI, a symbol and a layout descriptor, so the feature causes no monomorphisation or vtable bloat — and `[LT-34]` preserves every legacy one-region program. | XXIV §MRV, IV, VII |
| 2 | **`E3065`** is the multi-region provenance diagnostic, shape **B14** (`[DIA-7a]` amended, `[DIA-19]`). *H2 note: it is registered in XX.6 as of `0.9.7_Hardened_2`; H1 named it in three places and registered it in none.* | XX.6 |
| 3 | **Callback composition** (`[LT-8]`, `[LT-8a]`, `[LT-9]`, `[LT-10]`, `[LT-11]`, `[LT-12]`, `[LT-13]`, H5 source recovery). The owner-supplied rules restoring independent-lifetime view composition, with their 0.9.5 supersession boundary preserved. | XXIV §H14.3 |
| 4 | **Explicit all-mutable helpers** (H6, ODR-005): `with_views2_mut`, `with_views3_mut`, `with_views4_mut`, spelled with the canonical `MutSpan[T]`. Mixed-mutable overloads remain outside the language surface. | XV |
| 5 | **Callable parameter modes** (H7/H8, `[FN-6]` amended, `[FN-6a]`): borrowed/default, `mut` and `owned` in `fn_type`; mutable helper inputs are `mut` reborrows; `Callable[Args, R]` preserves the mode vector as compiler-known metadata. `[LT-11a]` rejects a `with_views*_mut` callback whose callable type omits the required `mut` mode, naming the mode mismatch rather than reading `MutSpan[T]` as an implicit ownership mode. No helper consumes its input view, and modes are compile-time only. | III, V |
| 6 | **Arena return provenance** (H9, `[LT-4a]`, `[LT-4b]`, `[TST-22]`, `[LT-4]` amended). A user-defined wrapper may name an `Arena` parameter in `@borrows` only when its returned view derives from storage that arena owns. This makes `Arena` neither a view type nor a licence for arbitrary non-view parameters in `@borrows`. | VII |
| 7 | **Arena initialisation** (H10, ODR-009, `[ARN-8]`, `[ARN-8a]`, `[ARN-9]`..`[ARN-13]`, `[TST-23]`): deterministic `Zeroable`/`Default` selection, the `!needs_drop` boundary, transactional construction, `MaybeUninit[T]` layout and destruction, canonical safe-write and unsafe `assume_init` APIs, uninitialised Arena allocation, and Phase 2 availability. | IX |
| 8 | **Supporting rules**: `[BUD-8]` (region inference cost ceiling), `[IMP-10]`, `[VERIFY-3]` (MIR verification of region vectors), `[TST-17]`..`[TST-19]` (conformance for one-, two-, four- and eight-field views). `[LT-1a]`, `[LT-1b]`, `[LT-7]`, `[PHIL-9]` are amended to match. | XXIV, XX.5 |
...
**0.9 is an implementation-contract release. It adds no source-language
mechanism**: every rule it coins says how to build, verify or report what 0.8.5
already means. Fourteen hardenings, of which H12 and H14 also carried owner
rulings.

*(Reconstructed by `0.9.7_Hardened_2` from the Hardening Cycle Log and the H12
and H14 records.)*

| # | Change | Where |
|---|---|---|
| 1 | **The implementation contract** (`[IMP-1]`..`[IMP-9]`): specification-to-implementation traceability, **no semantic invention**, canonical artifacts, implementation completeness, canonical type identity, MIR source provenance, pass and canonical-fact contracts, one consolidated normative source, and one defining module per standard-library symbol. `[IMP-2]` is the rule that makes a `SPEC_GAP` blocking rather than fillable by analogy — the same principle as the guiding sentence, applied to the compiler. | XXIV |
| 2 | **ABI and layout** (`[LAY-1]`, `[FFI-IMPL-1]`..`[FFI-IMPL-3]`, `[FFI-CB-1]`, `[CLO-ABI-1]`, `[CLO-ABI-2]`, `[COR-ABI-1]`, `[COR-ABI-2]`): canonical layout and ABI descriptors, the C++ ABI fingerprint, generated thunk tests, callback generation identity, and closure and coroutine frame layout. | XXIV |
| 3 | **Runtime** (`[RT-6]`..`[RT-9]`): canonical runtime ABI name, reference-count overflow, the atomic ordering matrix, handle generation exhaustion. | XXIV |
| 4 | **Hot reload implementation** (`[HR-IMPL-1]`..`[HR-IMPL-3]`): the reload state machine, epoch publication and reclamation, and transaction failure — the implementation side of Part XVIII's `[HR-34]`..`[HR-43]`. | XXIV |
| 5 | **Determinism** (`[DET-IMPL-1]`, `[DET-IMPL-2]`): the floating-point environment and the differential determinism oracle. | XXIV |
| 6 | **Standard library and self-verification** (`[STD-IMPL-1]`, `[STD-IMPL-2]`, `[VERIFY-1]`, `[VERIFY-2]`, `[COMP-1]`, `[GEN-COH-1]`, `[GEN-COH-2]`): normative API/source correspondence, standard-library conformance, MIR verification, pass preservation, the drop-check phase, and generic coherence. | XXIV |
| 7 | **OQ-8 resolved** (H12, `[OQ8-1]`..`[OQ8-7]`): the ECS is **library-owned** in `std.ecs`, with a RageV-compatible sparse-set layout, stable query iteration, access-set enforcement, comptime specialisation without language specialisation, FFI interoperability, and **no ECS compiler magic**. This keeps Part XII's facilities general rather than growing an engine into the language. | XXIV §H12 |
| 8 | **ARC-cycle analysis and diagnosis** (H14, `[WK-5]`..`[WK-10]`): a whole-program-visible ownership graph over strong edges only, `L3001` diagnostics naming the cycle, `ember explain --cycle` and `ember inspect --cycle`, and `Weak[T]` fix-it guidance. Diagnostic only — `[WK-5]` alters no ownership semantics, and `[WK-1]`'s statement that cycles leak is unchanged. | XXIV §H14.1 |
| 9 | **Dynamic exclusivity check hoisting** (H14, `[EXC-8]`..`[EXC-14]`, `[CLI-16]`): loop-hoisted class exclusivity tokens, nested-loop reuse, and the reporting classes `ember inspect --safety` must distinguish. `[EXC-3]`'s closed-interval restriction and `[EXC-3a]`'s elision record are unchanged. | XXIV §H14.2 |
| 10 | **Budgets and conformance** (`[BUD-7]`, `[TST-14]`..`[TST-16]`, `[TST-20]`..`[TST-23]`). | XXIV, XX.5 |
| 11 | No diagnostic code is added by 0.9 itself; `L3001` is reused for the cycle lint. | — |
...
fn_type         := ["@latebound"] ["extern" string_lit] "fn" "(" [fn_type_param {"," fn_type_param}] ")" ["->" type]
fn_type_param   := [("mut" | "owned")] type
dyn_type        := "dyn" bound_list
array_type      := "[" type ";" expression "]"                          (* fixed-size inline array, e.g. [f32; 16] *)
```

`[GRM-3]` `Span[T]`, `MutSpan[T]`, `Array[T]`, `Option[T]`, `Result[T, E]`, `Box[T]`, `Shared[T]`, and `Weak[O]` are ordinary library types spelled with `path_type`; the grammar does not special-case them. `[HEAP-7]` constrains the owner forms permitted for `O`.
...
| `@latebound` | **callable type only** — not an item or statement attribute | late-bound callable boundary: fresh invocation-local regions for borrowed and view parameters (`[FN-6b]`, Part XXIV §H14.3). Written `@latebound fn(Span[T]) -> R`; in any item or statement position it is `E0104`. |
...
* `Span[T]` (read) and `MutSpan[T]` (read/write): pointer + length. `Copy` and move-only respectively. Bounds-checked indexing; `.len()`, `.iter()`, `.iter_mut()`, `.split_at(i)`, `.chunks(n)`, `.chunks_mut(n)`, `.as_ptr()`, and `.as_mut_ptr()` as applicable and specified by `[SPN-1]`–`[SPN-10]`. Raw-pointer extraction is safe; use of the returned pointer remains governed by `[UNS-1]`.
...
**Standard interfaces** that the compiler knows about are spelled as ordinary interfaces. They live
in `std.core` except where a rule below assigns a different standard module:
...
* `[HASH-1]` **Hashing contract.** `std.collections` publicly exports the `Hash` and `Hasher`
  interfaces. `Hash` remains a prelude name under `[MOD-5]`; `Hasher` is not a prelude name. Their
  canonical interfaces are the declarations above. The hasher parameter to `Hash.hash` is statically
  generic: `H` is inferred from the concrete argument and is normally monomorphized through the
  existing `[TYP-17]`/`[TYP-18]` machinery. `Hash.hash` MUST NOT use bare `Hasher` as a concrete
  parameter type and introduces no mandatory dynamic dispatch. A separately specified API MAY use
  an explicit legal `dyn Hasher` representation when it intentionally requires runtime
  polymorphism; `Hash.hash` is not such an API. `Hasher` is a stateful, move-only hashing
  context. `finish(owned self)` consumes the hasher and returns the final `u64` hash value. The
  owner's prose explicitly requires consumption; the canonical signature therefore spells the
  existing `owned self` mode rather than the contradictory borrowed `self` shorthand present in the
  submitted sketch. A `Hash` implementation MUST feed a deterministic representation of the value
  into the supplied hasher. Equal values under `Eq` MUST produce equal hash values; the converse is
  not required. `Hash.hash` MUST NOT inspect or retain the hasher after returning, and a hasher MUST
  NOT retain any supplied `Span` beyond the corresponding call.
* `[HASH-2]` **Canonical hasher.** `std.collections` publicly exports the default concrete hasher
  `DefaultHasher`, which implements `Hasher`, with constructor
  `DefaultHasher.new() -> DefaultHasher`. `DefaultHasher` is not a
  prelude name. `Map` and `Set` MUST use `DefaultHasher` unless a future language/library revision
  explicitly specifies a custom-hasher API. The exact mixing algorithm is an implementation detail
  and is not part of the source-level compatibility contract unless a future rule explicitly freezes
  it.
* `[HASH-3]` **Hash coherence.** For every type `T` implementing both `Eq` and `Hash`, `a == b`
  implies that hashing `a` and `b` with equivalent fresh hashers produces equal final hash values.
  The compiler MUST NOT require unequal values to have distinct hashes. Violating this coherence
  requirement is a semantic error in the `Hash` implementation; `Map` and `Set` MUST NOT weaken their
  equality semantics to compensate.
* `[HASH-4]` **Map key requirement.** `ArenaMap[K, V]` and ordinary `Map[K, V]` require
  `K: Eq + Hash`. A container MAY call `hash` multiple times during lookup or insertion, and callers
  MUST NOT rely on a particular call count. Safe Map APIs MUST NOT expose `ref mut K` or otherwise
  permit a resident key to be changed in a way that changes its `Eq` or `Hash` result unless the
  implementation can prove that the Map invariant remains intact. Lookup and iteration expose keys
  only as values accepted by the existing borrowed parameter modes or as shared `ref K` views.
...
* `[MOD-6]` A module MAY declare a supported language version with `#! language "X.Y"` on its first line; the package's `ember.toml` `language` key is the default. Mismatch with the compiler's supported set is `E0006`. The compiler's supported set MUST include every language version whose source it still accepts. For this revision the supported set MUST include `"0.9.8"`, `"0.9.7"`, `"0.9.6"`, `"0.9.5"`, `"0.9"`, `"0.8.5"`, `"0.8.4"`, and `"0.8.3"`. Selecting an older version selects that version's accepted-program and semantic rules; it does not silently opt the module into later language semantics. *(updated 2026-09-20; ODR-017)*
* `[MOD-6a]` **Patch-level language revisions.** A language version MAY be written as `X.Y` or `X.Y.Z`. `0.9` remains the pre-0.9.5 0.9 language contract; `0.9.5` selects inferred multi-region-view semantics; `0.9.6` preserves the complete 0.9.5 source-language accepted/rejected program sets and observable semantics while selecting that revision's reference-implementation and conformance contract; `0.9.7` adds the explicit `@latebound` callable-type boundary of `[FN-6b]`/`[LT-7]`; and `0.9.8` adds the owner-selected `Shared[T]` and generalized `Weak[O]` surface of `[HEAP-3]`–`[HEAP-7]` and `[WK-11]`–`[WK-14]`. The compiler MUST NOT silently enable `0.9.5`, `0.9.6`, `0.9.7`, or `0.9.8` semantics for a module that explicitly selects an earlier version. The supported set is exactly the set stated by `[MOD-6]`; the package manifest and module directive MUST resolve to the same exact language contract after normalization.
...
* `[FN-6a]` **`Callable` preserves parameter modes.** The existing `Callable[Args, R]` abstraction
  ...
  reinterpreted as a new callable ownership mode. A callable supplied where the expected callable
  requires a different parameter mode, or whose mode vector `Args` would erase, is `E2228`, shape
  **B15** (owner decision ODR-016). The diagnostic MUST name the callable, the differing parameter
  position, the expected mode and the supplied mode. `E2228` is the parameter-mode diagnostic
  specifically and MUST NOT become a general callable type-mismatch code.
...
* `[FN-6b]` **Late-bound callable types.** `@latebound` is permitted only immediately before a
  callable type, as in `@latebound fn(Span[T]) -> R`; it is not an item attribute and does not alter
  the declaration or ordinary uses of the function value supplied to that boundary. It marks a
  callable boundary at which the callee selects fresh, invocation-local regions for every borrowed
  or view parameter under `[LT-7]`. The modifier is part of canonical callable type identity and
  MUST survive generic bounds, substitution, expected-callable checking, interface compatibility,
  and incremental invalidation. It MUST NOT create named source regions, runtime metadata, ABI
  bytes, reload-schema data, or a separate dispatch or ownership model. An ordinary `fn(...) -> R`
  remains an ordinary callback boundary.
...
* `[LT-4a]` **Arena-backed wrapper provenance.** An `Arena` parameter MAY be named in `@borrows` when a function returns a view whose storage is owned by that arena. The named parameter contributes the arena's borrow region to the returned value. For example:
  ...
  Standard-library signature sketch:
...
* `[LT-7]` **Late-bound callback regions.** A callback-taking API exposes a late-bound callback
  boundary only by writing `@latebound` on that callable type under `[FN-6b]`. For every invocation,
  the callee chooses a fresh compiler-internal region for each borrowed or view parameter of that
  callback. The callback is valid for any such chosen region; its borrowed arguments are valid only
  for that invocation unless the API separately transfers an owned value. `@latebound` does not name
  or expose those regions: region variables remain absent from Ember source, public generic arguments,
  ABI-visible names, runtime metadata, and reload schemas. The result of a `@latebound` invocation
  MUST be proven not to contain one of its invocation-local regions. Implementations MUST reject a
  callback-local view that escapes by return, storage, an `owned fn` capture, FFI publication, or any
  other publication path through the existing region/escape machinery. `[THR-5]`'s and `[JOB-2]`'s
  scope regions are instances of this rule rather than bespoke exceptions, and `[GPU-9]`'s
  `pass.native(fn(cmd) => …)` is expressible under it.
...
* `[SPN-4]` **Canonical iterator types.** `std.collections` publicly exports
  `SpanIter[T]`, `MutSpanIter[T]`, `SpanChunks[T]`, and `MutSpanChunks[T]`.
  None is a prelude name. They are named `@view` iterator types implementing
  the existing associated-type interface as follows:

  ```ember
  SpanIter[T]       implements Iterator[Item = ref T]
  MutSpanIter[T]    implements Iterator[Item = ref mut T]
  SpanChunks[T]     implements Iterator[Item = Span[T]]
  MutSpanChunks[T]  implements Iterator[Item = MutSpan[T]]
  ```

  These are ordinary library types, not compiler intrinsics or new ownership
  categories. Their concrete names make the associated `Item` contracts
  expressible without introducing opaque iterator returns.
* `[SPN-5]` **Iterator construction.** The canonical APIs are given by this
  standard-library signature sketch:
  ...
  `MutSpan.iter()` creates a shared reborrow and prevents conflicting mutation
  for the resulting iterator's lifetime. `MutSpan.iter_mut()` creates a
  mutable reborrow and does not consume the underlying `MutSpan`; after that
  iterator's borrow ends, ordinary NLL/reborrow rules permit the original
  `MutSpan` to be used again. No iterator may outlive its source view.
* `[SPN-6]` **Chunk APIs.** The canonical APIs are given by this
  standard-library signature sketch:
  ...
  For `n > 0`, iteration partitions the source into consecutive,
  non-overlapping chunks `[0 .. n]`, `[n .. 2n]`, and so on through the
  source length. The final chunk MAY contain fewer than `n` elements.
  `chunks_mut` produces disjoint `MutSpan[T]` values whose storage identities
  cover only their respective ranges; no two yielded mutable chunks may
  overlap. It is a sanctioned construction under `[BRW-5]`, not a new
  aliasing mechanism.
* `[SPN-7]` **Zero chunk size.** `chunks(0)` and `chunks_mut(0)` are invalid and
  MUST panic through the existing bounds/argument failure mechanism in every
  supported profile. They MUST NOT return an empty iterator or `Result`, yield
  zero-width chunks, or loop indefinitely. A future change to this failure
  policy requires an explicit language revision.
* `[SPN-8]` **Raw-pointer extraction.** The canonical APIs are given by this
  standard-library signature sketch:

  ```ember,ignore
  Span.as_ptr(self) -> *T
  MutSpan.as_ptr(self) -> *T
  MutSpan.as_mut_ptr(mut self) -> *mut T
  ```

  Extraction itself is safe because it neither dereferences nor accesses the
  pointed-to memory. Dereference, reads, writes, reference construction, and
  pointer arithmetic not otherwise proven safe require the existing `unsafe`
  rules. `MutSpan.as_mut_ptr` takes a mutable reborrow and does not consume the
  underlying view. A returned pointer carries no safe Ember borrow guarantee.
* `[SPN-9]` **Pointer lifetime and provenance.** `as_ptr` and `as_mut_ptr` do
  not extend, retain, or pin the source view or its storage. Code leaving the
  safe borrow model through a raw pointer is responsible for validity under
  the existing unsafe contract.
* `[SPN-10]` **Public module boundary.** The four iterator/chunk types are
  public exports of `std.collections`, but none is exported by the prelude.
  No opaque iterator-return mechanism is introduced by this completion.
* `[TST-25]` **Span API conformance.** Conformance MUST cover: (1)
  `Span.iter()` yielding `ref T`; (2) `MutSpan.iter()` yielding shared `ref T`;
  (3) `MutSpan.iter_mut()` yielding `ref mut T`; (4) reuse of the original
  `MutSpan` after the mutable iterator borrow ends; (5) rejection of conflicting
  access while an iterator/reborrow remains live; (6) normal and final-partial
  `chunks(n)`; (7) non-overlapping mutable results from `chunks_mut(n)`; (8)
  mutable-chunk alias rejection and interaction with `split_at_mut`; (9)
  all-profile panic through the existing failure path for `chunks(0)` and
  `chunks_mut(0)`; (10) safe extraction through `as_ptr` and `as_mut_ptr`; (11)
  unsafe-only dereference/use of raw pointers; (12) no source-lifetime extension
  from pointer extraction; (13) generic `T` substitution through every iterator
  and chunk type; and (14) ABI/layout erasure of region metadata. This matrix
  introduces no new borrowing or lifetime mechanism.
...
Every counted allocation — a class instance or `Shared[T]` payload — is a heap block:
...
24      …      base-class fields then own fields, or the single `Shared[T]` payload, each at its natural alignment
```
...
* `[OBJ-3]` `weak_count` starts at 1 for the "strong side"; when `strong_count` hits 0 the allocation is deinitialised (a class runs its `drop` chain and field drops; `Shared[T]` runs `T`'s ordinary destruction) and `weak_count` is decremented; the block is freed when `weak_count` hits 0. (Rust `Rc` scheme; avoids a separate control block.)
...
* `[RC-1]` Copying a counted owner emits `retain`; dropping one emits `release`; `release` reaching zero calls `ember_rt_deinit(obj)`.
...
* `[RC-5]` A loan whose place chain contains a `Deref` of a class handle or `Shared[T]` owner constitutes a **shared loan of that owner operand itself**. `ref h.f`, `ref mut h.f`, `s.get()`, `s.get_mut()`, a `Span`/`MutSpan` derived from a counted allocation, a `Ref[T]`/`RefMut[T]` guard obtained from one, and any view struct built from these, all keep their owner borrowed for `[BRW-1]` purposes until the last use of any value derived from them. The compiler MUST NOT kill that loan earlier, MUST NOT sink the owner's `release` above it, and MUST report `E3060` (shape B7) if the owner's storage ends first. XIX §4.7 **step 2** generates the extra loan when lowering a projection through a counted owner.
...
* `Weak[O]` does not keep a counted owner alive. `Weak(h)` creates one; `Weak[O].empty()` creates an empty one; and `w.upgrade()` returns `Option[O]`.
* `[WK-1]` Strong reference cycles among class instances and `Shared` allocations leak (they are never collected). This is a documented property, not a bug. Mitigations the language provides: `Weak` for back-pointers; the debug runtime's **cycle report** (`ember run --leak-check`), which on process exit walks all live counted allocations (the runtime keeps an intrusive list in debug builds) and prints any strongly connected components with the field names forming the cycle; and the lint `L3001 potential cycle: field <f> of class <A> holds <B> and <B>.<g> holds <A>` for statically visible cycles.
* `[WK-2]` Ordinary destruction runs when the strong count hits zero even if weak handles remain; those weak handles then fail to upgrade.
* `[WK-3]` `Weak.upgrade` MUST return `None` when `flags.is_deinitialising` is set on the target, and `retain` on such an allocation MUST NOT be treated by any optimisation as producing a usable owner. This is the mechanism behind `[WK-2]`.
* `[WK-11]` **Weak creation, copying, and dropping.** `Weak(owner)` creates a non-owning weak handle without consuming or retaining the strong owner. `Weak[O].empty()` creates an empty weak handle. `Weak[O]` is `Copy`: copying increments the weak count and dropping decrements it.
* `[WK-12]` **Weak upgrade.** `Weak[O].upgrade() -> Option[O]` returns a retained strong owner only while its target is strongly alive and not deinitialising; otherwise it returns `None`. Where the represented owner count is atomic, upgrade uses that same atomic lifetime protocol.
* `[WK-13]` **Shared weak identity.** A `Weak[Shared[T]]` refers to the same allocation and control block as its source `Shared[T]`; it never creates an independent control block.
* `[WK-14]` **No ownership conversion.** `Weak[C]` and `Weak[Shared[T]]` are distinct owner forms and do not interconvert. Neither Ember weak-owner form, nor `Shared[T]`, converts to `CppWeak[T]` or `CppShared[T]`.
...
| `Shared[T]` | shared, counted | borrow-checked + dynamic exclusivity for long-term mutable access | deterministic, at count 0 | atomic count iff `T: Sync` | shared ownership of a value payload where class identity is not wanted. Advanced; prefer `class` (IX.1) |
| `Weak[O]` | none | — | never | follows `O` | breaking a cycle, or observing a counted owner without keeping it alive (VIII.5, `[WK-1]`, `[HEAP-7]`) |
...
* `[SEL-2]` **Two mechanisms are never interchangeable across the foreign
  ...
  *different reference counts*; `Weak[C]` and `Weak[Shared[T]]` likewise never
  interconvert with `CppWeak[T]`. Converting one
...
| `Shared[T]` | reference-counted heap `T`, with the same header as classes; `Shared(value)`, `s.get() -> ref T`, `s.get_mut() -> ref mut T`, and companion `Weak[Shared[T]]`; long-term mutable access follows Part VIII §3 | Copy (retain) | count atomic iff `T: Sync` |
...
* `[HEAP-3]` **Shared construction.** `Shared(value: T) -> Shared[T]` allocates one counted heap object containing `T`, moves or copies `value` according to its ordinary argument mode, and returns the first strong owner.
* `[HEAP-4]` **Shared access.** `Shared[T].get() -> ref T` returns a shared borrow of the contained value. It neither retains nor transfers the `Shared` owner; the resulting loan keeps the allocation live for its region under `[RC-5]`.
* `[HEAP-5]` **Shared mutable access.** `Shared[T].get_mut(mut self) -> ref mut T` returns a mutable borrow of the contained value. `mut self` does not require or imply unique strong ownership. The access uses the same static borrowing and dynamic-exclusivity semantics as a long-term mutable class access; no `Shared`-specific aliasing mechanism exists.
* `[HEAP-6]` **Shared retention.** `Shared[T]` is `Copy`: copying retains the strong count and dropping releases it. The final release deinitializes the allocation and runs `T`'s ordinary destruction before the control block can be freed under `[OBJ-3]`.
* `[HEAP-7]` **Weak owner forms.** `Weak[O]` is legal exactly when `O` is a counted owner supported by the language: a class handle `C` or `Shared[T]`. `Weak[C]` preserves its existing class semantics; the companion of `Shared[T]` is `Weak[Shared[T]]`, not an overloaded `Weak[T]` spelling.
* `[TST-26]` **Shared and weak-owner conformance.** Conformance MUST cover `Shared` construction, copy/final release, `get` and `get_mut` borrow lifetime and conflicts, NLL reuse, weak construction/empty/copy/drop, live and expired upgrade, deinitialising upgrade failure, class `Weak[C]` regression, cycle-breaking behavior, foreign-owner rejection, and the atomic upgrade path when `T: Sync`.
...
* `[ARN-2]` Values allocated in an arena are **not dropped individually**. Consequently, ordinary Arena bulk allocation MUST NOT accept an element type for which `needs_drop(T)` is true, regardless of whether `T` implements `Default`, `Zeroable`, or both. This guarantees that ordinary Arena reclamation never needs per-element destructor execution.
* `[ARN-3]` Allocating a type that `needs_drop` in an arena is `E3090` unless the call is `alloc_nodrop` (explicit acknowledgement that `drop` will never run) — this keeps arenas free of destructor bookkeeping and prevents silent resource leaks of handles/GPU objects. `alloc_array[T](count)` always requires `!needs_drop(T)` and resolves initialization deterministically: if `T: Zeroable`, it allocates zeroed storage and treats every element as initialized `T`; otherwise, if `T: Default`, it calls `T.default()` once for each element; otherwise it reports the existing unsatisfied-bound diagnostic `E2040`. If both capabilities exist, `Zeroable` takes precedence. That precedence is observable API semantics and MUST NOT be changed by optimization.
...
* `[ARN-5]` **Fixed-capacity Arena-backed containers.** `ArenaArray[T]` and
  `ArenaMap[K, V]` are fixed-capacity, Arena-backed view containers. Their
  backing storage is allocated once from the supplying `Arena` at
  construction. They do not own that storage, MUST NOT outlive its Arena
  region, are `@view` types, and follow `[TYP-15]`. Neither type grows,
  reallocates, moves its backing buffer, or obtains more Arena storage after
  construction. This intentionally prevents growth from invalidating element
  views.
* `[ARN-5a]` **Public types, construction, and provenance.** `std.collections`
  publicly exports `ArenaArray`, `ArenaMap`, `CapacityError`,
  `ArenaArrayIter`, `ArenaArrayIterMut`, and `ArenaMapIter`. None is
  exported by the prelude. `CapacityError` is the unit-only public enum:
  ...
  Both constructors return an empty container with `len() == 0` and the
  requested maximum capacity, including when capacity is zero. The `arena`
  parameter is shared/borrowed as in `[ARN-1]`; construction needs no
  `mut arena`. The result carries the Arena's region through
  `@borrows(arena)`. Container methods use `self` or `mut self` according
  to ordinary observation/mutation and never mutate the Arena cursor.
* `[ARN-5b]` **Capacity, failure, and stability.** The construction capacity
  is the maximum number of stored entries. An operation that would add an
  entry beyond capacity MUST return `Err(CapacityError.Full)` through its
  specified `Result`; ordinary capacity exhaustion MUST NOT panic, allocate,
  reallocate, modify the Arena cursor, invalidate an existing element view, or
  partially modify the container. A replacement of an existing Map key does
  not consume another capacity slot. Because the backing buffer never moves,
  an element view remains storage-valid until that element is removed or the
  supplying Arena region ends, subject to the ordinary borrow/liveness rules.
* `[ARN-5c]` **`ArenaArray[T]` minimum API.** The canonical minimum API is:
  ...
  clear(mut self) -> void
  iter(self) -> ArenaArrayIter[T]
  iter_mut(mut self) -> ArenaArrayIterMut[T]
  ```

  `ArenaArrayIter[T]` is an Arena-backed `@view` iterator implementing the
  existing associated-type contract `Iterator[Item = ref T]`.
  `ArenaArrayIterMut[T]` is an Arena-backed `@view` iterator implementing
  `Iterator[Item = ref mut T]`. Iteration is in increasing index order and is
  deterministic. Indexing and insertion indices follow ordinary
  `Array`/`Span` bounds rules. `remove` moves out the removed value.
* `[ARN-5d]` **`ArenaMap[K, V]` minimum API.** `ArenaMap[K, V]` requires
  `K: Eq + Hash`, using `[HASH-1]`–`[HASH-4]`, the same equality/hash contract
  as ordinary `Map[K, V]`. Its canonical minimum API is:
  ...
  iter(self) -> ArenaMapIter[K, V]
  clear(mut self) -> void
  ```

  `ArenaMapIter[K, V]` is an Arena-backed `@view` iterator implementing
  `Iterator[Item = (ref K, ref V)]`. Inserting a key already present replaces
  its value and returns `Ok(Some(old_value))`. Inserting a new key returns
  `Ok(None)`; at full capacity it returns `Err(CapacityError.Full)` without
  modifying the map. Map iteration order is unspecified but deterministic for
  an unchanged map state and implementation configuration. Programs MUST NOT
  depend on a particular bucket order. A stable-insertion-order map would be a
  separate API revision. Lookup and iteration never expose `ref mut K`; keys
  remain read-only while resident so their equality/hash identity cannot be
  invalidated through a safe Map API.
* `[ARN-5e]` **Drop-free element boundary.** `ArenaArray[T]` requires
  `!needs_drop(T)`. `ArenaMap[K, V]` requires `!needs_drop(K)` and
  `!needs_drop(V)`. Arena rewind/drop runs no element destructors. `remove`
  and duplicate-key replacement may move an initialized value out before
  reclamation, but do not make the Arena responsible for its destructor. A
  future arena-backed container admitting `needs_drop` contents requires a
  separately specified ownership/destruction model.
* `[ARN-5f]` **Borrowing and mutation.** Reads and element views create
  ordinary shared borrows; mutable element views and all mutating operations
  use ordinary mutable borrows. The `mut self` methods borrow the whole
  container under the existing `[BRW-*]` rules, so a live element borrow
  conflicts with an overlapping mutating call. Removal cannot occur while its
  element is borrowed; `clear` cannot occur while any element is borrowed;
  and Arena reset/drop cannot occur while a container, iterator, or element
  view derived from it remains live. The compiler MUST NOT introduce a
  container-specific invalidation mechanism.
* `[ARN-5g]` **No hidden Arena mutation.** `push`, Array `insert`/`remove`,
  Map insertion/replacement/removal, and `clear` modify only the
  already-allocated container storage. They MUST NOT call `Arena.alloc*`,
  change the Arena allocation cursor, create a second hidden Arena region, or
  allocate a replacement buffer. The constructor is the only operation that
  obtains backing storage from the Arena.
* `[TST-24]` **Arena-backed container conformance.** Conformance MUST cover:
  (1) both empty `with_capacity` constructors and Arena return provenance;
  (2) fixed-capacity insertion and `CapacityError.Full` at exhaustion;
  (3) generated/runtime evidence that construction allocates once and later
  mutation neither grows nor changes the Arena cursor; (4) the complete
  `ArenaArray` API, bounds behavior, index-order iteration, and mutable
  iteration; (5) the complete `ArenaMap` API, Eq/Hash bounds, duplicate
  replacement, removal, lookup, clear, and unchanged-state deterministic
  iteration; (6) rejection of `needs_drop` element/key/value types;
  (7) rejection of conflicting removal, clear, and other `mut self` calls
  while element/iterator borrows are live; (8) rejection after the supplying
  Arena region ends and rejection of Arena reset/drop while derived views live;
  (9) named iterator types using the existing associated-type
  `Iterator[Item = ...]` contract; and (10) generic element/key/value
  substitution and Arena provenance through generic wrappers; (11) built-in
  and user-defined keys satisfying `[HASH-1]`–`[HASH-4]`, including equal-key
  coherence; and (12) the absence of safe mutable key access.
...
* `[ARN-8]` **`MaybeUninit[T]`.** `MaybeUninit[T]` has exactly the size and alignment of `T` and carries storage for a `T` without asserting that the bytes constitute a valid initialized `T`. Dropping it MUST NOT run `T`'s destructor. It is `Copy` iff `T: Copy`; otherwise it is move-only. Safe code MAY write a valid `T` into the storage but MUST NOT read it as `T` before initialization has been established. The canonical operations are:

  ```ember
  MaybeUninit[T].uninit() -> MaybeUninit[T]
  MaybeUninit[T].write(mut self, owned value: T) -> ref mut T
  unsafe MaybeUninit[T].assume_init(owned self) -> T
  ```

  `uninit()` is safe because its result does not claim to contain a `T`. `write` moves `value` into the slot without dropping the previous bytes and returns a mutable reference, with the slot's region, to the newly initialized value. `assume_init` consumes the slot and asserts that it contains a valid initialized `T`; violating that precondition is undefined behavior under the existing unsafe-memory rules. It MUST NOT manufacture a `T` from merely zeroed bytes unless `T: Zeroable`. No safe operation may read the contents as `T` before initialization is established.
* `[ARN-8a]` **Destruction after initialization.** Moving an initialized `T` out of `MaybeUninit[T]` transfers ownership to the destination and leaves the source slot uninitialized; the source slot runs no destructor. Even when `T: Drop`, `MaybeUninit[T]` itself never runs `T`'s destructor. A destructor runs only after the value has become an ordinary initialized `T` and later reaches an ordinary ownership destruction point.
* `[ARN-9]` **Uninitialized Arena allocation.** `arena.alloc_uninit[T](count) -> MutSpan[MaybeUninit[T]]` is safe and imposes no `Zeroable`, `Default`, or initialized-value requirement on `T`, because the returned type makes no initialization claim. `MutSpan[MaybeUninit[T]]` provides:

  ```ember
  write_at(mut self, index: usize, owned value: T) -> ref mut T
  unsafe assume_init(owned self) -> MutSpan[T]
  ```

  `write_at` bounds-checks by the ordinary `MutSpan` rules, initializes exactly one element without dropping its previous bytes, and returns a mutable reference to it. Span `assume_init` consumes the source span and asserts that every element exposed by the result is initialized. Calling either `assume_init` outside an `unsafe` context is rejected by `[UNS-1]`; calling it inside `unsafe` without satisfying its full-initialization precondition is undefined behavior. Partial initialization never establishes a safe full-span conversion. An implementation MAY track individual initialized elements or ranges for diagnostics, but soundness MUST NOT depend on a particular tracking precision.
* `[ARN-10]` **Transactional Arena initialization.** When `arena.alloc_array[T](count)` uses `T: Default`, the implementation constructs each element using `T.default()`.

  If a separately specified recoverable construction failure occurs, the Arena allocation cursor MUST be restored to its state immediately before the `alloc_array` operation, and no partially initialized result may become reachable.

  In v1, `T.default()` has no recoverable construction-failure path. A panic terminates according to `[PAN-1]` through process termination via `abort()`. No post-panic Arena state is observable, so this rule does not require panic recovery or unwinding.

  No rollback obligation arises unless a separate API explicitly defines both a recoverable construction-failure path and transactional rollback semantics.

  Because ordinary `Arena.alloc_array[T]` requires `!needs_drop(T)`, rollback does not require execution of element destructors. Conformance MUST establish successful `Default` construction, process termination through the `[PAN-1]` abort path on panic, no continuation after that panic, no compiler-invented unwinding solely for this rule, and no observation of post-panic Arena state. Any future recoverable construction API is tested against its own explicitly specified rollback contract.
* `[ARN-11]` **`Zeroable` validity.** `Zeroable` is an unsafe marker whose sole semantic claim is that an all-zero object representation is a valid initialized `T`. It implies neither `Copy`, `Send`, `Sync`, FFI safety, nor any ownership property. The compiler MAY implement it automatically only when it proves the property recursively from representation. Automatic derivation MUST reject any field for which all-zero validity is unproven, including `ref T`, a range excluding zero, or an enum layout whose zero discriminant or payload is invalid. A manual implementation, if exposed, requires `unsafe` and the existing `@safety` contract machinery.
* `[ARN-12]` **Phase ordering.** The core `Zeroable` validity predicate and built-in implementations needed by `alloc_array` MUST exist in the Arena implementation phase. General `@derive(Zeroable)` generation MAY remain in the later derive phase. Phase ordering MUST NOT make the already-normative `alloc_array` behavior impossible to implement.
* `[ARN-13]` **Canonical initialization boundary.** The v1 spellings are `MaybeUninit.uninit`, `MaybeUninit.write`, `MaybeUninit.assume_init`, `MutSpan[MaybeUninit[T]].write_at`, and `MutSpan[MaybeUninit[T]].assume_init`. Implementations MUST NOT add a second safe conversion from `MaybeUninit[T]` to `T`, or from `MutSpan[MaybeUninit[T]]` to `MutSpan[T]`, with different semantics. A future checked conversion is an additive language revision, not an implementation-defined alias. `write`/`write_at` are the safe initialized-state transitions; both `assume_init` forms are the one explicit unchecked boundary.
...
* `[UNS-5]` `MaybeUninit[T]`, `transmute[A, B]`, `ptr.copy_nonoverlapping`, and `mem.zeroed[T]()` are provided in `std.mem`. `mem.zeroed[T]()` requires `T: Zeroable`; `[ARN-8]`–`[ARN-13]` define `MaybeUninit`, `Zeroable`, and their initialization/safety boundary completely. “POD” or “all-scalar” is not a substitute for `[ARN-11]`'s recursive all-zero-validity proof.
...
| `std.collections` | `Array`, `SmallArray`, `Deque`, `Map`, `Set`, `BitSet`, `Pool`, `Handle`, `SoA` (derive support), `Hash`, `Hasher`, `DefaultHasher`, `ArenaArray`, `ArenaMap`, `CapacityError`, `ArenaArrayIter`, `ArenaArrayIterMut`, `ArenaMapIter`, `SpanIter`, `MutSpanIter`, `SpanChunks`, `MutSpanChunks`; `Hash` retains its `[MOD-5]` prelude export, while `Hasher`, `DefaultHasher`, the Arena-backed names, and the Span iterator/chunk names are public but not prelude exports |
...
```
 .em files ─► Lexer ─► Parser ─► AST ─► Name resolution ─► Type checking/inference ─► HIR (typed, desugared)
                          ...
                          ┌──────────────────────────────────────────────────────────────┘
                          ▼
   HIR ─► initial MIR ─► callable/provenance summaries ─► definite-init/ownership/borrow/region analysis
                                                                                         │
                          ┌──────────────────────────────────────────────────────────────┘
                          ▼
   Drop elaboration ─► MIR verification ─► Monomorphisation ─► MIR optimisations
                                                              │
                                                              ▼
                                  Effect analysis (contract profile, `[EFF-15]`)
                                                              │
                                                              ▼
                                                       MIR verification
                                                              │
...
Initial MIR is an analysis substrate and is not codegen-ready. The verified post-analysis and
post-optimisation MIR is the code-generation boundary. Summary derivation and interprocedural
analysis may iterate or interleave to reach a sound fixed point, but only summaries derived from
verified MIR or supplied by an existing audited `unsafe`/FFI contract may become trusted interface
metadata.
...
7. Emit the completed typed HIR.
...
A shared storage-lowering helper MUST preserve the operation's explicit replacement mode. Ordinary
assignment evaluates the new value, drops the old live value, then stores the new value (`[OWN-5]`).
`Cell.set` stores the new value before dropping the old value (`[CELL-1]`). `MaybeUninit.write` and
`write_at` store the new value without dropping prior bytes (`[ARN-8]`, `[ARN-9]`). These orderings
MUST NOT be generalized into one universal assignment rule.

### 4.6 Definite initialisation

Forward dataflow over MIR with a lattice per local (`Uninit | Init | Maybe`), also per field path for structs (partial moves/inits) and for `self` fields inside `init` methods (`[CLS-2]`). Reading `Uninit`/`Maybe` is `E3050`/`E2100`; `Maybe` at a drop point introduces a **drop flag**. Storage represented as `MaybeUninit[T]` uses the same analysis infrastructure while preserving its two-state language contract: layout alone never establishes initialization, `write`/`write_at` establish initialized slots, and consuming `assume_init` is the explicit unsafe assertion. Finer element/range tracking may improve diagnostics or optimization but MUST NOT become a runtime tag or an implementation-dependent safe conversion (`[ARN-8]`–`[ARN-13]`).
...
field-access and field-to-region provenance summaries in its interface artifact. For visible code the
compiler derives exact summaries from MIR. Separately compiled or intentionally opaque code may supply
a narrower declared summary only through compiler metadata derived from verified MIR or an existing
audited `unsafe`/FFI contract whose obligations cover access and retention; safe source code cannot lie
about this contract. Where neither source exists, the summary conservatively names all relevant fields
and all regions permitted by the callable type. Dynamic/interface dispatch uses the union of every
contractually valid current, separately compiled, and hot-reload target, or the conservative result when
that set cannot be bounded soundly.

Both summaries are erased from source syntax and runtime ABI but participate in interface hashing,
incremental-cache keys, and generic dependencies. Changing either summary MUST recheck affected callers
and invalidate dependent artifacts. The summaries MUST be sufficient for a caller to determine which
region slots are required by each call and where every returned view field derives from without
re-reading the callee body. Unavailable, stale, or incompatible trusted metadata is a hard compiler
failure, never permission to assume a narrower access set or an optimization hint. No summary or region
metadata enters runtime layout, serialization, ABI, or hot-reload schemas.
...
ember inspect --cycle <path>       ownership graph: strong/weak/unknown edges, shortest static cycle ([CLI-17])
ember explain --cycle <path> <Class[.field]>
                                  explain one class/field in that root's ownership graph ([WK-9], [CLI-18])
...
* `[MAN-6]` `[build] max_instantiations` is a positive integer or absent; absent means no ceiling and disables everything in XIX §4.11a (`E9034` otherwise).
* `[MAN-7]` `[build] reload` is one of `"all"`, `"opt-in"`, `"bodies"` or `"none"` (default `"opt-in"` in `debug`, `"none"` otherwise); any other value is `E9031`. It is forbidden in `shipping` (`E9033`). Every package linked into one process MUST agree on whether reload is enabled at all, which `[HR-12a]`'s link-time symbol enforces.
...
* `[TST-4c]` **Transition.** `[TST-4a]` ships with a **recorded baseline** of rules that currently have only an accept case, in the manner of `[TST-7]`, and fails CI only on a rule added or amended after 0.8.1 and on any rule whose entry improves and then regresses. The baseline shrinks and never grows: `tools/rule_index.py` rejects a commit that adds a rule to it, with **one permitted reason, in the manner of `[TST-7]`** — the revision that *creates* a rule MAY enter it in the same commit, when the rule specifies behaviour the compiler does not yet implement. The entry MUST name that revision, and no later commit may add the rule. *(stated by `0.9.7_Hardened_2`; 0.8.5's change log already placed `[CELL-12]`, `[UNS-10]`, `[UNS-10a]` and `[UNS-10b]` in the baseline citing this exact reason, which was stated in neither `[TST-4c]` nor `[TST-7]` — the enumeration was stale, not the practice, as in `0.8.4_Hardened_2`'s `[EFF-18]` repair.)* `[GATE-1]` requires the baseline to be **empty** for 1.0, which is what makes this a transition rather than a permanent exemption.
...
* `[DIA-2]` Message style: lowercase first letter, no trailing period, name the thing (`` `ps` ``), say what is wrong and, in `help`, what to do. Never say "you"; never blame the user. The primary message MUST explain the source-level semantic failure. Compiler-internal region numbers, capability identities, storage identities, and MIR facts MAY appear as secondary expert evidence, but MUST NOT replace the source value, place, lifetime, ownership, permission, or contract relationship that failed.
...
* `[DIA-6a]` The error-code registry is **exhaustive and normative, in both directions**: every code named anywhere in this document MUST have a registry entry (code, kind, subsystem, title, the rule it enforces) and a `docs/errors/EXXXX.md` page, **and** every registry entry MUST cite a rule id that exists in this document. `tools/rule_index.py` MUST fail CI on either violation, in the same pass that checks `[TST-4]`. **A code MUST be defined by exactly one rule.** `tools/rule_index.py` MUST fail CI when two rules name the same code with different titles, in the same pass that enforces rule-ID uniqueness (XXIII.4). It MUST additionally fail CI when a rule defined in the active index has no row in the current revision's change log, and when an attribute, construct or CLI surface named in normative text is absent from Part III §7, `[ATT-2]`, XX.1 or XXIII.3. It MUST also resolve every **section** reference and fail CI on one with no referent, resolving compound notation — `§XIX.4.7`, `§XIX.4 subsection 4.7` and `XIX.4 → 4.7` all denote `### 4.7` inside `## XIX.4` — because a checker that cannot follow a subsection path reports a reference as dangling when it is not, which `0.9.7_Hardened_2` did twice. **These checks, not more review, are what prevent a 0.7 that repeats this.**
...
| **B15** callable parameter-mode mismatch | a callable is supplied where the expected callable contract requires a different borrowed, `mut` or `owned` parameter mode, or where callable metadata would erase that mode | declare the callback parameter with the required `mut` or `owned` mode; preserve the mode-bearing callable type through `Callable[Args, R]`; keep the original callable type across generic forwarding. The `help` MUST print the expected and supplied signatures, not a category |
...
Codes added by 0.9.7_Hardened_3, closing owner decision ODR-016: `E2228` (callable parameter-mode mismatch, shape **B15**, subsystem `typeck`, enforced by `[FN-6a]` and `[LT-11a]`), with `docs/errors/E2228.md` and a `B15` fixture. `E3066` is **reserved and never emitted**: ODR-016's resolution proposed it for this diagnostic, numbering a `typeck` failure into the ownership band, and it is registered here only so that the number is not reused. It sits in `E2000–E2499` because the failure is one of canonical **type identity**, which is that band's subsystem; the `E3000–E3499` band is ownership, borrows, regions and drops, and `[DIA-7a]`'s shape table covers that band only, so `E2228` is classified by `[DIA-12]` instead. `E3065` gains two enforcing rules, `[LT-23]` and `[LT-39]`, and no new code: both describe the multi-region provenance failure it already names.

Codes added by 0.8.5: `E3105` (mutation through a shared borrow of an `UnsafeCell[T]` from outside `unsafe`, `[UNS-10]`/`[UNS-10a]`). Codes added by 0.9.5: `E3065` (a result whose region provenance is not satisfied by the multi-region view it derives from, `[LT-22]`, and, from `0.9.7_Hardened_3`, `[LT-23]` and `[LT-39]`, shape **B14**; `[DIA-19]` states its required registry entry, error page and fixture). 0.9, 0.9.6 and 0.9.7 add no code: each rejection they state is reported by an existing one, and `@latebound` violations are reported by the ordinary region and escape diagnostics. *(registered by `0.9.7_Hardened_2`; `E3105` and `E3065` were named in rules and in `[DIA-7a]`'s shape table from 0.8.5 and 0.9.5 respectively and had no registry paragraph, which `[DIA-6a]` makes a CI failure in both directions.)*
...
| Shape | Trigger | Required primary suggestion |
|---|---|---|
| **N1** unknown name | a path resolves to nothing | the in-scope binding, field, method or item within Damerau–Levenshtein distance ≤ 2 (or ≤ ⅓ of the name's length), named exactly; at most 3, ranked by the N1 ranking algorithm below |
...
#### N1 candidate ranking algorithm

N1 candidate discovery, visibility, and the candidate set are unchanged. Rank
every eligible candidate by this single deterministic ordering before
selecting the existing maximum of three suggestions:

1. **Primary — spelling distance.** Use the existing Damerau–Levenshtein
   distance between the unresolved name and the candidate's visible spelling.
   Preserve the existing eligibility threshold.
2. **Secondary — locality, only when spelling distances tie.** A candidate is
   `SAME_FILE` when its declaration and the unresolved use have the same source
   file identity; otherwise it is `CROSS_FILE`. `SAME_FILE` ranks before
   `CROSS_FILE`. A strictly better spelling distance always ranks first,
   regardless of locality.
3. **Tertiary — same-file proximity.** Among `SAME_FILE` candidates, use the
   existing declaration-proximity calculation unchanged. Source offsets are
   compared only when both spans belong to the same file.
4. **Tertiary — cross-file identity.** Among `CROSS_FILE` candidates, do not
   compute declaration proximity. Compare their canonical qualified
   declaration identities lexicographically by UTF-8 bytes, independent of
   locale. The canonical identity is the symbol's module/type/member identity,
   not an importing alias, import span, or user-facing spelling.
5. **Final fallback.** If all specified keys tie, compare the visible
   suggestion spellings lexicographically by UTF-8 bytes. This only resolves
   an otherwise identical ranking key; it does not replace canonical identity
   as the cross-file ordering key.

The ranking MUST NOT compare source offsets from different files or use
filesystem traversal, import-graph distance, import/source discovery order,
hash-table iteration, operating-system directory order, build/cache order, or
another project-wide proximity metric. The visible spelling remains the text
shown to the programmer; canonical identity is only an ordering key. Apply the
complete ordering before top-three truncation. This rule applies uniformly to
ordinary imports, aliases, inherited members, and prelude candidates; it adds
no diagnostic code or candidate-discovery mechanism.
...
* `[DOC-2]` **The user guide is a 1.0 artefact**, not v1.1. `docs/book/` MUST exist, MUST be the landing page with the specification linked from it as the reference, and every sample in it MUST be compiled and run by `ember test --doc`. Its "getting started" chapter is identical to milestone M0 and is tested against it; it MUST include a "coming from Python" chapter carrying the three-column table of silent and diagnosed differences (`/`, `%`, the `f32` literal default, `if xs:`, `is None`, chained comparison, `xs[-1]`, `let`, `with … as`, multi-statement closures) **and the short list of things that behave exactly as in Python**, because the absence of a row is not reassurance. Its learning path SHOULD introduce values, functions, ownership, borrowing/exclusivity, structs, classes/ARC, containers, interfaces/generics, views, and only then advanced memory, FFI, and unsafe. Compiler-only region vectors, capability identities, storage identities, `CallInfo`, and initialization lattices are not prerequisite source-language vocabulary.
* `[DOC-3]` `ember doc` moves from Phase 8 to **Phase 4**, whose effects and derives it depends on; from that point each phase documents its own surface as it lands. Every view-producing or view-consuming API SHOULD present a consistent generated contract covering ownership, permission, source, provenance, escape, effects, and allocation. Tooling SHOULD derive that presentation from the authoritative signature and contract; it MUST NOT maintain a hand-written semantic duplicate.
...
* `[BUD-5]` **The budget has veto power.** A proposal that adds a language or
  ...
  The proposal MUST also declare public concepts, syntax, public APIs, compiler
  facts, compiler passes, and runtime mechanisms added, plus existing mechanisms
  reused, and classify itself as `SIMPLIFIES`, `NEUTRAL`, or
  `ADDS COMPLEXITY — JUSTIFIED`. The last classification MUST explain why existing
  generic, interface, parameter-mode, metadata, inference, proof, and concrete API
  mechanisms cannot express the requirement safely and clearly.
...
* `Arena`, `FixedArena`, `ScopedArena` (`[ARN-*]`), `unsafe`, raw pointers, `MaybeUninit`, `transmute`, and the core `Zeroable` validity predicate/built-in implementations required by `[ARN-12]`.
...
* Exclusivity: static analysis (§4.8) + runtime checks; `Shared[T]`, `Weak[O]` for the supported counted-owner forms.
...
* Derives: `Copy, Clone, Debug, Display, Eq, Ord, PartialOrd, Hash, Default, Zeroable, Reflect, Error`. This phase owns general derive generation; `[ARN-12]` already requires the core `Zeroable` predicate and built-in implementations in Phase 2.
...
| Conjunctive validity | `[LT-17]`: a multi-region view is usable only while **every** region slot a reachable field needs is still valid |
| Late-bound callable boundary | `@latebound fn(…) -> R` (`[FN-6b]`): each invocation binds fresh regions for the callback's borrowed parameters, and the result must be free of them |
| Callable mode vector | the borrowed / `mut` / `owned` parameter modes a callable type carries as compile-time identity, preserved through `Callable[Args, R]` (`[FN-6a]`) |
| Cycle inspection | `ember inspect --cycle <path>` reports the whole ownership graph; `ember explain --cycle <path> <Class[.field]>` explains one selected class or field in that same root; `[WK-4]`'s leak reporter is the run-time view of the same data |

## XXIII.4 Rule index

Rule ID prefixes and where they are specified: `PHIL` (I.3), `TIER` (I.2), `LEX` (II), `GRM` (III, and V §8 for `[GRM-20]`..`[GRM-23]`, whose productions are stated beside the declaration rules that constrain them), `TYP` (IV), `MOD` `FN` `STR` `ENM` `CLS` `IFC` `STA` `ATT` (V), `EXP` `CTL` `CLO` `PAN` (VI), `OWN` `BRW` `LT` `DRP` `SPN` (VII), `OBJ` `RC` `EXC` `DSP` `WK` `OPT` (VIII), `HEAP` `ARN` `ALC` `UNS` `HND` `CELL` `DSJ` (IX), `EFF` (X), `THR` `PAR` `JOB` (XI), `SOA` `SIMD` `ECS` (XII), `ERR` (XIII), `CT` `RFL` `DRV` (XIV), `STD` (XV), `FFI` `BLD-FFI` (XVI), `GPU` (XVII), `RNG` (IV.2a), `CORO` (VI.5a), `DET` (X.2a), `TCB` (XX.6a), `CMP` `AST` `HIR` `MIR` `MONO` `CG-C` `MNG` `RT` (XIX), `CLI` `MAN` `BLD` `PRF` `TST` `DIA` `FMT` `LNT` `BUD` `CONF` `ABI` `CXX` `IDE` `DOC` `VER` `TOOL` (XX), `BEN` `GATE` (XXI), `RV` (XXII), `CAT` (XXIII.5). `HR` (XVIII), `TXT` (XV.4a), `SEL` (IX.0), `COST` (X.4). **Added by the 0.9 series, all specified in Part XXIV unless noted**, each under the addendum heading named here: `OQ8` (Owner-Decision Resolution Cycle), `IMP` (0.9 Hardened Implementation Contract), `COMP` (Compiler Pipeline Addendum), `GEN-COH` (Generic Coherence Addendum), `CLO-ABI` (Closure ABI Addendum), `COR-ABI` (Coroutine ABI Addendum), `RT` for `[RT-6]`..`[RT-9]` (Runtime Addendum; `[RT-1]`..`[RT-4]` remain in XIX), `LAY` (Layout Contract), `FFI-IMPL` (FFI Implementation Contract), `FFI-CB` (Foreign Callback Safety), `HR-IMPL` (Hot Reload Implementation Contract), `DET-IMPL` (Determinism Addendum), `STD-IMPL` (Standard Library Completeness Contract), `VERIFY` (Compiler Self-Verification), `MIR-REG` (XIX.4) and `HASH` (IV). *(completed by `0.9.7_Hardened_2`; H1 defined these 15 families and declared none of them here, and its own Non-normative heading index says of itself that it is not a registry.)* **Appendix H is provenance and defines nothing**: `tools/rule_index.py` MUST ignore rule ids occurring inside it, exactly as it ignores ids inside a superseded revision's change log, so that a historical record quoting `[TYP-15]` or `[ARN-5]` at the start of a line neither demands a conformance test nor registers a second definition. *(stated by `0.9.7_Hardened_2`; seven such lines exist in Appendix H, every one of them a citation carried over from `0.9.7_Hardened_1`'s front matter.)* Every normative rule MUST have a unique rule ID, a conformance test directory, a diagnostic mapping where it rejects source, and an entry discoverable by the extractor. **Rule-ID uniqueness is a hard invariant**: `tools/rule_index.py` MUST fail CI when the same rule ID is defined more than once in the active index, regardless of section or prefix. A *reference* to an existing rule is not a definition and MUST NOT create a second index entry; historical RFC or provenance prose may mention an existing ID without being indexed. **A superseded ID named only in an earlier revision's change log is likewise not an index entry**: `tools/rule_index.py` MUST ignore rule ids occurring inside a `## Change log — <version>` section for a version other than the current one, so that the record of what a past revision did — `[HOT-1]`..`[HOT-10]`, replaced by Part XVIII in 0.7.1, and the `[CTR-*]`/`[PRV-*]` rules removed in 0.6.2 — neither demands a conformance test nor silently resurrects a deleted rule. A superseded id MUST NOT be reused for a new rule. No new normative rule may be introduced without a test mapping. `tools/rule_index.py` extracts every rule id matching `\[([A-Z][A-Z0-9-]*-[0-9]+[a-z]?)\]` from the specification — the trailing letter admits amendment ids such as `[LEX-11a]`, and the hyphenated class admits `[BLD-FFI-1]` and `[CG-C-3]` — and cross-checks `tests/conformance/`. It fails CI on a duplicate id, on an id with no test directory, and on a code named in a diagnostic shape but absent from the registry in XX §6.
...
Everything between the consolidated-body markers is current normative text of the 0.8.5 lineage,
with the owner-approved 0.9, 0.9.5, 0.9.6 and 0.9.7 replacements applied in place. **Part XXIV, which
follows, is equally normative** (`[IMP-8]`) — it was authored by the 0.9 series rather than inherited,
which is what the markers distinguish and all they distinguish. Appendix B is specification tooling
guidance for the current revision; Appendix H is history and binds nobody.
*(amended by `0.9.7_Hardened_2`: H1's wording said everything after the marker was tooling guidance,
which the reordering would have made false.)*


---

# Part XXIV — 0.9 Series Normative Addenda

This Part carries the normative material the 0.9 series added. It is one contract with Parts
I–XXIII (`[IMP-8]`). It sits after Appendix A because the `CONSOLIDATED_NORMATIVE_BODY` markers
delimit the text inherited from the 0.8.5 lineage — Parts 0–XXIII and Appendix A — and this Part
was authored by the 0.9 series; a language specification also states the language before it states
how to build it. Nothing here depends on where it sits, and `[IMP-8]` forbids position-based
supersession, so relocating this Part changes nothing about what it requires.

`0.9.7_Hardened_2` moved this material from the front of the file. **No sentence of it was
rewritten**, and three things changed and nothing else. Two directional words were corrected, each
marked in place: this Part used to precede Parts 0–XXIII, so two of its sentences said *below* where
the referent is now *above*. Beyond those two words: each heading was pushed down one level, so that
what were top-level headings in `0.9.7_Hardened_1` now nest under this Part instead of standing
beside it; and one new heading — *Reference-compiler model and boundaries* — was added to parent
three sections whose former parent was a provenance record that now sits in Appendix H. Rule
identifiers, rule text, tables and code blocks are byte-identical to Hardened_1.

| Heading inside this Part | Contents | Rules |
|---|---|---|
| Reference-compiler model and boundaries | the reusable facts, preservation and evidence boundaries | — |
| 0.9_Hardened_12 Owner-Decision Resolution Cycle | OQ-8: the ECS is library-owned | `[OQ8-1]`..`[OQ8-7]` |
| 0.9_Hardened_14 Hardening Cycle | ARC-cycle diagnosis, exclusivity hoisting, independent-view composition | `[WK-5]`..`[WK-10]`, `[EXC-8]`..`[EXC-14]`, `[LT-8]`..`[LT-13]`, `[BUD-7]`, `[CLI-16]`, `[CLI-17]`, `[TST-14]`..`[TST-16]`, `[TST-20]`..`[TST-23]` |
| 0.9.5_Hardened_1 Language Revision | inferred multi-region views (MRV-1..MRV-8) | `[LT-14]`..`[LT-43]`, `[BUD-8]`, `[IMP-10]`, `[VERIFY-3]`, `[TST-17]`..`[TST-19]` |
| 0.9 Hardened Implementation Contract | implementation contract | `[IMP-1]`..`[IMP-9]` |
| Compiler Pipeline Addendum · Generic Coherence Addendum | drop-check phase, coherence | `[COMP-1]`, `[GEN-COH-1]`, `[GEN-COH-2]` |
| Closure ABI Addendum · Coroutine ABI Addendum | callable ABI | `[CLO-ABI-1]`, `[CLO-ABI-2]`, `[COR-ABI-1]`, `[COR-ABI-2]` |
| Runtime Addendum | runtime ABI name, RC overflow, atomic ordering, handle generations | `[RT-6]`..`[RT-9]` |
| Layout Contract | canonical layout descriptor | `[LAY-1]` |
| FFI Implementation Contract · Foreign Callback Safety | ABI descriptors, fingerprint, thunk tests, callback identity | `[FFI-IMPL-1]`..`[FFI-IMPL-3]`, `[FFI-CB-1]` |
| Hot Reload Implementation Contract | state machine, epoch publication, transaction failure | `[HR-IMPL-1]`..`[HR-IMPL-3]` |
| Determinism Addendum | FP environment, differential oracle | `[DET-IMPL-1]`, `[DET-IMPL-2]` |
| Standard Library Completeness Contract | API/source correspondence, std conformance | `[STD-IMPL-1]`, `[STD-IMPL-2]` |
| Compiler Self-Verification | MIR verification, pass preservation | `[VERIFY-1]`, `[VERIFY-2]` |
| Implementation Completeness Matrix · Differential Conformance Architecture · Implementation Plan Revision · RageV Final Audit | build-out architecture and plan | — |
| 1.0 Readiness Gate · End-State Principle | what 1.0 requires | — |
| Non-normative heading index | navigation aid, not a registry | — |

---

## Reference-compiler model and boundaries

The three sections below were carried under the *0.9.6_Hardened_1 Simplicity-Consolidation Record*
in `0.9.7_Hardened_1`. That record's narrative — version treatment, amendment classes, source and
authority — is provenance and sits in Appendix H; these three sections are contract and sit here.
`0.9.7_Hardened_2` added this one heading so that they have a parent in this Part. Their text is
unchanged.

### Consolidated reference-compiler model

The reference compiler is organized around reusable facts rather than feature-specific safety
systems:
...
`BorrowCapability` conceptually keeps lifetime provenance separate from concrete storage identity,
and keeps each of access permission, representation kind, ownership relation, acquisition/checking,
unsafe authority, synchronization, validity, and escape constraints on an independent axis. A raw
pointer is a representation kind, not an access permission; a mutable raw pointer combines mutable
permission with raw-pointer representation and an unsafe authority boundary. Escape constraints are
a set/predicate rather than one mutually exclusive enum.

Feature-specific semantics remain distinct applications of those facts. Arena is a region allocator,
not an interior-mutability primitive. `Cell`, `RefCell`, synchronization guards, and `UnsafeCell`
retain their different authority and runtime invariants. ECS remains library-owned. Foreign calls
remain contract-driven. Coroutines do not acquire an escape exemption. Shared machinery never
generalizes H10's dynamic class-exclusivity policy into ordinary borrow checking.

### Preservation boundaries

The consolidation preserves three intentionally different replacement paths:

```text
ordinary assignment:          evaluate new -> drop old live value -> store new
Cell.set:                     evaluate/receive new -> store new -> drop old
MaybeUninit.write/write_at:   evaluate/receive new -> store new -> do not drop prior bytes
```

It also preserves H10's mandatory cycle diagnostics, exact/declared/unknown callable-access model,
field-to-region provenance summaries, conservative opaque-call behavior, summary invalidation,
FFI exception policy, and runtime erasure of compiler-only region/capability metadata.

### Evidence boundary

This file specifies the target architecture. It does not establish that the current compiler uses the
canonical facts, implements 0.9.5 multi-region views, or passes the target's conformance conditions.
Existing conformance-backed behavior, specified-but-unimplemented behavior, and current divergences
remain separate evidence classes. The current compiler is not an oracle for rules it has not
implemented; differences are classified against this specification and owner decisions.
...
* `[WK-9]` **Cycle explanation command.** `ember explain --cycle <path> <Class[.field]>` MUST use
  `[CLI-18]` to select its analysis root, then print the statically visible ownership path(s) that
  make the selected class or field cycle-prone. A selected field MUST identify the exact field
  declaration and the complete ownership path containing it. When the cycle crosses a generic
  container, the instantiated ownership type MUST be shown. When no static cycle can be established,
  the command MUST say that the edge is dynamically cycle-capable rather than pretending it has
  proved a runtime cycle.
...
* `[LT-8]` **Canonical `with_views` helpers.** `std.borrow` MUST provide canonical,
  ...
  type relationships MUST remain these. Their callback boundary is explicitly `@latebound` under
  `[FN-6b]` and uses `[LT-7]` late-bound regions,
...
* `[LT-8a]` **Callbacks retain callable modes and boundary kind.** The callback parameter of each
  `with_views` helper MUST use `[FN-6a]`/`[FN-6b]`'s canonical callable representation. Shared helpers require corresponding
  ...
  or the `@latebound` fact merely because the callable passes through `Callable[Args, R]`.
...
* `[LT-11a]` The compiler MUST reject a mutable `with_views*_mut` callback whose callable type omits
  ...
  implicit ownership mode. That diagnostic is `E2228`, shape **B15** — the same code `[FN-6a]`
  names, because it is the same failure reached through a helper rather than through `Callable`
  (owner decision ODR-016).
...
* `[TST-20]` Conformance MUST cover:
  ...
  6. rejection of a `with_views*_mut` result that would contain an invocation-local view region.
...
* `[TST-21]` Conformance MUST additionally cover:
  ...
  6. generic forwarding of a mode-bearing callable without loss of its borrow or mutability
     semantics; and
  7. late-bound callback-region freshness, nested-boundary separation, and rejection of callback
     results or publication paths that retain an invocation-local view region.
...
* `[TST-23]` Conformance MUST cover the Arena initialization boundary:

  1. `MaybeUninit.uninit()` followed by `write`, including the returned `ref mut T`;
  2. `alloc_uninit[T](count)` followed by per-element `write_at`;
  3. rejection of every safe read or safe full-span conversion before initialization;
  4. both `assume_init` forms requiring an `unsafe` context and transferring ownership exactly once;
  5. rejection of a safe partial-span conversion, while an unsafe assertion retains `[ARN-9]`'s
     full-initialization precondition;
  6. destruction behavior for `T: Drop` after transition to ordinary initialized ownership;
  7. size and alignment equality between `MaybeUninit[T]` and `T`;
  8. rejection of source use after consuming `assume_init`;
  9. zeroed bytes not being treated as initialized `T` unless `T: Zeroable`;
  10. `alloc_array`'s `Zeroable`-before-`Default` precedence, `E2040` fallback,
      `!needs_drop` rejection, successful `Default` construction, v1 process termination through
      `[PAN-1]` on a `Default` panic, no continuation or compiler-invented unwinding after that panic,
      and no observation of post-panic Arena state. A future recoverable construction API is tested
      against its own explicitly specified rollback contract.

  The supplied mnemonic `[TST-ARN-MU]` is normalized to this numeric rule ID. No test may imply
  that an unsafe `assume_init` call proves its own precondition; the caller remains responsible.
...
* `[CLI-17]` `ember inspect --cycle <path>` MUST resolve `<path>` as the canonical cycle-analysis
  root defined by `[CLI-18]` and report the same ownership graph used by `[WK-5]`, including
  strong/weak/unknown edge classification and the shortest cycle when one is statically visible.

* `[CLI-18]` **Cycle explanation analysis root.** `ember inspect --cycle <path>` and `ember explain
  --cycle <path> <Class[.field]>` MUST construct their analysis universe from exactly the same
  `<path>` resolution: either a package directory containing `ember.toml`, using that package's
  manifest and ordinary module/import closure, or one standalone `.em` file under `[CLI-4]`'s
  single-file package synthesis. A module file inside a package is still interpreted only by the
  standalone-file rule; it MUST NOT acquire a third, partial-package interpretation. The commands
  MUST NOT scan cwd, consult a stale or unrelated build, infer a prior root, or select an arbitrary
  package. `<Class[.field]>` resolves only in the resulting analysis universe. An invalid root MUST
  fail before ownership analysis and MUST NOT fall back to another root. `::` qualifies a module/type
  name and `.` selects its field; an unknown class, missing field, or ambiguous unqualified class
  uses the ordinary name/member-resolution diagnostics, with ambiguity naming every qualified
  candidate.
...
* `[LT-23]` **`@borrows` remains useful but does not collapse regions.** Existing `@borrows(p, …)`
  ...
  is independently meaningful and MUST NOT make an unsafe field escape legal. A `@borrows`
  contract that would collapse a multi-region return is `E3065`, shape **B14** — the same
  provenance failure `[LT-22]` names, reported under the same code because it is the same
  observable class (owner decision ODR-016).
...
The canonical multi-region wording of `[TYP-15]` and `[TYP-15a]` appears in Part IV §4 above. *(“below” in `0.9.7_Hardened_1`, where this section preceded the body.)*
...
* `[LT-39]` **Unknown field selection is conservative.** Pattern matching, reflection, indexing through a
  ...
  branch, the narrower slot set MAY be used inside that branch. An operation that would evade a
  region constraint this way is `E3065`, shape **B14** (owner decision ODR-016); the message MAY
  specialise to name the unknown selection while the code and shape stay fixed.
...
* `[TST-19]` Callable-access tests MUST cover: a function accessing one field of a multi-region view;
  ...
  the rule can reject, a reject case. The 0.9.6 consolidation additionally requires equivalent safety
  decisions for semantically equivalent capabilities produced as `Span`, `MutSpan`, `ref`, `ref mut`,
  a view field, an Arena-backed result, a `RefCell` guard, or a foreign view; tests MUST keep provenance,
  storage identity, access permission, representation kind, runtime checking, and synchronization
  distinct rather than asserting that those public abstractions are semantically identical.
...
* `[VERIFY-3]` The MIR verifier MUST verify that every multi-region view projection carries the
  ...
  region bookkeeping. Under the 0.9.6 consolidated representation it additionally verifies that lifetime
  provenance is not substituted for storage identity or disjointness, that raw-pointer representation is
  not encoded as an access permission, and that any shared replacement lowering declares and preserves
  the ordering required for ordinary assignment, `Cell.set`, or `MaybeUninit.write`/`write_at`.
...
This section is normative. It supplements the existing Parts I–XXIII above. *(“below” in `0.9.7_Hardened_1`, where this section preceded them.)*
...
### [IMP-7] Compiler pass and canonical-fact contracts
...
The 0.9.6 reference compiler MUST exchange shared semantic facts rather than re-solving the same
relationship independently in each feature. The canonical set is `TypeIdentity`, `BorrowCapability`,
`OwnershipGraph`, `AccessContract`, `InitializationState`, `LayoutDescriptor`, and `EffectSet`.
Their concrete Rust representation is implementation-defined, but their semantic boundaries are not.

Each semantic invariant MUST have one authoritative representation. Derived indexes, caches,
summaries, serialized metadata, and backend-specific forms MAY exist only when they are mechanically
derived from that authority, checked for consistency, and invalidated whenever an input authority
changes. A stale derived fact MUST NOT be used to accept a program. A call-level aggregate such as
`CallInfo { access_contract, effect_set, provenance, retention }` is therefore permitted only as a
derived cache: it MUST NOT replace, redefine, or become independently authoritative over any member.

For explanation and implementation organization, the facts MAY be grouped as provenance, storage,
authority, validity, initialization, and effects. Grouping MUST NOT collapse independent facts.
In particular, `AccessContract` and `EffectSet` remain separate; provenance and storage identity remain
separate; and `InitializationState` retains `Uninit | Maybe | Init` plus field, element, range, and
other refinements required by dataflow. Unknown initialization is `Maybe` or not proven initialized;
it is never silently treated as definitely uninitialized.

A `BorrowCapability` conceptually contains:
...
projection_path
region
access_permission             # shared | mut
reference_kind                # reference | raw_pointer | value | view | handle | runtime_guard
ownership_relation            # owned | borrowed | observing
acquisition_or_check          # static | runtime_checked | lock_acquired
safety_authority              # safe | unsafe
synchronization_domain
validity_interval
escape_constraints            # a set/predicate, not one exclusive enum
```

Lifetime provenance and concrete storage identity MUST remain separate. A static provenance root need
not have a local source place; distinct Arena allocations may share one Arena region while retaining
distinct storage identities. Access permission, representation kind, ownership, checking mechanism,
unsafe authority, and synchronization MUST remain orthogonal: `raw_pointer` is a reference kind, not
an access permission, and `unsafe` is an authority boundary, not a proof algorithm. Region equality or
inequality alone MUST NOT establish storage overlap, disjointness, or `noalias`.

`synchronization_domain` distinguishes `none`, `thread_confined`, and `synchronized` without changing
the ownership relation. `validity_interval` records where use is permitted; it MUST NOT be substituted
for ownership or storage identity. `escape_constraints` can simultaneously express such existing
questions as return, storage, capture, FFI publication, statement/block survival, and crossing `yield`.
Storage identity plus projection may establish disjointness only through an existing proof such as
distinct allocation identities or non-overlapping Arena ranges, disjoint fields or SoA columns,
`split_at`-style structure, or a proof-carrying disjointness API.

Feature modules consume these facts while retaining their authoritative semantics. Arena remains a
region allocator; `Cell`, `RefCell`, synchronization guards, and `UnsafeCell` retain different
invariants; ECS remains library-owned; FFI remains contract-driven; coroutine suspension does not
extend a forbidden borrow; and H10's dynamic class-exclusivity mechanism MUST NOT turn an ordinary
borrow conflict into a runtime-checked acceptance.

Across every domain, absence of proof grants no capability. Unknown borrow overlap supplies no
disjointness or `noalias` fact; unknown callable access covers all relevant fields; unknown foreign
retention covers all relevant regions; a soundly bounded dynamic target uses the union of its targets
and an unbounded target is conservative; and an unsafe validity claim supplies no automatic proof.
This least-permissive policy MUST preserve compile-time safety guarantees rather than replacing them
with an unrequested runtime best effort.

Before adding a public type, interface, syntax form, annotation, or feature-specific safety subsystem,
the design and implementation MUST determine whether an existing generic parameter, interface,
parameter mode, concrete API, compiler fact, or existing explicit proof path expresses the requirement.
Inference and existing metadata MUST be attempted before new source syntax. A new abstraction is
permitted only when the existing mechanisms cannot express the requirement without loss of safety,
clarity, ABI guarantees, or required performance. The reference compiler MUST NOT create a parallel
safety authority merely because two feature paths currently use different code.

These canonical fields are compiler facts, not a new public type or ordinary-diagnostic vocabulary.
The compiler MUST NOT expose a public helper merely because internal analysis paths differ. Normal
diagnostics name source values, places, and semantic causes; existing expert inspection tooling MAY
show region, storage, projection, capability, access-summary, and MIR details.
...
The normative specification, implementation ledger, owner-decision ledger, and conformance evidence
are separate artifacts with separate jobs. None may silently override another: the specification
defines required behavior, the implementation ledger records current support, the owner ledger records
authority for changes, and executable conformance records evidence. An implementation document SHOULD
map an existing rule to architecture rather than restating that rule as a second semantic contract.
...
initial MIR lowering
  ↓
callable access/provenance summary computation
  ↓
...
monomorphisation
  ↓
target-independent MIR optimisation
  ↓
effect analysis under the contract profile
  ↓
MIR verification
  ↓
backend lowering
...
Initial MIR is the analysis substrate; verified MIR is the code-generation boundary. Summary
derivation and interprocedural analysis MAY iterate to a fixed point or be interleaved where recursion,
generic substitution, or dynamic target sets require it, but an unverified summary MUST NOT be consumed
as trusted interface metadata. The verifier runs after every safety-critical MIR transformation as
required by `[IMP-7]`.
...
* `IMP-10` and `VERIFY-3` — multi-region additions carried under the 0.9.5_Hardened_1 heading.
* `MIR-REG-1` (XIX.4) and `HASH-1`..`HASH-4` (IV) — defined in the consolidated body, not here.
* `OQ8-1` through `OQ8-7` are also declared in XXIII.4, which is the registry this index is not.
...
# Appendix B — 0.9.7_Hardened_1 Consistency Checklist

The following checks are mandatory CI checks for the consolidated specification itself:

1. Exactly one current version declaration exists and names `0.9.7_Hardened_1`.
2. The immediate predecessor is frozen `0.9.6_Hardened_6`, which remains byte-for-byte unchanged.
3. `[MOD-6]` and `[MOD-6a]` name the supported `0.9.7`, `0.9.6`, `0.9.5`, `0.9`, and 0.8.x source-language contracts explicitly; `0.9` does not silently enable later semantics, H6 changes H5 only through the owner-approved ODR-014 completion, and H1 changes H6 only through the owner-approved `@latebound` callable-boundary revision.
...
28. `[ARN-2]`, `[ARN-3]`, and `[ARN-8]`–`[ARN-13]` agree on one deterministic, drop-safe Arena initialization contract and one canonical `MaybeUninit` transition API.
29. `[TST-23]` covers safe writes, unsafe assertions, full-span initialization, ownership transfer, layout, zero validity, capability precedence, and the owner-approved `[ARN-10]` abort-only clarification without inventing unwinding or observable post-panic state.
30. `[IMP-7]`, the compiler-pipeline addendum, Part XIX, `[MIR-REG-1]`, and `[VERIFY-3]` agree that initial MIR precedes initialization/ownership/borrow/region analyses and verified MIR is the code-generation boundary.
31. The canonical capability model separates lifetime provenance from storage identity and keeps permission, representation, ownership, checking, unsafe authority, synchronization, validity, and escape constraints orthogonal.
32. A raw pointer is represented by `reference_kind = raw_pointer`; mutable raw access uses mutable permission and does not introduce a `raw` permission value.
33. Exact and trusted declared callable summaries come only from visible/verified MIR or an audited unsafe/FFI contract; unknown access is conservative across every contractually valid static, separate-compilation, dynamic, and hot-reload target.
34. Field-access and field-to-region summaries invalidate affected callers, interface hashes, generic artifacts, and caches together; stale metadata is a hard compiler failure and none of it enters runtime ABI or reload schemas.
35. `[FN-6b]`, `[LT-7]`, `[LT-8]`, `[LT-8a]`, `[LT-10]`, `[TST-20]`, and `[TST-21]` agree that `@latebound` is a callable-type boundary, generates fresh invocation-local regions, preserves the existing callable modes, forbids result/publication escape through ordinary region checking, and is neither named lifetime syntax nor a `std.borrow` compiler special case.
35. Ordinary assignment, `Cell.set`, and `MaybeUninit.write`/`write_at` retain their three distinct replacement/destruction orderings in the normative rules, MIR lowering, verifier obligations, and generated-code tests.
36. H10's cycle diagnostics, class-only dynamic exclusivity, interior-mutability distinctions, ECS library boundary, FFI exception/retention contracts, coroutine restrictions, and runtime-erasure guarantees are not weakened by shared compiler machinery.
37. The received Revision 5 RFC hash, owner-approved `[ARN-10]` wording, ADR-030, HC-096-01, migration intake, and H1 audit record form one traceable decision chain.
38. No RFC architecture label has become a parallel normative rule family; the alternate rule-index audit has no duplicate definition and zero genuine dangling normative references. Its raw inherited `IDE-2`/`IDE-5`/`IDE-10` reservations and historical `HOT-10` citation remain classified by ERR-042 rather than misrepresented as rules. Every unimplemented requirement remains identified as `SPECIFIED` rather than evidence-backed.
39. The preserved ODR-011 ruling, ADR-031, HC-096-02, `[ARN-5]`–`[ARN-5g]`,
    `[TST-24]`, and the `std.collections` table form one traceable contract:
    fixed capacity, one Arena allocation, empty construction, `CapacityError.Full`,
    no prelude exports, named iterator views using `Iterator[Item = ...]`,
    `!needs_drop` contents, ordinary borrowing, no hidden cursor mutation, and
    the specified deterministic-iteration boundaries.
40. The owner-approved ODR-012 ruling, ADR-032, HC-096-03, and `[HASH-1]`–`[HASH-4]`
    form one traceable contract: `Hash` remains in the prelude, `Hasher` and
    `DefaultHasher` do not enter it, `finish` consumes the move-only context,
    equal values hash equally, supplied spans do not escape, safe Map APIs expose
    no mutable key, and no concrete mixing algorithm is frozen.
41. ODR-013/ERR-054 are closed by the owner-selected generic
    `fn hash[H: Hasher](self, mut h: H)` contract. `DefaultHasher implements Hasher`,
    ordinary inference and monomorphization apply, and no bare-interface or
    compiler-only dynamic conversion is inferred.
42. The received post-H4 Revision 5 RFC remains byte-for-byte preserved at its recorded SHA-256,
    ADR-034 records the owner's approval and later H5-materialization clarification, and H4 remains
    byte-for-byte unchanged.
43. One authoritative representation exists per semantic invariant; all indexes, caches, summaries,
    serialization forms, backend forms, and `CallInfo`-style aggregates are derived and invalidated.
44. `AccessContract`, `EffectSet`, initialization, provenance, retention, storage identity, access,
    representation, ownership, checking, unsafe authority, synchronization, validity, and escape are
    not collapsed by implementation grouping.
45. Initialization retains `Uninit | Maybe | Init` plus required refinements, and absence of proof
    grants no capability in every unknown-boundary domain.
46. No `Len`/`Capacity` interface, duplicate collection abstraction, parallel safety system,
    automatic dynamic-borrow fallback, or runtime capability metadata is introduced by H5.
47. New feature work documents public/compiler/runtime complexity and reuses inference, generics,
    interfaces, parameter modes, concrete APIs, metadata, and existing proof paths before adding a
    programmer-visible concept.
48. Primary diagnostics describe source semantics; compiler bookkeeping is secondary expert evidence.
    View API documentation is generated from authoritative signatures/contracts where practical and
    never becomes a hand-maintained semantic duplicate.
49. Specification, implementation, owner-decision, and conformance ledgers remain separate, and no
    implementation status is inferred merely from H6's requirements.
50. The H4-to-H5 classified diff contains no source-language semantic, accepted-program,
    rejected-program, observable-behavior, ABI, safety, effect, reload, or diagnostic-identity change.
51. `[SPN-4]`–`[SPN-10]` define exactly four named public, non-prelude `@view` iterator/chunk types
    under `std.collections`, use the existing associated-type `Iterator` interface, and introduce no
    opaque-return or second ownership/lifetime mechanism.
52. Shared and mutable iteration/chunking preserve ordinary reborrow, NLL, provenance, storage-
    identity, and `[BRW-5]` disjointness semantics; zero-sized chunks panic through the existing
    failure mechanism in every profile.
53. `Span.as_ptr` and `MutSpan.as_ptr` return canonical Ember `*T`,
    `MutSpan.as_mut_ptr` returns `*mut T`, extraction is safe, and pointer use and lifetime validity
    remain governed by the existing unsafe rules without retaining or extending source storage.
54. The preserved ODR-014 ruling, ADR-035, HC-096-06, `[SPN-4]`–`[SPN-10]`, and `[TST-25]` form one
    traceable contract, while implementation and conformance remain unclaimed until executable
    evidence exists.
...
# Appendix H — 0.9 Series Provenance, Audit and Evidence Record

**This appendix is history, not contract.** It records how the 0.9 series reached its present
state: the banner notices each revision carried, the six 0.9.6 completion records, the hardening
cycle log, every audit record, and every Evidence Status statement. Nothing here defines a rule,
and nothing here may be cited as authority for one — Parts I–XXIV are the contract (`[IMP-8]`).
**Read every *above* and *below* in this appendix against `0.9.7_Hardened_1`'s layout, not this
file's.** These records were written when this material stood at the front, so phrases such as *the
body below*, *the current addenda above* and *incorporated below* point at text that now precedes
them. The wording is preserved unchanged because altering a historical record to suit a later
file's layout is how a record stops being one; four such phrases occur, all in the 0.9.6_Hardened_6
consolidation record and the consolidated-body header.

XXIII.4 requires `tools/rule_index.py` to ignore rule ids appearing in this appendix, so that the
seven lines here that *open* with a citation cannot be mistaken for definitions. Those seven cite
ARN-5, ARN-10, LT-4a, LT-8, LT-8a, LT-14 and TYP-15, each of which is defined once, in the body.

`0.9.7_Hardened_2` moved this material out of the front of the file, where it occupied 29% of the
document ahead of *How to use this document*, and rewrote none of it. What was a change log's job
is now done by a change log: see **Change log — 0.9**, **— 0.9.5**, **— 0.9.6**, **— 0.9.7_Hardened_1**
and **— 0.9.7_Hardened_2** in Part 0.

---

**Version:** 0.9.7_Hardened_1
...
**Immediate predecessor:** 0.9.6_Hardened_6
...
The owner-approved `@latebound` callable-type boundary changes which callback-return and escape
programs are accepted, so this is the additive **0.9.7 language revision** and the hardening counter
resets to 1. `0.9.6_Hardened_6` remains its immutable immediate predecessor. `@latebound` adds one
reusable callable-boundary contract while keeping region variables compiler-internal: it creates no
named-lifetime language, runtime metadata, ABI field, reload-schema field, compiler intrinsic, or
alternative ownership system. No implementation or conformance result is asserted merely because
this frozen target specifies one.

> **Authority boundary:** This frozen revision records the owner-selected 0.9.7 development target, but
...
> **LATE-BOUND CALLBACK BOUNDARY COMPLETED:** on 2026-09-14 the owner resolved ODR-015 by selecting
> `@latebound` as a callable-type modifier. H6 remains immutable. A `@latebound fn(...) -> R` boundary
> supplies fresh invocation-local regions for its borrowed/view parameters without exposing region names
> in source. The modifier is part of compile-time callable identity, generic substitution, interface
> compatibility, call checking, and incremental invalidation, but is absent from runtime values, ABI
> bytes, reload schemas, and source-visible lifetime identities. `std.borrow.with_views*` remains an
> ordinary library API; no compiler recognition of those names is permitted.
...
> **ARENA INITIALIZATION CONTRACT COMPLETED:** on 2026-09-12 the owner resolved ODR-009 by
> defining deterministic `Zeroable`/`Default` selection, the `!needs_drop` boundary, transactional
> construction, `MaybeUninit[T]` layout and destruction, canonical safe-write and unsafe
> `assume_init` APIs, uninitialized Arena allocation, and Phase 2 availability. H9 remains frozen;
> this H10 preserves its existing Arena rule IDs and assigns the new clauses unused continuation
> IDs `[ARN-8]`–`[ARN-13]` plus numeric conformance rule `[TST-23]`.

> **SIMPLICITY CONSOLIDATION APPROVED:** on 2026-09-13 the owner approved Revision 5 of the
> simplicity-consolidation RFC as the design basis for `0.9.6_Hardened_1`, then explicitly approved
> the final `[ARN-10]` clarification. H10 remains immutable. This H1 revises existing implementation
> contracts in place instead of introducing a parallel rule family, and it does not claim that the
> corresponding compiler refactor or conformance evidence already exists.

> **ARENA-BACKED CONTAINER CONTRACT COMPLETED:** after H1 exposed that `[ARN-5]` named
> `ArenaArray` and `ArenaMap` without an executable API, the owner resolved ODR-011. H2 keeps H1
> frozen and defines fixed-capacity, single-allocation Arena-backed view containers, recoverable
> `CapacityError.Full` exhaustion, drop-free element constraints, ordinary borrowing, named
> Arena-backed iterator view types, deterministic iteration requirements, and the complete
> `[TST-24]` matrix. No collection type is added to the prelude.

> **HASHING CONTRACT COMPLETED:** after H2 made `K: Eq + Hash` load-bearing for
> `ArenaMap`, an audit found that the inherited `Hash.hash(self, mut h: Hasher)` signature had no
> definition for `Hasher`, no public construction/finalization boundary, and no explicit coherence
> rule. The owner resolved ODR-012. H3 keeps H2 frozen, defines `Hash`, `Hasher`, and
> `DefaultHasher` under `std.collections`, preserves `Hash` as a prelude name under `[MOD-5]`, keeps
> the other two names out of the prelude, requires equal values to hash equally, and leaves the
> concrete mixing algorithm implementation-defined. Safe Map APIs never expose `ref mut K`.

> **HASH CALLABLE-TYPE BOUNDARY COMPLETED:** the owner resolved ODR-013 by selecting the statically
> generic `fn hash[H: Hasher](self, mut h: H)` signature. A concrete hasher supplies `H`, normal
> generic inference and monomorphization apply, and `Hash.hash` introduces no mandatory dynamic
> dispatch or compiler-only interface coercion. `DefaultHasher` is a move-only concrete type
> implementing `Hasher`. An intentionally dynamic `dyn Hasher` API requires a separate explicit
> contract.

> **POST-H4 SIMPLICITY CONSOLIDATION ADOPTED:** on 2026-09-13 the owner approved Revision 5 of the
> second 0.9.6 Simplicity Consolidation RFC and clarified that approval was intended to produce the
> next hardened target. H4 remains immutable. This H5 materializes the RFC's architecture/process
> contract: one authority per semantic invariant, derived-only caches and aggregates, conservative
> treatment of unknown facts, reuse and inference before new public concepts, semantic diagnostics,
> explicit complexity accounting, generated view-contract documentation where possible, and strict
> separation of specification, implementation, owner-decision, and conformance evidence. It preserves
> every H4 semantic distinction and introduces no new public abstraction, runtime metadata, accepted
> program, or rejected program.

> **SPAN API CONTRACT COMPLETED:** after H5's implementation audit found that the inherited Span
> surface named `iter`, `iter_mut`, `chunks`, `chunks_mut`, and `as_ptr` without complete public
> types, receiver modes, zero-size behavior, or raw-pointer boundaries, the owner resolved ODR-014.
> H5 remains immutable. This H6 adds `[SPN-4]`–`[SPN-10]` and `[TST-25]`, uses named `@view`
> iterators under `std.collections`, preserves ordinary shared/mutable reborrowing and NLL, defines
> all-profile panic for zero-sized chunks, and makes raw-pointer extraction safe while leaving raw
> pointer use under the existing unsafe contract. The owner's `*const T` notation is normalized to
> Ember's existing canonical shared raw-pointer spelling `*T`; the selected semantics are unchanged.


---

## 0.9.6_Hardened_6 Span API Completion Record

**Version treatment:** owner-approved standard-library API and safety-boundary completion of a
surface already advertised by the inherited Span contract. **Amendment class:** OWNER-APPROVED
SEMANTIC COMPLETION. H5 remains the immutable predecessor. This cut changes only the previously
unresolved identity and behavior of the named iterator, chunk, and raw-pointer operations; it does
not introduce another ownership, lifetime, iterator, opaque-return, raw-pointer, or unsafe model.

**Source authority.** The owner-supplied ODR-014 ruling is preserved byte-for-byte at
`docs/spec-source/as-received/ODR-014_Span_MutSpan_API_completion.md` (SHA-256
`0B2BC1BCB6450F266EC9EC264035C2F6C4A469574591CD8C9BDC8B9421A7058F`). Its `*const T` spelling is
transport notation from a C/Rust-style type vocabulary; current Ember has always spelled a shared
raw pointer `*T` and a mutable raw pointer `*mut T`. H6 applies that existing spelling without
altering the owner's selected const/mutable authority boundary.

**Resolved contract.** `std.collections` publicly exports the non-prelude named `@view` types
`SpanIter`, `MutSpanIter`, `SpanChunks`, and `MutSpanChunks`. Shared iteration yields `ref T`;
mutable iteration yields `ref mut T`; shared chunks yield `Span[T]`; mutable chunks yield disjoint
`MutSpan[T]`. Mutable iterator/chunk construction reborrows rather than consumes its `MutSpan`, and
ordinary NLL governs later reuse. A zero chunk size panics through the existing failure path in every
profile. `as_ptr` and `as_mut_ptr` are safe extraction operations returning `*T` and `*mut T`;
dereference, access, arithmetic, reference construction, and validity obligations remain governed by
the existing unsafe rules. No raw pointer extends or retains source storage lifetime.

**Evidence boundary.** H6 specifies the target and `[TST-25]` matrix; it does not claim that the
compiler, runtime, standard library, or conformance suite already implements that matrix. The
adopted repository specification remains `docs/spec-source/ember-spec.md`
(`0.8.5_Hardened_1`) until the explicit adoption gates and owner action install a successor.

## 0.9.6_Hardened_5 Simplicity-Consolidation Adoption Record

**Version treatment:** owner-approved architecture, tooling, documentation, and development-contract
hardening. **Amendment class:** REFERENCE-IMPLEMENTATION, TOOLCHAIN, DOCUMENTATION, and PROCESS;
SEMANTIC CHANGE: NONE. H4 remains the immutable predecessor. The received RFC is preserved
byte-for-byte as
`docs/spec-source/as-received/Ember_0.9.6_Simplicity_RFC_Revision_5.md` (SHA-256
`0AB0F9C2F4F52E10492A0CD94897BBD9C8B47C08D86E7246EA2FA29F18007525`).

**Owner clarification.** The RFC's original statements that adoption alone would not create H5 were
procedural safeguards against an unreviewed document silently becoming normative. The owner has now
explicitly approved materialization as H5. That later ruling supersedes only the RFC's no-version-bump
statements; it does not convert the RFC into an implementation-status ledger or authorize any source
semantic change.

**Adopted contract.** Ember keeps the smallest practical programmer-visible vocabulary while allowing
sophisticated compiler internals. Each semantic invariant has one authoritative representation.
Indexes, caches, summaries, and `CallInfo`-style aggregates are derived, invalidated with their source
facts, and never independently trusted. Absence of proof grants no capability. Existing generic,
interface, parameter-mode, metadata, and concrete APIs are considered before a new public abstraction;
inference is attempted before new annotations. Initialization retains `Uninit | Maybe | Init` and all
required finer-grained facts. `AccessContract`, `EffectSet`, provenance, retention, storage identity,
permission, representation, ownership, checking mechanism, unsafe authority, synchronization,
validity, escape constraints, and initialization remain independent where H4 requires them.

**Surface result.** This hardening adds no `Len` or `Capacity` interface, no parallel safety system,
no new runtime capability representation, and no automatic dynamic-borrow fallback. Existing
collections reuse existing interfaces only when their semantics match. Programmer documentation and
primary diagnostics explain source-level ownership, permission, provenance, escape, effects, and
allocation rather than exposing compiler bookkeeping as a second language vocabulary.

**Evidence boundary.** H5 is a target contract, not evidence that the compiler has completed the
consolidation. Existing behavior, specified-but-unimplemented behavior, and conformance evidence remain
separate ledgers. Adoption of H5 as `docs/spec-source/ember-spec.md` still requires all recorded gates.

## 0.9.6_Hardened_4 Static Hasher Boundary Completion Record

**Version treatment:** owner-approved hardening of the public hashing protocol completed in H3.
**Amendment class:** OWNER-APPROVED SEMANTIC COMPLETION. H3 remains the immutable predecessor. This
cut changes only the previously unresolved callable/interface representation of `Hash.hash`; it does
not change the remaining hashing, equality, collection, ownership, borrowing, ABI, effect, reload,
or determinism contracts.

**Resolved contract.** `Hash.hash` is generic over `H: Hasher`. The concrete argument type is
inferred and normally monomorphized through the existing generic/interface machinery. Bare `Hasher`
is not treated as a concrete parameter type, and no implicit `dyn` conversion is introduced.
`DefaultHasher` implements `Hasher`. Runtime-polymorphic hashing remains available only to a future
API that explicitly specifies a legal `dyn Hasher` representation.

**Evidence boundary.** This specification completion does not claim compiler or conformance support
for the public hashing surface. Executable status remains external repository evidence.

## 0.9.6_Hardened_3 Hashing Contract Completion Record

**Version treatment:** owner-approved standard-library API completion of a protocol already required
by the existing Map contracts. **Amendment class:** OWNER-APPROVED SEMANTIC COMPLETION. H2 remains
the immutable predecessor. This cut does not change existing equality, collection ownership,
borrowing, Arena allocation, failure, effect, FFI, ABI, reload, or determinism semantics.

**Resolved contract.** `std.collections` publicly exports the `Hash` and `Hasher` interfaces and the
concrete `DefaultHasher`. Existing `[MOD-5]` keeps `Hash` in the prelude; neither `Hasher` nor
`DefaultHasher` is a prelude name. `Hasher` is stateful and move-only, `finish` consumes it, supplied
byte spans cannot be retained, and `Hash` implementations must satisfy `Eq` coherence. `Map` and
`ArenaMap` require `K: Eq + Hash`; their safe APIs expose keys only through shared references. The
default mixing algorithm is deliberately not a source-compatibility contract.

**Evidence boundary.** This specification completion does not claim that custom-key hashing or the
complete `DefaultHasher` implementation is present. Executable status remains external repository
evidence.

## 0.9.6_Hardened_2 Arena-Backed Container Completion Record

**Version treatment:** owner-approved hardening completion of an existing named standard-library
surface. **Amendment class:** OWNER-APPROVED SEMANTIC COMPLETION. H1 remains the immutable
predecessor. This cut does not change any existing ownership, borrowing, Arena allocation,
initialization, failure, effect, FFI, ABI, reload, or determinism rule outside the completed
`[ARN-5]` family.

**Resolved contract.** `std.collections` publicly owns `ArenaArray`, `ArenaMap`,
`CapacityError`, `ArenaArrayIter`, `ArenaArrayIterMut`, and `ArenaMapIter`; none is a prelude
name. Both containers are empty after `with_capacity`, allocate backing storage once from the
supplying Arena, never grow or reallocate, and retain that Arena's region. Capacity exhaustion is
`Err(CapacityError.Full)`, not panic. The named iterator views implement the existing
associated-type `Iterator[Item = ...]` interface; no opaque-return or second iterator mechanism is
introduced.

**Evidence boundary.** This specification completion does not claim that the compiler, runtime, or
standard library implements `ARN-COLL-1`. Executable status remains external repository evidence.

## 0.9.6_Hardened_1 Simplicity-Consolidation Record

**Version treatment:** owner-selected architecture line. **Amendment classes:** IMPLEMENTATION
INVARIANT and SEMANTICALLY NEUTRAL CLARIFICATION, plus an OWNER-APPROVED SEMANTIC CHANGE limited to
accepting the new `0.9.6` version selector. Apart from that selector, H1 preserves H10's
source-language accepted/rejected program sets,
observable behavior, ownership and destruction, borrowing and regions, initialization validity,
unsafe obligations, FFI and ABI behavior, coroutine restrictions, determinism, and stable diagnostic
identities.

**Source and authority.** The received design basis is preserved byte-for-byte as
`docs/spec-source/as-received/Ember_0.9.5_Simplicity_Consolidation_RFC_Revision_5.md` (SHA-256
`593E86A61738B32DCFAAB14875C12EA3A7BBF1A16BAD5286260EE1BAFD803923`). The owner's subsequent
message is the authority for the final `[ARN-10]` wording. This record is navigation and rationale;
the amended existing rules in this specification are the executable contract.
...
| 0.9.5_Hardened_10 | Owner-approved hardening clarification | defined Arena bulk initialization, `Zeroable`, `MaybeUninit`, canonical initialization APIs, rollback, and Phase 2 availability; added `[ARN-8]`–`[ARN-13]` and `[TST-23]` |
| 0.9.6_Hardened_1 | **Owner-selected architecture-line revision** | consolidated reference-compiler semantic facts and MIR boundaries without weakening H10 behavior; clarified `[ARN-10]` for v1 abort-only panic; accepted `0.9.6` as an additive version selector |
| 0.9.6_Hardened_2 | Owner-approved semantic completion | closed ODR-011 with fixed-capacity Arena-backed collections, `CapacityError.Full`, named iterator view types, exact provenance/drop/failure behavior, and `[TST-24]` |
| 0.9.6_Hardened_3 | Owner-approved standard-library API completion | closed ODR-012 by defining the `Hash`/`Hasher` protocol, `DefaultHasher`, Eq/hash coherence, Map key bounds, and the read-only safe key-access boundary without freezing a mixing algorithm |
| 0.9.6_Hardened_4 | Owner-approved callable/interface completion | closed ODR-013 by making `Hash.hash` statically generic over `H: Hasher`, confirming `DefaultHasher implements Hasher`, and excluding implicit dynamic/interface coercion |
| 0.9.6_Hardened_5 | Owner-approved architecture/process hardening | materialized the post-H4 simplicity RFC through derived-only compiler aggregates, conservative unknown handling, reuse/inference gates, semantic diagnostics, complexity accounting, generated view-contract documentation, and separate authority/evidence ledgers without changing source semantics |
| 0.9.6_Hardened_6 | Owner-approved standard-library API and safety-boundary completion | closed ODR-014 with named Span iterator/chunk view types, exact shared/mutable reborrow contracts, all-profile zero-chunk panic, safe raw-pointer extraction, and existing-unsafe pointer use; normalized shared raw-pointer spelling to Ember's canonical `*T` |
| 0.9.7_Hardened_1 | **Owner-approved language revision** | closed ODR-015 with the explicit `@latebound` callable-type boundary: fresh invocation-local callback regions, binder-free results, compiler-only callable identity/EMIF facts, and ordinary library APIs rather than name-specific compiler behavior |
| 0.9.8_Hardened_1 | **Owner-approved language revision** | closed ODR-017 with `Shared(value)`, `Shared.get()`, `Shared.get_mut()`, and the generalized counted-owner form `Weak[O]`, preserving `Weak[C]` and adding `Weak[Shared[T]]` |
...
### 0.9.5_Hardened_10 Historical Audit Record

| ID | Classification | Resolution |
|---|---|---|
| AUD-095-49 | Owner-approved initialization contract | Closed ODR-009 with deterministic `Zeroable` then `Default` selection, `E2040` when neither capability exists, and an unconditional `!needs_drop(T)` boundary for ordinary Arena bulk allocation. |
| AUD-095-50 | Unsafe-state boundary | Defined `MaybeUninit[T]` layout, non-dropping ownership behavior, `Copy` condition, safe initialization, unsafe consumption, full-span conversion, and partial-initialization prohibition. |
| AUD-095-51 | Canonical API | Fixed the v1 spellings `MaybeUninit.uninit`, `write`, `assume_init`, and the corresponding `MutSpan[MaybeUninit[T]]` `write_at`/`assume_init` operations; no implementation-defined second conversion is allowed. |
| AUD-095-52 | Failure and phase ordering | H10 described `Default` initialization as transactional and required the core `Zeroable` predicate/built-ins during the Arena phase while leaving general derive generation in its later phase. H1's owner-approved `[ARN-10]` clarification governs the abort-only failure boundary. |
| AUD-095-53 | Rule-ID integrity | Preserved H9's existing `[ARN-4]`–`[ARN-7]` meanings. The owner feed's colliding headings were normalized to unused `[ARN-8]`–`[ARN-13]`; `[TST-ARN-MU]` became numeric `[TST-23]`. The owner-approved substance is unchanged. |

**Amendment:** `HC-095-09` — owner-approved Arena allocation and initialization contract.
Because H9 was already frozen, ADR-023 required this clarification to be issued as H10. At issuance,
the adopted 0.8.5 source remained unchanged and H10 became the new frozen, not-yet-adopted development target; it made
no blanket implementation or conformance claim.

#### 0.9.5_Hardened_10 historical language-conformance condition

`0.9.5_Hardened_10` MUST NOT be declared conformant unless `[LT-8]`–`[LT-13]`,
`[LT-8a]`, `[LT-11a]`, `[FN-6a]`, `[TST-20]`, `[TST-21]`,
`[LT-4a]`, `[LT-4b]`, `[TST-22]`, `[ARN-8]`–`[ARN-13]`, `[TST-23]`,
`[LT-14]`–`[LT-43]` including `[LT-31a]`, `[MIR-REG-1]`,
...
H10 incorporates the Hardened_14 hardening record including the owner-supplied
`[LT-8]`–`[LT-13]` source, the accepted 0.9.5 multi-region-view language revision, and the
callable-provenance hardening of Hardened_2. H10's revision identity, consolidated-body
...
#### 0.9.5_Hardened_10 historical acceptance condition

`0.9.5_Hardened_10` MUST NOT be declared clean unless:
1. the H10 document has one version identity;
2. H10's consolidated Parts I–XXIII body and then-current addenda form one normative source;
3. H10's then-active revision lineage ends at `0.9.5_Hardened_10`;
4. H10's consistency checklist targets `0.9.5_Hardened_10`;
...
15. `[ARN-8]`–`[ARN-13]` define one complete initialization boundary without changing `[ARN-4]`–`[ARN-7]`'s existing meanings.

---

### 0.9.5_Hardened_10 Historical Evidence Status — No Implementation Claim
...
### 0.9.7_Hardened_1 Audit Record

**Version treatment:** owner-approved language revision. The new source-level `@latebound`
callable-type modifier changes the accepted/rejected program set at callback boundaries, so H6 is
immutable and this successor resets the hardening number rather than claiming an implementation-only
clarification. The modifier adds no named lifetime syntax and no runtime or ABI representation.

| ID | Classification | Resolution |
|---|---|---|
| AUD-097-01 | Owner-approved language/API boundary | Closed ODR-015 with `@latebound` on callable types, never on named function declarations and never by compiler recognition of a library name. |
| AUD-097-02 | Lifetime and escape preservation | Bound fresh invocation-local regions to the callback boundary and required its result to be free of those regions; ordinary region/escape diagnostics remain authoritative. |
| AUD-097-03 | Compiler-only contract identity | Required the boundary fact through canonical callable identity, generic substitution, expected-callable checking, EMIF/interface compatibility, and incremental invalidation, while excluding runtime metadata, ABI bytes, reload schemas, and source-visible region identities. |
| AUD-097-04 | Conformance boundary | Required adversarial escape, storage, owned-capture, FFI-publication, freshness, and nested-boundary cases without introducing a bespoke `with_views` diagnostic or compiler intrinsic. |

**Amendment:** `HC-097-01` records the owner-approved ODR-015 language revision. H6 remains
byte-for-byte unchanged as H1's immediate predecessor. `0.9.7_Hardened_1` MUST NOT replace
`docs/spec-source/ember-spec.md` until every H6 condition remains green and executable evidence
proves the complete `[LT-7]`, `[LT-8]`–`[LT-10]`, `[FN-6b]`, and updated `[TST-20]`/`[TST-21]`
matrix.

### 0.9.7_Hardened_1 Evidence Status — No Implementation Claim

This H1 file is a frozen development target. The adopted repository specification remains
`docs/spec-source/ember-spec.md` (`0.8.5_Hardened_1`). `@latebound` is **SPECIFIED**, not
implemented or conformant, until parser, canonical type/EMIF facts, higher-ranked invocation-region
checking, diagnostics, generated-code inspection, and adversarial conformance evidence are recorded.

### 0.9.6_Hardened_6 Audit Record

| ID | Classification | Resolution |
|---|---|---|
| AUD-096-29 | Owner-approved API completion | Closed ODR-014 with exact named Span iterator/chunk types and signatures rather than an implementation-invented or opaque return mechanism. |
| AUD-096-30 | Borrow and provenance preservation | Required shared and mutable reborrows, source-bounded iterator lifetimes, ordinary NLL reuse, and non-overlapping mutable chunk storage identities under the existing region and `[BRW-5]` models. |
| AUD-096-31 | Unsafe-boundary consistency | Made raw-pointer extraction safe while keeping dereference, access, arithmetic, reference construction, and validity under the existing unsafe contract; normalized the owner's shared-pointer notation to canonical Ember `*T`. |
| AUD-096-32 | Failure determinism | Required zero-sized shared and mutable chunks to panic through the existing failure mechanism in every supported profile and prohibited empty, fallible, zero-width, or non-progressing interpretations. |
| AUD-096-33 | Public identity and evidence | Assigned four public, non-prelude `@view` iterator types to `std.collections`, added `[TST-25]`, and made no implementation or conformance claim from specification prose. |

**Amendment:** `HC-096-06` records the owner-approved ODR-014 completion, and
ADR-035 preserves its API, borrowing, failure, public-identity, and unsafe-boundary decisions. H1
through H5 remain unchanged; H5 is H6's immediate predecessor.

#### 0.9.6_Hardened_6 language-conformance condition

`0.9.6_Hardened_6` MUST NOT be declared conformant until every H5 condition remains satisfied and
repository evidence additionally proves the complete `[TST-25]` matrix: shared and mutable iterator
items, source-bounded reborrows and NLL reuse, rejection of conflicts, partial final chunks,
non-overlapping mutable chunk identities, all-profile zero-size panic, safe pointer extraction,
unsafe-only pointer use, no lifetime extension, generic substitution, and region-metadata erasure.

#### 0.9.6_Hardened_6 acceptance condition

H6 MUST NOT replace `docs/spec-source/ember-spec.md` unless it has one version identity and H5 as its
immutable predecessor, every H5 adoption condition remains green, the H5-to-H6 classified diff is
traceable only to the preserved ODR-014 owner ruling and ADR-035, the public API and unsafe boundary
are implemented without a second iterator/lifetime/ownership model, and generated-code plus
adversarial conformance evidence satisfies `[TST-25]`.

### 0.9.6_Hardened_6 Evidence Status — No Implementation Claim

This H6 file is a frozen development target. The adopted repository specification remains
`docs/spec-source/ember-spec.md` (`0.8.5_Hardened_1`). The Span iterator/chunk/raw-pointer completion
is `SPECIFIED` until the implementation and conformance ledgers cite executable evidence. Passing
the six gates against the adopted source does not install H6 or prove its new requirements.

### 0.9.6_Hardened_5 Audit Record

| ID | Classification | Resolution |
|---|---|---|
| AUD-096-22 | Owner-approved architecture/process hardening | Materialized the approved post-H4 Simplicity Consolidation RFC as H5 while preserving H4 unchanged. |
| AUD-096-23 | Single semantic authority | Required indexes, caches, summaries, serialized metadata, backend forms, and `CallInfo`-style aggregates to remain derived, checked, and invalidated with their authoritative inputs. |
| AUD-096-24 | Conservative uncertainty | Unified unknown-boundary handling under “absence of proof grants no capability” without replacing any domain's existing H4 rule. |
| AUD-096-25 | Surface restraint | Required reuse and inference review before new syntax, annotations, public abstractions, or feature-specific safety systems; added no `Len`/`Capacity` interface. |
| AUD-096-26 | Documentation and diagnostics | Required source-semantic primary diagnostics and derivable view-contract documentation while keeping compiler bookkeeping expert-only. |
| AUD-096-27 | Process and evidence | Added explicit complexity accounting and preserved separate normative, implementation, owner-decision, and conformance ledgers. |
| AUD-096-28 | Semantic preservation | Preserved H4's accepted/rejected sets, observable behavior, ABI, diagnostics identities, safety guarantees, initialization lattice, and independent semantic axes. |
| AUD-096-19 | Owner-approved callable completion | Closed ODR-013 with `fn hash[H: Hasher](self, mut h: H)`, ordinary inference/monomorphization, and no mandatory dynamic dispatch. |
| AUD-096-20 | Interface consistency | Confirmed `DefaultHasher implements Hasher`; rejected bare-interface concrete parameters and compiler-only coercions under `[TYP-22]`. |
| AUD-096-21 | Source normalization | Removed transport angle brackets around the ordinary parameter list while preserving the owner-selected generic bound exactly. |
| AUD-096-14 | Owner-approved API completion | Closed ODR-012 by defining the public `Hash`/`Hasher` protocol and concrete `DefaultHasher` required by the existing Map contracts. |
| AUD-096-15 | Coherence and safety | Required `a == b` to imply equal hashes, prohibited retention of supplied byte spans, kept hashing state move-only, and prohibited safe mutable key exposure. |
| AUD-096-16 | Implementation freedom | Kept the exact mixing algorithm outside the source-level compatibility contract while retaining deterministic input and equality-coherence requirements. |
| AUD-096-17 | Public identity | Assigned `Hash`, `Hasher`, and `DefaultHasher` to `std.collections`; preserved inherited `[MOD-5]` prelude export of `Hash` and added neither remaining name to the prelude. |
| AUD-096-18 | Historical H3 boundary | H3 exposed that `mut h: Hasher` did not determine how a concrete move-only hasher crossed `[TYP-22]`; ODR-013/ERR-054 stopped implementation until the owner selected the static generic contract now recorded by AUD-096-19. |
| AUD-096-10 | Owner-approved API completion | Closed ODR-011 with fixed-capacity `ArenaArray`/`ArenaMap` semantics, exact construction/provenance, minimum operations, failure, invalidation, destruction, and ordering requirements. |
| AUD-096-11 | Type-system consistency | Replaced the proposed generic-looking `Iterator[ref T]` spelling with named `@view` iterator types implementing the existing associated-type `Iterator[Item = ...]` contract; no opaque-return or second iterator mechanism was introduced. |
| AUD-096-12 | Public identity | Assigned all six Arena-backed collection names to `std.collections`, kept them out of the prelude, made construction observably empty, and defined the sole capacity-exhaustion value as `CapacityError.Full`. |
| AUD-096-13 | Evidence integrity | Added `[TST-24]` but made no implementation/conformance claim merely from the completed specification. |
| AUD-096-01 | Source authority | Preserved the owner-supplied Revision 5 RFC and ODR-011 ruling byte-for-byte, preserved H1 through H3 unchanged, and recorded the ODR-012/ODR-013 authorities separately; H3 is the immutable immediate predecessor, while `ember-spec.md` remains the adopted 0.8.5 contract until H4 passes explicit adoption gates. |
| AUD-096-02 | Owner-approved architecture | Consolidated compiler facts around `TypeIdentity`, `BorrowCapability`, `OwnershipGraph`, `AccessContract`, `InitializationState`, `LayoutDescriptor`, and `EffectSet`; no public type or parallel ownership/lifetime system was introduced. |
| AUD-096-03 | Capability safety | Separated provenance root, optional source place, storage identity, projection, region, access permission, reference kind, ownership, acquisition/checking, safety authority, synchronization, validity, and escape constraints. Region equality is not storage overlap or `noalias`. |
| AUD-096-04 | Compiler boundaries | Made type/interface solving construct completed typed HIR from the resolved AST, then lowered initial MIR before initialization/ownership/borrow/region analyses and retained MIR verification after safety-critical transformations. |
| AUD-096-05 | Callable contracts | Preserved field-access and field-to-region provenance summaries, trusted metadata boundaries, conservative opaque/dynamic calls, caller/cache invalidation, stale-summary hard failure, and runtime erasure. |
| AUD-096-06 | Semantic preservation | Preserved mandatory cycle reporting, class-only dynamic exclusivity, interior-mutability distinctions, FFI/coroutine/ECS boundaries, stable diagnostics, and all three replacement/destruction orderings. |
| AUD-096-07 | Owner-approved `[ARN-10]` clarification | Removed the impossible v1 requirement to recover from an aborting `T.default()` panic. No post-panic Arena state is observable; no unwinder is introduced; future recoverable APIs receive rollback only from their own explicit contract. |
| AUD-096-08 | Rule-ID discipline | Amended existing implementation rules rather than copying the RFC's architecture labels into the normative rule index. No suffixed Arena rule or parallel `SIMP-*` rule family was introduced. The raw extractor's inherited `IDE-2`/`IDE-5`/`IDE-10` reservations and historical `HOT-10` citation are not normative rule references under the existing ERR-042 inventory; the classified dangling-normative-reference count is zero. |
| AUD-096-09 | Evidence integrity | Classified this as a specification target only. No compiler, runtime, conformance, or performance status was inferred from the document. |

**Amendments:** `HC-096-01` records the owner-approved simplicity consolidation and
`[ARN-10]` clarification in H1; `HC-096-02` records the owner-approved ODR-011
Arena-backed collection completion in H2; and `HC-096-03` records the owner-approved ODR-012
hashing-contract completion in H3; `HC-096-04` records the owner-approved ODR-013
static Hasher boundary in H4; and `HC-096-05` records materialization of the approved
post-H4 simplicity RFC in H5. ADR-030 through ADR-034 preserve the five decisions.
H1 through H4 remain unchanged; H4 is H5's immediate predecessor.

#### 0.9.6_Hardened_5 language-conformance condition

`0.9.6_Hardened_5` MUST NOT be declared conformant until all H1 through H4 conditions remain satisfied and
repository evidence additionally proves that:

1. equivalent borrow relationships represented by `Span`, `MutSpan`, `ref`, `ref mut`, `@view`
   fields, Arena-backed views, `RefCell` guards, and foreign views receive equivalent safety decisions
   where their semantic axes are equivalent;
2. lifetime provenance and concrete storage identity remain distinct, and neither region equality nor
   region inequality is used as a substitute for an overlap/disjointness proof;
3. exact, trusted-declared, and conservative-unknown callable facts preserve both access and
   field-to-region provenance, including generic, separately compiled, dynamic, and hot-reload target
   sets;
4. changing either callable summary rechecks affected callers and invalidates all dependent interface,
   generic, and cache artifacts; stale metadata is a hard compiler failure;
5. compiler-only capability, region, access-summary, and initialization-proof metadata is absent from
   runtime layout, ABI, serialization, and hot-reload schemas;
6. ordinary assignment, `Cell.set`, and `MaybeUninit.write`/`write_at` preserve their distinct ordering
   contracts in MIR and generated code;
7. `[ARN-10]` tests show successful `Default` construction, v1 process termination through `[PAN-1]`
   on panic, no continuation after panic, no compiler-invented unwind path, and no observation of
   post-panic Arena state;
8. existing H10/H1/H2 safety regressions remain present and green; implementation-path consolidation is
   not used to delete an adversarial test;
9. `[TST-24]` proves empty fixed-capacity construction, exact Arena provenance, no post-construction
   allocation/cursor movement, `CapacityError.Full`, complete Array/Map operations, drop-free bounds,
   ordinary borrow conflicts, named iterator contracts, deterministic iteration requirements, and
   generic substitution;
10. `[HASH-1]`–`[HASH-4]` are implemented through the ordinary interface/type machinery, equal
    values produce equal hashes, supplied spans cannot escape the hasher call, `finish` consumes its
    move-only state, custom Map keys satisfy `Eq + Hash`, and no safe Map API exposes mutable keys;
11. implementation and conformance evidence does not freeze `DefaultHasher`'s current mixing
    algorithm as source-level behavior;
12. ODR-013's statically generic `H: Hasher` representation is reflected consistently in signatures,
    inference, monomorphization, implementation, and conformance without bare-interface or
    compiler-only dynamic coercion.
13. every `CallInfo`-style aggregate, cache, index, summary, and serialized compiler artifact is
    derived from authoritative semantic facts, invalidated with them, and rejected when stale;
14. `AccessContract`, `EffectSet`, provenance, retention, storage identity, permission,
    representation, ownership, checking, unsafe authority, synchronization, validity, escape, and
    initialization remain independent wherever H4 distinguishes them;
15. initialization analysis retains `Uninit | Maybe | Init` plus required field/element/range facts,
    and no unknown fact becomes a capability or definite-uninitialized proof;
16. same-Arena distinct allocations retain distinct storage identities, unknown overlap never emits
    `noalias`, and unknown callable/foreign/dynamic boundaries use their domain's conservative result;
17. no `Len`/`Capacity` interface, parallel safety system, automatic dynamic-borrow fallback, or
    compiler-only runtime metadata is introduced by this hardening;
18. public feature proposals carry the required reuse, inference, complexity, and performance review;
19. primary diagnostics and generated view API documentation are derived from authoritative facts and
    expose source semantics without becoming duplicate semantic authorities.

#### 0.9.6_Hardened_5 acceptance condition

H5 MUST NOT replace `docs/spec-source/ember-spec.md` unless it has one version identity and predecessor,
the complete alternate-spec rule/index/grammar/diagnostic audit has no unintended regression, the
implementation and conformance matrices separate `SPECIFIED` from evidence-backed states, all required
compiler/runtime/test work passes, generated-code inspection proves runtime erasure and ordering where
source observation is insufficient, and every difference from H4 is classified and traceable to the
owner-approved post-H4 simplicity ruling and ADR-034.

### 0.9.6_Hardened_5 Evidence Status — No Implementation Claim

This H5 file is a frozen development target. The adopted repository specification remains
`docs/spec-source/ember-spec.md` (`0.8.5_Hardened_1`). H5's architecture, 0.9.5 multi-region semantics,
Arena-backed collections, hashing contract, and later phases are `SPECIFIED` unless the repository's
implementation matrix and executable evidence say otherwise. Passing the six gates against the adopted
source does not constitute H5 adoption or H5 conformance.


---

### 0.9.6_Hardened_6 consolidation record

The body below originated in `0.8.5_Hardened_1`, incorporates the accepted 0.9/0.9.5 changes in
place, carries the owner-selected 0.9.6 reference-compiler consolidation, and
completes ODR-011's Arena-backed collection contract, ODR-012's hashing
protocol/API contract, ODR-013's static Hasher call boundary, the owner-approved post-H4
simplicity architecture/process contract, and ODR-014's Span iterator, chunk, and raw-pointer
boundary. It is normative within
this target together with the current addenda above. Frozen predecessor files are
...
### Consolidated-body header, as carried by 0.9.7_Hardened_1
...
the H6 target, not a verbatim historical snapshot. The front-matter authority boundary controls
...
of the current H1 document, which is identified in the front matter.
