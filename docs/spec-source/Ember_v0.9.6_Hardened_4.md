...
**Immediate predecessor:** 0.9.6_Hardened_3
...
conformance result is asserted merely because the specification requires one. This H4 successor
...
boundary on which the Map contracts already depended.
...
hashing-contract completion in H3; and `HC-096-04` records the owner-approved ODR-013
static Hasher boundary in H4. ADR-030 through ADR-033 preserve the four decisions.
H1 through H3 remain unchanged; H3 is H4's immediate predecessor.

### 0.9.6_Hardened_4 language-conformance condition

`0.9.6_Hardened_4` MUST NOT be declared conformant until all H1, H2, and H3 conditions remain satisfied and
...
### 0.9.6_Hardened_4 acceptance condition

H4 MUST NOT replace `docs/spec-source/ember-spec.md` unless it has one version identity and predecessor,
...
source observation is insufficient, and every difference from H3 is classified and traceable to the
owner-approved ODR-013 ruling.

## 0.9.6_Hardened_4 Evidence Status — No Implementation Claim

This H4 file is a frozen development target. The adopted repository specification remains
`docs/spec-source/ember-spec.md` (`0.8.5_Hardened_1`). H4's architecture, 0.9.5 multi-region semantics,
...
source does not constitute H4 adoption or H4 conformance.
...
## 0.9.6_Hardened_4 consolidation record
...
protocol/API contract, and ODR-013's static Hasher call boundary. It is normative within
...
# Appendix B — 0.9.6_Hardened_4 Consistency Checklist

The following checks are mandatory CI checks for the consolidated specification itself:

1. Exactly one current version declaration exists and names `0.9.6_Hardened_4`.
2. The immediate predecessor is frozen `0.9.6_Hardened_3`, which remains byte-for-byte unchanged.
3. `[MOD-6]` and `[MOD-6a]` name the supported `0.9.6`, `0.9.5`, `0.9`, and 0.8.x source-language contracts explicitly; `0.9` does not silently enable later semantics, and H4 changes H3 only through the owner-approved ODR-013 completion.
