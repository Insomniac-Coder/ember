...
**Immediate predecessor:** 0.9.6_Hardened_1
...
conformance result is asserted merely because the specification requires one. This H2 successor
incorporates the owner-approved ODR-011 completion of the previously named but under-specified
Arena-backed collection surface.
...
| AUD-096-01 | Source authority | Preserved the owner-supplied Revision 5 RFC and ODR-011 ruling byte-for-byte; H1 is the immutable immediate predecessor, while `ember-spec.md` remains the adopted 0.8.5 contract until H2 passes explicit adoption gates. |
...
Arena-backed collection completion in H2. ADR-030 and ADR-031 preserve the two decisions.
H1 remains unchanged and is H2's immediate predecessor.

### 0.9.6_Hardened_2 language-conformance condition

`0.9.6_Hardened_2` MUST NOT be declared conformant until all H1 conditions remain satisfied and
...
8. existing H10/H1 safety regressions remain present and green; implementation-path consolidation is
...
### 0.9.6_Hardened_2 acceptance condition

H2 MUST NOT replace `docs/spec-source/ember-spec.md` unless it has one version identity and predecessor,
...
source observation is insufficient, and every difference from H1 is classified and traceable to the
owner-approved ODR-011 ruling.

## 0.9.6_Hardened_2 Evidence Status — No Implementation Claim

This H2 file is a frozen development target. The adopted repository specification remains
`docs/spec-source/ember-spec.md` (`0.8.5_Hardened_1`). H2's architecture, 0.9.5 multi-region semantics,
Arena-backed collections, and later phases are `SPECIFIED` unless the repository's
implementation matrix and executable evidence say otherwise. Passing the six gates against the adopted
source does not constitute H2 adoption or H2 conformance.
...
## 0.9.6_Hardened_2 consolidation record
...
completes ODR-011's Arena-backed collection contract. It is normative within
...
* `[ARN-5d]` **`ArenaMap[K, V]` minimum API.** `ArenaMap[K, V]` requires
  `K: Eq + Hash`, using the same equality/hash contract as ordinary
  `Map[K, V]`. Its canonical minimum API is:
...
* `[TST-24]` **Arena-backed container conformance.** Conformance MUST cover:
  ...
  substitution and Arena provenance through generic wrappers.
...
| `std.collections` | `Array`, `SmallArray`, `Deque`, `Map`, `Set`, `BitSet`, `Pool`, `Handle`, `SoA` (derive support), `ArenaArray`, `ArenaMap`, `CapacityError`, `ArenaArrayIter`, `ArenaArrayIterMut`, `ArenaMapIter`; the Arena-backed names are public but not prelude exports |
...
# Appendix B — 0.9.6_Hardened_2 Consistency Checklist

The following checks are mandatory CI checks for the consolidated specification itself:

1. Exactly one current version declaration exists and names `0.9.6_Hardened_2`.
2. The immediate predecessor is frozen `0.9.6_Hardened_1`, which remains byte-for-byte unchanged.
3. `[MOD-6]` and `[MOD-6a]` name the supported `0.9.6`, `0.9.5`, `0.9`, and 0.8.x source-language contracts explicitly; `0.9` does not silently enable later semantics, and H2 changes H1 only through the owner-approved ODR-011 completion.
