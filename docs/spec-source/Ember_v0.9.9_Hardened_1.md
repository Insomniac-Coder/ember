...
**Version:** 0.9.9_Hardened_1
**Supersedes:** 0.9.8_Hardened_3 (development target) and 0.8.5_Hardened_1 (adopted). This document is
...
* `[VER-9]` *(new in 0.9.9)* A language revision that changes the set of accepted programs or their meaning moves the
  language version and resets the hardening number to 1. A hardening adds precision without changing
  any accepted program's meaning. The change log of each revision (Appendix H for this one) lists every
  rule it adds, removes or changes; a change without a row is a defect in the document.
...
* `[TIER-1]` Safe code MUST NOT invoke an operation with an unverifiable precondition unless an
  ...
  (`[FFI-10]`); an `unsafe overlay` (`[FFI-2]`). No attribute, profile, manifest key or contract
  introduces a fourth.
...
* `[PHIL-13]` *(new in 0.9.9)* **One meaning per program.** No build profile, compiler flag, manifest key or
  ...
  in diagnostics and panic messages (`[PRF-1]`).
...
| Object aliasing (long-term access through a class handle) | the accesses go through one handle local and are statically ordered (`[EXC-3]`) | otherwise | header access word; a load, a compare and a store |
...
access on the emitter (`[EXC-1]`). `1.0 / 60.0` takes `f32` from the parameter it is passed to.
...
Part XIII); implicit numeric narrowing; function overloading, apart from the output functions
(`[TYP-26]`); macros (`macro` is reserved); named lifetimes, ever (`[LEX-22]`); specialisation or
...
* `[GRM-2]` A file contains only imports and items. A statement at file scope is `E0100` with the
  help `put statements inside 'fn main():'`.
...
extern_item   := {attribute} (fn_header NEWLINE
...
postfix       := "(" [arg {"," arg} [","]] ")" | "[" expr_list "]" | "." IDENT | "." INT
               | "?." IDENT | "?"
...
* `[TYP-5]` *(changed in 0.9.9)* **Coercions — the complete list.** At a coercion site (assignment or
  ...
  Each rule composes with the others only where stated (widening after range erasure). Nothing
...
* `[TYP-28]` *(new in 0.9.9)* **Division.** `/` is **true division** and applies to floats. `/` with two integer
  ...
  * a zero divisor panics (`division by zero`); `MIN // -1` and `MIN.div_trunc(-1)` overflow and panic
    per `[TYP-8]`.
* `[TYP-29]` *(new in 0.9.9)* For floats, `a // b` is `floor(a / b)` and `a % b` is `a - b * floor(a / b)` (the sign of
  `b`, Python); both follow IEEE for infinities and NaN.
...
* `[RNG-4]` The compiler tracks a known range for numeric expressions — literals, `min`, `max`,
  `clamp`, the arms of a comparison, and arithmetic on known ranges — and uses it to remove `[RNG-3]`'s
  check, `[TYP-8]`'s overflow check and bounds checks (`[OPT-2]`). A fact is derived from an operation's
  mathematical result only when that result provably fits; never inside `@fastmath`.
...
* `[TYP-35]` *(new in 0.9.9)* A type parameter need not appear in any field (a **phantom** parameter): `struct
  Handle[Tag]: index: u32` is legal. A phantom parameter affects type identity only; it is `Send` and
  `Sync` and does not make the type a view.
...
* `[HASH-1]` `Hash.hash` is generic over `H: Hasher` and monomorphised; hashing a value MUST feed a
  ...
  `Hasher` (in `std.collections`, not the prelude) is a move-only hashing state with `write_u8`,
  `write_u64`, `write_bytes` and friends, and `finish(owned self) -> u64`.
* `[HASH-2]` *(changed in 0.9.9)* `std.collections.DefaultHasher` is a fixed-seed hasher: the same
  keys hash the same way in every run and on every machine. `std.collections.RandomState` is a
  per-process randomly seeded hasher for maps keyed by untrusted input; a `Map[K, V, RandomState]`
  still iterates in insertion order (`[STD-11]`), so the seed never becomes visible.
* `[HASH-3]` `Map` and `Set` MUST NOT weaken equality to compensate for an incoherent `Hash`.
...
* `[TYP-23]` Inference is local to a function body and bidirectional. Function signatures, fields,
  statics and constants are annotated (an omitted return type is `void`). A local declared by `x = e`
  takes `e`'s type; a type left open by `e` (`[]`, `Map()`, `None`) is fixed by later uses in the same
  function, and one still open at the end is `E2060`, highlighting the first use. Expected types flow
  into literals, lambdas, `None`, `Ok(…)`, `Err(…)` and generic calls. A lambda with unannotated
  parameters and no expected callable type is `E2061`. Untyped literals are resolved last.
...
* `[MOD-3]` *(changed in 0.9.9)* `import a.b.c` binds the name `c` to module `a.b.c`, and
  `import a.b.c as d` binds `d`. `from a.b import x, y as z` binds items. A module path that names no
  module is `E1060 no module named 'a.b.c'` at the import, whose help lists the nearest existing module
  paths; an item missing from an existing module is `E1061`. Neither is ever deferred to the first use
  (`[PHIL-12]`).
...
* `[MOD-5]` *(changed in 0.9.9)* **The prelude.** Every module implicitly imports these names from
  ...
  | Interfaces | `Copy`, `Clone`, `Drop`, `Default`, `Eq`, `Ord`, `Ordering`, `Hash`, `Debug`, `Display`, `Iterator`, `Iterable`, `IntoIterator`, `Contains`, `From`, `Send`, `Sync`, and the operator interfaces `Add`, `Sub`, `Mul`, `Div`, `FloorDiv`, `Rem`, `Pow`, `Neg`, `Not`, `BitAnd`, `BitOr`, `BitXor`, `Shl`, `Shr`, `Index`, `IndexMut`, `IndexSet`, with the `…Assign` forms of the arithmetic and bitwise interfaces |
  | Functions | `print`, `println`, `eprint`, `eprintln`, `input`, `assert`, `assert_eq`, `assert_ne`, `debug_assert`, `panic`, `todo`, `unreachable`, `min`, `max`, `abs`, `clamp` |
...
* `[FN-1]` **Parameter modes.**
  * `a: A` — **borrowed** (the default). The callee reads `a` and cannot move it or assign through it.
    A `Copy` value no larger than two pointers is passed in registers; the meaning is the same.
...
* `[FN-3]` A function returns by move. Returning a reference or view requires its region to derive
  from a parameter (`[LT-1]`).
...
* `[FN-6]` *(changed in 0.9.9)* **Callable types.** A function is a value. A callable type is written
  ...
  also coerces to `extern "C" fn(…)` when every type is FFI-safe. Borrowed and view parameters of a
...
* `[STR-5]` *(changed in 0.9.9)* **Implicit derives.** A struct or enum implements `Eq`, `Debug` and
  ...
  (or `Debug`, `Clone`) opts out, and a hand-written implementation replaces the implicit one.
...
* `[CLS-2]` `fn init(self, …)` is the constructor. Every field without a default MUST be assigned on
  every path before `self` is used as a whole (`E2100`). Several constructors are written as associated
  functions that call `Self(…)`.
* `[CLS-3]` A class with no `init` and no base class gets a memberwise constructor as a struct does.
* `[CLS-10]` *(new in 0.9.9)* A derived class with no `init` gets its base's constructor: `Door(entity)` runs
  `Script.init(entity)` and initialises `Door`'s own fields from their defaults. A derived class with a
  field that has no default MUST declare an `init` (`E2101`).
* `[CLS-4]` A class is final unless declared `open` or `abstract`. Methods are non-virtual unless
  `virtual`; replacing one requires `override`; overriding a non-virtual method is `E2110`. A derived
  `init` MUST call `super.init(…)` exactly once, before using inherited fields. Fields cannot be
  overridden. Base fields are laid out first; there is at most one base.
...
* `[CLS-7]` *(changed in 0.9.9)* Inside a class method, `self` is a handle. **Any method may read and
  ...
  on a class method declares that the method holds one write access to the whole object for its whole
  duration, which the compiler may use to remove the per-access checks inside it (`[EXC-15]`).
...
```text
offset  size  field
0       4     strong   u32   atomic iff the class is Sync
4       4     weak     u32   same atomicity
8       4     access   u32   dynamic exclusivity (bit 31 writer, bits 0..30 reader count); unused by Sync classes
...
* `[OBJ-1]` The header is 24 bytes on 64-bit targets and is part of the runtime ABI (`[VER-4]`).
  Foreign code never reads it; it calls `ember_rt`.
...
* `[RC-3]` Further elisions are allowed only when semantics are preserved (`[PHIL-5]`). An object may
  ...
  an observable `drop` is bound and never read again.
* `[RC-4]` A non-`Sync` class's counts use plain loads and stores; a `Sync` class's use a relaxed
  increment and an acquire-release decrement (`[RT-8]`).
...
| passing a field to a parameter (`f(h.items)`) | write for `mut`, read for borrowed, `E3012` for `owned` | the call |
| `ref h.f` / `ref mut h.f`, or a view of a field | read / write | the reference's liveness |
...
| calling a `mut self` class method (`[CLS-7]`) | write, whole object | the call |

* `[EXC-1]` *(changed in 0.9.9)* Beginning a **write** access to an object while any access to it is
  active panics: `exclusivity violation: write access to Node.children while a read access is
  active`, in every profile. There is no setting that removes this check (`[PHIL-13]`).
* `[EXC-2]` Beginning a **read** access while a write access is active panics likewise.
...
* `[EXC-6]` A panic from `[EXC-1]`/`[EXC-2]` names both the offending access and the active one it
  conflicts with (file, line, column, field). The `debug` runtime keeps a per-thread stack of active
  accesses for this; other profiles always name the offending access and name the active one where they
  can without any per-access cost (the header layout of `[OBJ-1]` does not grow).
* `[EXC-7]` The opt-in lint `L3013` reports a long-term access held across a virtual or `dyn` call
  within one `open` class hierarchy.
...
* `[EXC-15]` *(new in 0.9.9)* A `mut self` class method holds a write access to the whole object from entry to return;
  accesses to `self`'s fields inside it are covered by it and not checked individually.
...
* `[WK-11]` `Weak(h)` creates a weak handle to a class object or `Shared` payload without retaining it
  strongly; `Weak[O].empty()` (or `Weak.empty()` where the type is known) creates one that points
  nowhere. `Weak[O]` is `Copy`: copying increments the weak count, dropping decrements it.
...
* `[WK-12]` `w.upgrade() -> Option[O]` returns a retained strong handle while the object is alive and
  not deinitialising, and `None` otherwise.
...
| `Shared[T]` | shared, counted | borrow-checked + dynamic exclusivity | at count 0 | shared ownership of a plain value without declaring a class |
...
* `[HEAP-4]` `s.get() -> ref T` borrows the payload; the loan keeps the block alive (`[RC-5]`).
* `[HEAP-5]` `s.get_mut() -> ref mut T` borrows it mutably with the same dynamic exclusivity as a class
  object's write access (`[EXC-1]`); it does not require the `Shared` to be unique.
* `[HEAP-6]` `Shared[T]` is `Copy`: copying retains, dropping releases.
* `[HEAP-7]` `Weak[O]` exists for `O` a class handle or a `Shared[T]` (`[WK-11]`).
...
* `[ALC-1]` `Array[T, A: Allocator = Global]`, `Map`, `Set` and `Box` accept an allocator type
  parameter; an allocator instance is passed at construction (`Array.new_in(a)`).
...
* `[HND-2]` *(changed in 0.9.9)* `Handle[T]` is a `u64`: 32 bits of index and 32 of generation.
  `Pool[T, Bits]` may narrow it (`Pool[T, Split20]` gives RageV's 20-bit index and 12-bit generation in a
  `u32`).
...
* `[EFF-15]` *(changed in 0.9.9)* Effects and contract verdicts are computed once, after the
  ...
  `[SIMD-7]` and `[DSJ-1]` — so every implementation reaches the same verdict; an implementation may
  remove further checks from the code it emits, but that never changes a verdict. Adding a proof to the
  list is a language revision (`[VER-9]`) and makes more programs satisfy a contract, never fewer.
...
* `[THR-8]` *(new in 0.9.9)* A type is **`Send`** when a value of it may be moved to another thread.
  ...
  `MutSpan[T]` when `T` is `Send`. A class handle is `Send` only if the class is `@sync`
  (`[THR-2]`). `Shared[T]` and `Weak[Shared[T]]` are `Send` when `T` is `Sync`.
* `[THR-9]` *(new in 0.9.9)* A type is **`Sync`** when several threads may read one value of it at
  ...
  raw pointers, handles of classes that are not `@sync`, and `Shared[T]` of a non-`Sync` `T` are not
  `Sync`. `Atomic[T]`, `Once`, and `Mutex[T]`/`RwLock[T]` for a `Send` `T` are `Sync`.
* `[THR-1]` *(changed in 0.9.9)* **A class is `Sync` only when it is declared `@sync class`.** Every
  ...
  * a base class and every derived class are `@sync` too (`E7001`).
  ...
  objects. Every access to a `@sync` object is a read, so it needs no exclusivity check and its header
  access word is unused (`[OBJ-1]`). The error's help names the field to wrap:
...
* `[THR-13]` *(new in 0.9.9)* **The data-race guarantee.** In Safe Ember a memory location is reachable
  ...
  (`[THR-1]`); (b) a `static`, which is immutable and `Sync` (`[STA-1]`); (c) a `Shared[T]` of a
  `Sync` `T`; (d) a borrow handed to a scoped task, which the borrow rules make either shared and of a
...
* `[THR-10]` *(new in 0.9.9)* `thread.spawn(f: owned fn() -> R) -> JoinHandle[R]` runs `f` on a new
  thread. The closure's captured state and `R` MUST be `Send` (`E7004`, naming the capture).
  `h.join() -> R` waits and returns the result. Dropping a `JoinHandle` detaches the thread. When
  `main` returns, the process exits; detached threads stop without running destructors.
...
* `[THR-6]` A type whose `drop` a safety guarantee depends on is `@must_drop`. A `@must_drop` value
  MUST NOT be passed to `mem.forget`, stored in a class field, `Shared`, `Box`, a collection or an
  `owned fn` capture, or returned from the function that created it (`E3015`). The standard library
...
* `[PAR-2b]` The analysis runs after the body's calls are inlined. A call it cannot see into that
  ...
  argument (`E7010`).
* `[PAR-3]` `@parallel(reduce=[total: +, best: max])` declares variables combined after the loop with
  an associative operator; inside the body each chunk has its own copy.
...
* `[JOB-2]` `jobs.scope()` follows `[THR-5]` and `[THR-11]`: jobs submitted to a `JobScope` may borrow
  the caller's data, and the block's exit joins them. `jobs.submit(f: owned fn())` outside a scope
  takes an owned, `Send` closure and returns a `JobHandle`; `jobs.wait(h)` waits for it.
...
* `[SOA-6]` *(new in 0.9.9)* `ps[i]` is an **element proxy**: `SoARef[T]` where `ps` is read-only here
  ...
  yields `SoARef[T]`, `for p in ps.iter_mut():` yields `SoAMut[T]`.
...
* `[SIMD-3]` Alias facts given to the backend (`restrict`) MUST be derived, never assumed. Two views
  ...
  code or received through FFI may alias until (d) is applied. Two views being live at once is not a
  proof, and neither are parameter modes.
* `[SIMD-5]` *(changed in 0.9.9)* **Vectorisable form**, which `@simd(assert)` checks, is computed by
  ...
  `RuntimeCheck(Arithmetic)` is removed or grouped by `[SIMD-7]`; the loop has one exit; and every
  floating-point reduction is declared by `@parallel(reduce=…)` or the function is `@fastmath`.
...
* `[SIMD-7]` *(new in 0.9.9)* **Grouped overflow checks.** In a loop whose body has none of `Sync`,
  ...
  iterations (a vector's width times the unroll factor) instead of once per operation: it computes the
  group with wrapping arithmetic, accumulates the lanes' overflow bits, and panics at the end of the
...
* `[ECS-3]` *(changed in 0.9.9)* `Query[(A, Mut[B], Option[C], Not[D])]` is a view over a world's
  ...
  type. A query iterates the smallest required storage and probes the others; `iter_mut` yields
  `ref mut` for `Mut` parameters and requires `mut q`. There is no access-mode generic kind in the
  language.
* `[ECS-4]` A query's read and write sets are compile-time constants computed from its type.
  `world.run(system, args…)` borrows exactly those storages. `world.run_parallel([s1, s2, …])` runs
  systems at once when their sets do not conflict; a conflict between two systems known at compile
  time is `E7020`, naming both systems and the component.
...
* `[ERR-13]` *(new in 0.9.9)* The standard library follows one convention, and user code SHOULD: an
  ...
  (`index 7 out of range for length 3; use .get(i) for an Option`).
...
* `[STD-13]` *(new in 0.9.9)* **One naming convention.** Names are `snake_case` words joined by
  underscores (`starts_with`, `to_upper`, `push`, `is_empty`). Where Python's name for an operation
  ...
  (the full table is Appendix E). A Python name that Ember keeps (`partition`, `items`, `count`)
  keeps Python's meaning (`[PHIL-14]`).
...
* `[STD-9]` *(new in 0.9.9)* `print(a, b, …, sep=" ", end="")` and `println(a, b, …, sep=" ",
  ...
  variable number of positional arguments (`[TYP-26]`). An argument that is not `Display` is `E2040`,
  whose help offers `f"{x!r}"` when the type is `Debug`. `println()` prints an empty line.
...
* `[TXT-10]` *(new in 0.9.9)* **`str` operations.** `len()` is the length in bytes and
  ...
  Unicode-aware; nothing depends on the locale.
...
* `[STD-15]` *(new in 0.9.9)* **`Array[T]`** is a growable contiguous list (Python's `list`).
  ...
  `swap_remove(i) -> T`, `extend(iterable)`, `truncate`, `clear`, `retain(pred)`, `drain(range)`,
  `dedup`, `reverse`, `sort` (stable; `T: Ord`), `sort_by_key(f)`, `sort_by(cmp)`, `sorted() ->
  Array[T]`, `binary_search(x) -> Result[int, int]`, `index_of(x) -> Option[int]`, `first`, `last`,
  `get(i) -> Option[ref T]`, `get_mut`, `iter`, `iter_mut`, `split_at`, `chunks(n)`, `chunks_mut(n)`,
  `windows(n)`, `join(sep)` (for elements that are `Display`), and `x in xs` (`[STD-8]`). `xs[i]`
  takes any integer and panics when `i < 0` or `i >= len` (`[TYP-31]`).
* `[STD-11]` *(new in 0.9.9)* **`Map[K, V, H = DefaultHasher]`** is a hash map that **iterates in
  ...
  iteration is linear in the number of entries. `Set[T, H]` is the same with no values. Because order
  is fixed by the program's operations and the default hasher is fixed-seed (`[HASH-2]`), iterating
  a `Map` or `Set` is not `Nondet`.
* `[STD-16]` *(new in 0.9.9)* **`Map` operations.** `m[k]` reads the value and panics when the key is
  ...
  (`[STD-17]`); `m[k] += 1` requires the key to exist. `get(k) -> Option[ref V]`, `get_mut`,
  `get_or(k, default) -> V` (for `V: Copy`), `insert(k, v) -> Option[V]`, `remove(k) -> Option[V]`,
  `entry(k).or_insert(v) -> ref mut V`, `k in m` (by key), `keys`, `values`, `values_mut`, `items`
  (`(ref K, ref V)` pairs, as `for k, v in m:` iterates), `len`, `is_empty`, `clear`, `retain`.
  `Set`: `add`, `remove`, `x in s`, `union`, `intersection`, `difference`, `is_subset`, and the
  operators `|`, `&`, `-`.
* `[STD-12]` *(new in 0.9.9)* **Borrowed keys.** Lookup methods, `m[k]` and `k in m` accept any key
  type `Q` with `Q: AsKey[K]`: `K` itself, and `str` for `K = String`. `AsKey[K]` requires that `q` and
  `q.to_key()` hash and compare alike. `m[k] = v` with a `str` key converts it with `to_key()`, which
...
* `[STD-19]` *(new in 0.9.9)* Every `Iterator` has the adapters `map`, `filter`, `filter_map`,
  ...
  `for` loop compiles to one loop with no iterator object in memory (`[CTL-3]`).
...
* `[CLI-4]` `ember run file.em` and `ember build file.em` accept a single file with no manifest, as a
  package named after the file with default settings. A first program needs no manifest.
...
* `[CLI-20]` *(new in 0.9.9)* `ember fmt --migrate` rewrites source written for 0.9.8 into 0.9.9 where
  ...
  `Box[dyn fn]` to an owned callable type — and lists what it could not rewrite.
...
* `[PRF-1]` *(changed in 0.9.9)* **A profile never changes what a program means.** Every profile
  ...
  There are no exceptions. Only the following differ between profiles.
...
* `[TST-6]` Appendix A's code is generated from a fixture that is compiled in CI.
...
* `[DIA-21]` *(new in 0.9.9)* **Python habits.** Each of these is recognised and answered with the
  ...
  | `def f():` | `fn f():` |
  | `len(x)`, `str(x)`, `int(s)`, `float(s)`, `int(x)` | `x.len()`, `x.to_string()` or `f"{x}"`, `s.parse[int]()`, `s.parse[float]()`, `x as int` |
  | `range(n)`, `range(a, b)`, `range(a, b, s)` | `0..n`, `a..b`, `(a..b).step_by(s)` |
...
|---|---|---|---|
| O1 | use after move | `E3040` | `x.clone()` if `Clone`; else reorder so the last use precedes the move |
...
| N4 | scalar mismatch | the exact lossless `as`, and which side set the expected type |
...
* `[LNT-6]` *(new in 0.9.9)* `ember lint` also reports, at `warn` unless `[lints]` says otherwise:
  ...
  dynamic dispatch on a final type; `L5001` `unsafe extern` declaration with no contract; `L5002`
...
| `E2031` | `@borrows` names a parameter that is not view-typed | `[LT-1a]` |
...
| `E2072` | no method or field of that name (with the Ember name for a Python one) | `[STD-13]` |
...
| `E3060` | borrowed value does not live long enough | `[LT-3]` |
...
| `E5002` | foreign call requires `unsafe`: its contract is incomplete | `[FFI-2]` |
...
| `E7004` | value crossing a thread boundary is not `Send` | `[THR-10]`, `[THR-11]` |
...
| `W2015` | float literal loses precision in its type | `[LEX-17]` |
...
* `[CG-C-3a]` `@inline` is binding: the function is emitted with `__forceinline` or
  ...
  hints.
* `[CG-C-4]` **Aliasing facts.** For each loop, the base pointer of every view whose base and length
  are invariant in the loop is hoisted into a local declared `T *restrict` exactly when `[SIMD-3]`
  proves it disjoint from every other view the loop writes or reads; its length is hoisted into a local
...
* `[RT-10]` *(new in 0.9.9)* **Counting is inline.** The fast path of retain (one increment and an
  ...
  inlined into the caller; only deinitialisation is out of line. A `@sync` retain is one atomic
  `fetch_add`, never a compare-exchange loop. The deinitialising and resurrection checks of `[OBJ-5]`
...
| F-123 — Ember cannot call C at all today, and the C-import directive is silently ignored | SPEC | Manual declarations are module items; scalar-only functions are safe to call. | `[FFI-10]` |
...
| `[BRW-8]` | VII | A borrowed `Copy` parameter no larger than two pointers is passed by … |
...
| `[CLS-2]` | V | `fn init(self, …)` is the constructor. Every field without a default … |
...
| `[CTL-3b]` | VI | Iteration over `Span`, `MutSpan`, `Array`, `[T; N]`, `SoA` columns, … |
...
| Rule | Part | Begins |
|---|---|---|
| `[EXC-1]` | VIII | Beginning a write access to an object while any access to it is … |
| `[EXC-2]` | VIII | Beginning a read access while a write access is active panics … |
...
| `[EXC-15]` | VIII | A `mut self` class method holds a write access to the whole object … |
...
| `[FFI-39d]` | Annex C | Re-entrant calls from C++ into the same Ember object are expected and … |
...
| `[GRM-2]` | III | A file contains only imports and items. A statement at file scope is … |
...
| `[HEAP-4]` | IX | `s.get() -> ref T` borrows the payload; the loan keeps the block … |
| `[HEAP-5]` | IX | `s.get_mut() -> ref mut T` borrows it mutably with the same dynamic … |
| `[HEAP-6]` | IX | `Shared[T]` is `Copy`: copying retains, dropping releases. |
| `[HEAP-7]` | IX | `Weak[O]` exists for `O` a class handle or a `Shared[T]` (`[WK-11]`). |
...
| `[LEX-17a]` | II | A float literal that receives `f32` or `f16` and whose decimal value … |
...
| Rule | Part | Begins |
|---|---|---|
| `[LT-1]` | VII | Signature elision. For a function whose result is a reference or … |
...
| `[LT-44]` | VII | For elision, an `Arena`, `FixedArena` or `ScopedArena` parameter … |
...
| `[OWN-6]` | VII | `mem.take(mut place) -> T` (leaves `Default`), `mem.replace(mut … |
...
| `[RC-4]` | VIII | A non-`Sync` class's counts use plain loads and stores; a `Sync` … |
...
| `[STD-11]` | XV | `Map[K, V, H = DefaultHasher]` is a hash map that iterates in … |
...
| `[THR-10]` | XI | `thread.spawn(f: owned fn() -> R) -> JoinHandle[R]` runs `f` on a new … |
...
| `[TYP-29]` | IV | For floats, `a // b` is `floor(a / b)` and `a % b` is `a - b * … |
...
| `[WK-11]` | VIII | `Weak(h)` creates a weak handle to a class object or `Shared` payload … |
