...
**Version:** 0.9.8_Hardened_2
**Immediate predecessor:** 0.9.8_Hardened_1 (immutable; preserved as the diff base)
**Status:** Frozen development target. Implementation and conformance evidence is **not** claimed; normative repository adoption still requires the recorded adoption gates.
    ...
    Specification    frozen for this target. ODR-018 is closed here by one
                     explicit cycle-analysis root shared by `inspect` and
                     `explain`. ODR-017's ownership API remains unchanged.
...
**What Hardened_3 changed.** It closes ODR-016 by giving the three rules that stated a
...
this language version; this file is `0.9.8_Hardened_2` because ODR-018 only
completes their explicit CLI/tooling boundary.**
...
## 0.9.8_Hardened_2 authority and evidence status

This file is the frozen development target selected by the owner ruling that
closes ODR-018. It carries the inherited `Shared[T]`/`Weak[O]` work from
0.9.8_Hardened_1 and the canonical cycle-analysis-root tooling contract after
its recorded source hash is accepted through
...
`as-received/ODR-018_Cycle_explanation_analysis_root.md`; ADR-038 records its
application here.
