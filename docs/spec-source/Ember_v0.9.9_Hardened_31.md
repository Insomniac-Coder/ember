...
**Version:** 0.9.9_Hardened_31
**Supersedes:** 0.9.9_Hardened_30, 0.9.9_Hardened_29, 0.9.9_Hardened_28, 0.9.9_Hardened_27, 0.9.9_Hardened_26, 0.9.9_Hardened_25, 0.9.9_Hardened_24, 0.9.9_Hardened_23, 0.9.9_Hardened_22, 0.9.9_Hardened_21, 0.9.9_Hardened_20, 0.9.9_Hardened_19, 0.9.9_Hardened_18, 0.9.9_Hardened_17, 0.9.9_Hardened_16, 0.9.9_Hardened_15, 0.9.9_Hardened_14, 0.9.9_Hardened_13, 0.9.9_Hardened_12, 0.9.9_Hardened_11, 0.9.9_Hardened_10, 0.9.9_Hardened_9, 0.9.9_Hardened_8, 0.9.9_Hardened_7, 0.9.9_Hardened_6, 0.9.9_Hardened_5, 0.9.9_Hardened_4, 0.9.9_Hardened_3, 0.9.9_Hardened_2, 0.9.9_Hardened_1, 0.9.8_Hardened_3 (development target) and 0.8.5_Hardened_1 (adopted). This document is
...
are tracked separately (§XVII.5, §XVII.8) and are not claimed by this text.
...
* `[LEX-2]` *(changed in 0.9.9)* Line endings are LF or CRLF, both normalised to LF before
  tokenisation. The formatter always writes LF (`[FMT-1]`); output never depends on the host platform
  (ODR-060).
...
* `[LEX-15]` *(changed in 0.9.9)* The table above is the complete reserved set. **Contextual
  ...
  version (`E0005`), so `Set` can have its `union` method (`[STD-16]`). The reserved word `not` is
  also a method name immediately after `fn` and after `.`, so `Not` can have its method `not`
  (`a.not()` is `~a`, ODR-040). **Reserved for a future version** (lexed as keywords; using one as a
  name is `E0005`, whose message names the reservation): `async`, `await`, `macro`.
...
* `[GRM-39]` *(new in 0.9.9)* **Nesting depth.** Every compiler accepts expressions, statements,
  patterns and types nested 256 levels deep, counting each operand, argument, element, bracket, block
  and type argument as a level; an operator or method-call chain is as deep as it is long
  (`1 + 1 + … + 1` is left-nested). A compiler may accept more and states how much (this one, 1,024).
  A program nested more deeply than the compiler accepts is `E0112`, reported once where the limit is
  passed; the compiler never ends without a diagnostic (ODR-070).
...
| **View** | `ref T`, `Span[T]`, `MutSpan[T]`, `str`, any struct holding one | inline pointer(s) + compile-time region | copy (`MutSpan`, `ref mut`: move) | a plain view trivially; a guard ends its access (`[DRP-6]`) |
...
* `[TYP-27]` *(new in 0.9.9)* `()` is the value of type `void`; `Ok(())` is the success value of `Result[void, E]`. A
  function with no `->` returns `void`. Written as a type, `()` is `void`: a tuple type has two or more
  elements (ODR-071).
...
* `[TYP-34]` *(new in 0.9.9)* *(replaces the 0.9.8 `@view` requirement)* A struct, enum or tuple that contains a
  ...
  is `E2030`. A view type may also need drop, as a guard does (`[DRP-6]`).
* `[TYP-15]` *(changed in 0.9.9)* **Where views may be stored.** A view value may be stored only in a
  ...
  heap-collection elements (an `Array`'s, a `Map`'s keys and values, a `Set`'s elements), a span's
  elements and `owned fn` captures are not bounded, so they may hold only views whose every region is
  `static` — which admits string literals, `bytes` literals and views of `static` items: `names =
  ["ann", "bob"]` is an `Array[str]` of static strings, and a view copied out of such storage is
  `static` too. Any other stored view is `E3063 stored view may not outlive its source`, shape B12.
  The rule is about the stored value, wherever the store happens (ODR-069):
  * a view stored through a `mut` parameter or a reference is stored in the place it points to, which
    carries the view's regions afterwards (`[LT-21]`): `mem.replace(name, v)` and `put(name, v)` for
    `fn put(mut x: str, v: str): x = v` leave `name` borrowing what `v` borrows;
  * a function that stores a view it was given where only a `static` view may go, or into the place a
    `mut` parameter points to, passes that to its callers: a call must give it a `static` view there
    (`E3063` at the call otherwise), or is taken to store the view in the argument's place. So a
    generic `Map[str, V].insert(k, v)` takes a literal key and refuses one borrowed from a local;
  * a function called where its callers are not checked — a virtual method, a closure, a function
    value, a `dyn` interface's method — must store only `static` views there, or views already in
    that place.
...
* `[TYP-41]` *(new in 0.9.9)* **Const arguments.** A `const` generic argument, like an array length,
  is any expression compile time evaluates (`[CT-1]`) over constants and the enclosing declaration's
  own `const` parameters: `[f32; rows * cols]` and `zeros[side * side]()` are as legal as a literal,
  and a function may compute one. Whether two const arguments are equal, and so whether two types
  that differ only in them are one type, is decided by these rules and no others (ODR-066):
  1. An argument whose `const` parameters are all known (every argument of an instance, after
     substitution) is its value, evaluated as `[CT-1]` evaluates; two are equal when their values
     are. An evaluation that overflows or panics is `E6004`, and one past `[CT-3]`'s limits `E6001`,
     where the argument is written; neither is ever reported as a type mismatch.
  2. In generic code, an integer argument that names the declaration's `const` parameters is compared
     by its **normal form**: a sum of terms, each an integer coefficient times a product of
     parameters and opaque terms, with like terms collected and zero terms dropped, computed in
     mathematical integers. `+`, `-`, multiplication and parentheses are what the normal form sees
     through; any other operation (`/`, `%`, a shift, a comparison, a call) is an **opaque term**,
     equal to another only when it is the same operation, or a call of the same function, on
     arguments whose normal forms are equal. Two arguments are equal when their normal forms are the
     same: `N + 1` and `1 + N` are, and `2 * N` and `N + N`; `N / 2 * 2` and `N` are not. An argument
     of another type is equal to another only when both are the same parameter.
  3. Arguments rules 1 and 2 do not show equal make different types (`E2020`). When either names a
     parameter, the diagnostic says the two could not be shown equal, rather than that they differ;
     it prints both normal forms and suggests computing the value once, in one shared expression or a
     `const` parameter taken for it.

  The rules depend only on the source, never on what an optimiser can prove. Each instance
  evaluates its arguments again (rule 1), so an equality rule 2 accepts holds at every instance whose
  arguments evaluate; an instance whose arguments overflow is `E6004` there.
...
| `Send` | every field is `Send`; raw pointers are not; `ref T` and `Span[T]` are when `T` is `Sync`, `ref mut T` and `MutSpan[T]` when `T` is `Send` (`[THR-8]`); a class handle is `Send` iff the class is `Sync` |
...
interface Ord: Eq:
    fn cmp(self, other: Self) -> Ordering            # sort, min, max; a float's `<` stays IEEE
...
* `[TYP-37]` *(new in 0.9.9)* Floats implement `Eq` with IEEE `==` (so `NaN != NaN`) and `Ord` with the IEEE-754
  ...
  *operators* on float values keep IEEE meaning (`[TYP-9]`) in concrete and generic code alike: `a < b`
  with `T: Ord` and `T = f64` is IEEE `<`, false when either side is NaN. `cmp`, `sort()`, `sorted()`,
  `binary_search`, `min()` and `max()` use `cmp`, so sorting floats is total and needs no extra step.
  A float's `==` and `cmp(…) == Equal` can differ (`-0.0 == 0.0`, which `cmp` orders; a NaN is not `==`
  itself), so an algorithm keeps to one relation: the ordered ones to `cmp` throughout, the
  equality-based ones (`contains`, `index_of`, `dedup`) to `==` (ODR-055). Floats do not implement
  `Hash`; a float map key is written with `f.to_bits()`.
...
* `[TYP-26]` *(changed in 0.9.9)* There is no overloading: two functions of one name in one scope are
  ...
  compiler-known output functions `print`, `println`, `eprint`, `eprintln` (`[STD-9]`) and `format_to`
  (`[STD-18]`) are the only calls with a variable number of arguments (ODR-060).
---
...
| `a < b` etc. | comparison | scalars built in, in generic code too (a float's is IEEE, `[TYP-37]`); other types through `Ord.cmp`; chains (`[GRM-25]`) |
...
* `[TYP-38]` *(new in 0.9.9)* **Collection literals.**
  ...
  * A map literal `{k: v, …}` is a `Map[K, V]` (insertion-ordered, `[STD-11]`); a set literal
    ...
    a map written with no type owns its text; it holds `str` only where the program says so
    (`labels: Map[str, int]`, `[STD-11]`).
...
* `[EFF-1]` Effects are computed from each function's body and its callees', over the call graph, with
  recursive groups taking the union of their members. An operation's effects are its own and those of
  everything it runs where it runs: the methods and closures it calls (a `Display` a formatter
  invokes, a closure an adapter applies), the copies, retains, releases and drops it performs, and the
  argument and default expressions it evaluates. Calling a generator runs none of its body; each
  `next` carries the effects of the body up to the next `yield`, and dropping a suspended one those
  of the drops it performs (ODR-057).
...
* `[EFF-7]` `@assume_noalloc(call)`, an expression admitted only inside `unsafe` (`E0104` elsewhere),
  has the call's type and value and overrides the analysis for that one call, which the compiler
  cannot see into: the call is taken to carry no `Alloc`, and the programmer is answerable for it, as
  for any `unsafe` obligation (`[UNS-7]`) (ODR-060).
...
* `[EFF-16]` *(changed in 0.9.9)* `@nopanic(explicit)` forbids only the panics the programmer writes and
  ...
  allowed under it; the diagnostics and error page for `@nopanic(explicit)` MUST say so. A function
  that is `@nopanic(explicit)` **and** free of `RuntimeCheck` — two facts `ember inspect --safety`
  reports together — has no panic the effect model knows of; it can still stop on what the model does
  not cover (`[PHIL-11]`: exhaustion outside `Alloc`, a hardware fault, foreign code breaking its
  contract) (ODR-057). (`NonZero[T]` divisors and range facts remove division
...
* `[DET-4]` `std.math.det` provides `sin`, `cos`, `tan`, `exp`, `log`, `pow`, `atan2` and `sqrt` for
  `f32` and `f64` returning the same bits on every supported target. Each takes either type and
  answers in it (`det.sin(x)` of an `f32` is an `f32`); an `f32`'s result is its `f64` result
  rounded once. Each is within one unit in the last place of the exact result, `atan2` within 1.3;
  `sqrt` is IEEE 754's, correctly rounded. An integer is converted by the program (ODR-046).
...
* `[THR-8]` *(new in 0.9.9)* A type is **`Send`** when a value of it may be moved to another thread.
  ...
  when `T` is `Send` and `Sync`: the last release may drop the `T` on any thread (ODR-052).
* `[THR-9]` *(new in 0.9.9)* A type is **`Sync`** when several threads may read one value of it at
  ...
  `Atomic[T]`, `Once`, `Mutex[T]` for a `Send` `T` (one thread at a time holds its `ref mut T`),
  `RwLock[T]` and `SyncShared[T]` for a `T` that is `Send` and `Sync` (several threads hold a `ref T`
  at once: `RwLock[Cell[int]]` is not `Sync`), and handles of `@sync` classes are `Sync` (ODR-052).
* `[THR-1]` *(changed in 0.9.9)* **A class is `Sync` only when it is declared `@sync class`.** Every
  ...
  In a `@sync` class:
  * every field's type is `Sync` and `Send`, since the last release may drop it on any thread
    (`E7001`, naming the field and the component that is not) (ODR-052);
...
* `[THR-6]` *(changed in 0.9.9)* A type whose `drop` a safety guarantee depends on is `@must_drop`. A
  ...
  applies it to `Scope` and `JobScope` only. It is structural: a struct, enum, tuple or generic type
  that owns a `@must_drop` value is `@must_drop` too, whatever its own `drop` does, so wrapping one
  hides nothing; a reference to one is not, its region bounding it instead (`[TYP-15]`). A `Scope` or
  `JobScope` cannot be moved into a wrapper at all (`[THR-5]`) (ODR-054).
...
* `[PAR-2b]` The analysis reads through the body's calls as if inlined: each call to a function
  `[CG-C-3]` places in the inline header, recursively until a call is recursive — never according to
  what the C compiler chose to inline. A call it cannot see into that receives a captured `MutSpan`
  or column defeats the proof for everything reachable through that argument (`E7010`) (ODR-062).
...
* `[SIMD-5]` *(changed in 0.9.9)* **Vectorisable form**, which `@simd(assert)` checks, is computed by
  ...
  to a function `[CG-C-3]` places in the inline header, read through as `[PAR-2b]` says, whatever
  the C compiler then does (ODR-062); the body has none of `Alloc`, `Sync`, `Block`, `Io`, `FFI`,
...
* `[ERR-1]` *(changed in 0.9.9)* `Option[T]` is absence: `Some(v)` or `None`. `Result[T, E = AnyError]`
  is success or failure: `Ok(v)` or `Err(e)`. Both are ordinary enums in the prelude, and neither
  parameter is bounded: `Result[int, int]` is a `Result`. An operation that needs more of `E` says so
  (ODR-050): `?` into another error type needs `From[E]` (`[ERR-2]`), printing one needs `Display`,
  walking its causes needs `Error` (or is `AnyError`'s own, `[ERR-8]`), and converting one into
  `AnyError` needs `E: Error`.
...
* `[ERR-10]` *(new in 0.9.9)* `r.context(msg)` on a `Result[T, E]` whose `E` converts into `AnyError`
  (`AnyError: From[E]`, `[ERR-8]`) returns a `Result[T, AnyError]`
...
* `[ERR-12]` *(new in 0.9.9)* An `Err` returned from `main` (`[FN-8]`) prints `error: <Display>` to
  standard error, then one `caused by: <Display>` line for each cause in its chain — an `AnyError`'s
  own chain (`[ERR-8]`), or each `source` of an `E: Error` — and exits with status 1. An `E` that is
  `Display` but not `Error` has no chain and prints its message alone (ODR-050).
...
fn sine_table() -> [float; 256]:
    t = [0.0; 256]
    for i in 0..256:
        t[i] = det.sin(TAU * i as float / 256.0)
    return t
...
* `[CT-4]` *(changed in 0.9.9)* Compile-time evaluation is deterministic by construction: the evaluator
  has no source of `Nondet` (`[DET-2]`). A transcendental function runs at compile time when the
  source names `std.math.det`'s, with the same result it gives at run time; the platform library's
  `sin` is `Nondet` and does not run at compile time, and the evaluator never puts `det.sin` in its
  place (ODR-056). The evaluator computes integer and floating-point results exactly as the target does, including overflow panics
...
* `[DRV-1]` *(changed in 0.9.9)* `@derive(…)` requests generated implementations. The built-in derives,
  ...
  | `Ord` | lexicographic over fields in declaration order, each by `cmp`, the next only when it gives `Equal`; enums by variant order, then fields |
...
* `[STD-18]` *(new in 0.9.9)* `format(value, spec="") -> String` formats one value with a format spec
  ...
  into a caller's buffer; its own machinery never allocates, so the call allocates only if a value's
  `Display` does (`[EFF-1]`). An f-string lowers to a `StringBuilder` sized from its
  literal parts.
...
* `[STD-15]` *(new in 0.9.9)* **`Array[T]`** is a growable contiguous list (Python's `list`).
  ...
  `index_of(x) -> Option[int]`, `first`, `last`, `get(i) -> Option[ref T]`, `get_mut`,
  `get_pair_mut(i, j) -> Option[(ref mut T, ref mut T)]` (`[BRW-5]`), `iter`,
...
* `[STD-11]` *(new in 0.9.9)* **`Map[K, V, H = DefaultHasher, A = Global]`** is a hash map that **iterates in
  ...
  same with no values. Keys, values and elements may be views under `[TYP-15]`'s rule for heap
  storage: only `static` ones, such as string literals (`labels: Map[str, int] = {"start": 1}`), and a
  view that borrows a local is `E3063` at the call that would store it (ODR-069). A map of `static`
  views is not itself a view. Because order is fixed by the program's operations and the default hasher is
...
* `[STD-12]` *(new in 0.9.9)* **Borrowed keys.** Lookup methods, `m[k]` and `k in m` accept any key
  ...
  `std.collections`, not the prelude) has `is_key(self, key: K) -> bool`; `interface ToKey[K]:
  AsKey[K]` adds `to_key(self) -> K`, which makes the key (ODR-067). A lookup hashes `q` and compares
  it with `is_key`, so it never converts or copies a key. `m[q] = v` needs `Q: ToKey[K]` and calls
  `to_key()` only when the key is new, never to replace the value of an existing one. Every
  `K: Eq + Hash` is `AsKey[K]` (`is_key` is `==`), and `ToKey[K]` when it is also `Clone` (`to_key`
  clones): a key type that cannot be cloned is looked up and removed by value and inserted with
  `insert`, which moves the key in. std implements `AsKey[String]` and `ToKey[String]` for `str`. An
  implementation MUST hash and compare as the key it matches would, and `to_key()` MUST make that
  key; one that does not gives `[HASH-3]`'s wrong answers, never undefined behaviour.
* `[STD-17]` *(new in 0.9.9)* **Index assignment.** `a[i] = v` calls `IndexSet.index_set(i, v)` when
  ...
  always goes through `IndexMut`. `Array` and `MutSpan` implement `Index[int]` and `IndexMut[int]`,
  and `Span` `Index[int]`, through their built-in indexing; `Map[K, V]` implements `Index[Q]` and
  `IndexMut[Q]` for every `Q: AsKey[K]`, and `IndexSet[Q, V]` for every `Q: ToKey[K]` (blanket
  implementations, `[GRM-34]`, ODR-042, ODR-067).
...
* `[STD-19]` *(new in 0.9.9)* Every `Iterator` has the adapters `map`, `filter`, `filter_map`,
  ...
  `collect[C]()`, `to_array()`,
  and `join(sep)` for `Display` items. Adapters are lazy and allocate nothing themselves; what the
  closures they apply do is the loop's effect (`[EFF-1]`). A chain of adapters in a
...
* `[STD-5]` *(changed in 0.9.9)* `sum` and `product` combine elements left to right in the element
  ...
  `f32` elements in `f64`, and `math.KahanSum` gives compensated summation: an `f64` accumulator with
  Neumaier's correction, `KahanSum.new()`, `add(x)` and `value()`, and `KahanSum.of(xs)` for a
  `Span[f64]` (ODR-043).
...
* `[STD-20]` *(new in 0.9.9)* Integer methods, built into every integer type: `abs` (a signed
  ...
  least) (ODR-039). Every float type has the constants, `f16` included (`f16.MAX` is 65504 and
  `f16.EPSILON` 2^-10), though only `f32` and `f64` have the methods (ODR-041). A float's `Display` is the shortest text that reads back as the same value, with `.0`
...
* `[STD-4]` `NonZero[T]` for each integer `T` is a `Copy` wrapper with a niche (`Option[NonZero[T]]` is
  ...
  division-by-zero check. It is `std.core`'s, imported by name. `get()` returns the integer; the field
  and the memberwise constructor are private, so no `NonZero` holds 0. `T` is one of the twelve
  integer types, and any other type is `E2040`. `x // d` and `x % d` take `d: NonZero[T]` for `x: T`
  and answer a `T`; an overflow still panics (`[TYP-8]`) (ODR-047).
* `[STD-21]` *(new in 0.9.9)* `std.math` provides `PI`, `TAU`, `E`; `sin`, `cos`, `tan`, `asin`, `acos`,
  ...
  `[STD-27]`): an integer's answer is an `f64` (`math.sqrt(9)` is `3.0`), an `f16`'s and an `f32`'s
  an `f32`, and an `f64`'s an `f64`; and the vector, matrix, rotation and shape types of `[STD-28]`,
  with `dot`, `cross`, `length`, `normalize` (panics on a zero vector; `normalize_or_zero` does not)
  and the arithmetic operators. `PI`, `TAU` and `E` are
...
* `[STD-27]` *(new in 0.9.9)* `std.math.Number` is every number type. `std.math` adds each with an
  ...
  come back as (`f64` for every integer type, `f32` for `f16`, which it holds exactly (ODR-041), and
  itself for `f32` and `f64`), and `to_real`, which turns it into one. `[STD-21]`'s functions take `T: Number` and answer in `T.Real` (`[IFC-4]`); a program
...
* `[STD-28]` *(new in 0.9.9)* **Vectors, matrices, rotations and shapes** (ODR-043). `std.math` has
  `Vec2`, `Vec3`, `Vec4` (`f32` components `x`, `y`, `z`, `w`), `IVec2/3/4` (`i32`), `UVec2/3/4`
  (`u32`), `Mat2`, `Mat3`, `Mat4` (`f32`, column-major: the columns are `x_axis`, `y_axis`,
  `z_axis`, `w_axis`, and a transform's translation is `w_axis`), `Quat` (`x`, `y`, `z`, `w`, `w`
  the scalar part), `Transform` (`translation`, `rotation`, `scale`), `Aabb` (`min`, `max`),
  `Sphere` (`center`, `radius`), `Ray` (`origin`, `direction`), `Plane` (`normal`, `d`: the points
  `p` with `normal.dot(p) + d == 0`) and `Frustum` (six planes facing in: `left`, `right`, `bottom`,
  `top`, `near`, `far`). Each is `@layout(c)` and `Copy`, with a public memberwise constructor.
  * Vectors have the constants `ZERO`, `ONE` and the unit axes (`X`, `Y`, `Z`, `W`); `splat`;
    component-wise `+`, `-`, `*`, `/` (`//` and `%` for the integer ones), the same with a scalar
    on either side (`2.0 * v`, `[TYP-21]`), and unary `-` (not `UVec`); `min`, `max` and `abs` (not
    `UVec`), component-wise; `dot`; for the float vectors `length`, `length_squared`, `distance`,
    `distance_squared`, `normalize`, `normalize_or_zero`, `lerp` and `mul_add` (component-wise,
    each rounded once, `[STD-3]`); `Vec3.cross`; and `extend` and `truncate` between sizes.
  * Matrices have `IDENTITY`, `ZERO`, `from_cols` and `from_diagonal`; `m * n` (which applies `n`
    first), `m * v`, `m * s` and `s * m` for an `f32` `s`, `+`, `-` and unary `-`; `transpose`,
    `determinant`, `col(i)`, `row(i)`, `inverse` (a matrix with none panics) and `try_inverse` (an
    `Option`). `m * v` is `m.x_axis * v.x + m.y_axis * v.y + …`, each term after the first added
    by a fused multiply-add (`[STD-3]`). `Mat2` and `Mat3` have `from_angle` (a rotation in the
    plane); `Mat3` and `Mat4` `from_quat` and `from_scale`; `Mat4` `from_translation`,
    `from_rotation_x`/`y`/`z`, `from_scale_rotation_translation`, `look_at_rh`, `perspective_rh`
    and `orthographic_rh` (right-handed, depth in `[0, 1]`), `transform_point3` (`w` = 1, divided
    by the result's `w`) and `transform_vector3` (`w` = 0).
  * Rotations are right-handed. `Quat` has `IDENTITY`, `from_axis_angle` and
    `from_rotation_x`/`y`/`z`; `q * p` (the rotation `p`, then `q`) and `q * v` (the vector
    rotated); `length`, `normalize`, `conjugate`, `inverse`, `dot` and `slerp`. `Transform` has
    `IDENTITY`, `from_translation`, `from_rotation` and `from_scale`; `transform_point` (scale, then
    rotate, then move), `transform_vector` (without the move), `t * u` (`u`, then `t`) and
    `to_mat4`.
  * `Aabb` has `from_center_half_extents`, `center`, `half_extents`, `contains`, `intersects`,
    `union` and `expand`; `Sphere` `contains`, `intersects` and `intersects_aabb`; `Ray` `at(t)` and
    `intersect_plane`, `intersect_sphere` and `intersect_aabb`, each the least `t ≥ 0` at which the
    ray is on the plane or in the ball or box (0 when it starts there), as an `Option[f32]`; `Plane`
    `from_point_normal`, `from_points`, `signed_distance` and `normalize`; `Frustum`
    `from_view_projection(m)`, for `m` a `perspective_rh` or `orthographic_rh` projection times a
    view, with `contains_point`, `intersects_sphere` and `intersects_aabb`.
...
* `[FFI-10]` *(changed in 0.9.9)* An `unsafe extern "C":` block declares foreign functions, statics
  ...
  On a hand declaration, `@ffi(param(name, word, …), result(word, …))` attaches `[FFI-11]`'s
  contract words to each named pointer parameter and the pointer result (ODR-073). Each slot is named
  at most once; `link_name` and `effects` can share the same `@ffi`. A `safe fn` writes the safe Ember
  types or `mut` parameter modes produced by those words; the compiler checks that
  mapping and lowers it to the C ABI carrier. A raw `*T` can be declared for an unsafe call, but
  complete facts do not make a raw-pointer parameter or result safe to expose; `safe fn` with one is
  `E5002`. Missing facts or a type/contract mismatch is `E5002`, naming the slot.
  On a hand-declared `safe fn` with `count(n)`, `n` is written as an integer scalar parameter at
  its C ABI position (ODR-074). The contract marks it as an ABI witness: it is absent from the
  callable Ember signature, and the compiler supplies the corresponding `Span[T]` or `MutSpan[T]`
  length at the call. The written integer type fixes the C width and signedness. A length that does
  not fit that type, or unequal lengths from two pointer clauses sharing `n`, panics before the
  foreign call. A missing or non-integer witness or an incompatible pointer surface is `E5002`.
  An unsafe raw-pointer declaration keeps its written C ABI parameters; only a mapped `safe fn`
  erases the witness.
...
* `[FFI-11]` *(changed in 0.9.9)* **Contract vocabulary.** A pointer contract has five axes;
  mutability comes from `const` and is never written. The same words appear after a parameter name
  in an overlay and inside `@ffi(param(name, word, …), result(word, …))` on a hand declaration
  (ODR-073); that attribute does not create a separate contract system.
  ...
  | count | `one`, `count(n)` (as many elements as parameter `n`, which then leaves the callable signature; a hand declaration writes its ABI witness under `[FFI-10]`, ODR-074), `nul_terminated`, `fixed(N)`, `inout_count(p)` (`[FFI-11b]`) | `ref T`, `Span[T]`/`MutSpan[T]`, `cstr`, `ref [T; N]` |
...
* `[BLD-2]` A module's front-end result is keyed by its source, the compiler and language versions,
  ...
  effect sets, inline bodies), so editing a private body recompiles that module and re-checks its
  callers only if a fact they use changed: its effect set (`[EFF-4]`), an inline body, or a borrow
  summary a caller relies on (a view result's sources, `[LT-22]`, `[LT-35]`; a method's field set,
  `[BRW-10]`) (ODR-058).
* `[BLD-8]` Within a module, editing one function body re-checks that function, and those of the
  module that use a fact of it that changed (`[BLD-2]`).
...
| B1 | two mutable borrows of one place, e.g. two indices | `E3022` | `get_pair_mut(i, j)`, `split_at`, `chunks_mut`, `iter_mut`, `swap(i, j)`; `SoA` columns |
...
* `[FMT-1]` *(changed in 0.9.9)* `ember fmt` writes LF line endings, four-space indentation and at most 100
  columns, whatever the platform. It changes layout only: never a bound, a mode, a capture, a clone
  or anything else that changes what a program means; rewriting source is `ember fmt --migrate`'s,
  which lists what it changed (`[CLI-20]`) (ODR-059).
...
| `E2103` | assignment to `self` in a class method | `[CLS-7]` |
...
| ODR-071 | the type `()` is `void`; a tuple type has two or more elements (Hardened_29) | `[TYP-27]` |
| ODR-072 | in a class method `self` is the object the method was called on for the whole call; assigning to it is `E2103`, and re-pointing a caller's handle is a `mut` parameter's (Hardened_29) | `[CLS-7]`, `[FN-9]`, `[EXC-15]` |
| ODR-073 | hand-declared pointer facts use `@ffi(param(name, words…), result(words…))`; a `safe fn` exposes the mapped safe Ember type, never a raw pointer (Hardened_30) | `[FFI-10]`, `[FFI-11]` |
| ODR-074 | `count(n)` on a hand-declared safe C function writes `n` at its C ABI position as an integer witness, hides it from the callable signature, and supplies it from a checked span length (Hardened_31) | `[FFI-10]`, `[FFI-11]` |
...
| `[EFF-7]` | X | `@assume_noalloc(call)`, an expression admitted only inside `unsafe` … |
...
| `[ERR-10]` | XIII | `r.context(msg)` on a `Result[T, E]` whose `E` converts into … |
...
| `[FN-1a]` | V | A `mut` parameter whose type is itself a view (`MutSpan[T]`), the … |
...
| `[GRM-39]` | III | Nesting depth. Every compiler accepts expressions, statements, … |
...
| `[OWN-7]` | VII | A `Copy` value may be duplicated on use and the source stays valid. … |
...
| `[PAR-2b]` | XI | The analysis reads through the body's calls as if inlined: each call … |
...
| `[PHIL-3]` | I | No implicit copy of a non-`Copy` value occurs. A copy of a `Copy` … |
...
| `[STR-3]` | V | A struct with a `drop` method, or with a field that is not `Copy`, is … |
...
| `[TYP-41]` | IV | Const arguments. A `const` generic argument, like an array length, is … |
