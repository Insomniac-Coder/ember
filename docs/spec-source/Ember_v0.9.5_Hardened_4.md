...
**Version:** 0.9.5_Hardened_4
**Status:** Frozen development target; implementation/conformance evidence is not claimed and normative repository adoption is blocked by the source-recovery item below
**Immediate predecessor:** 0.9.5_Hardened_3
...
> **SOURCE RECOVERY REQUIRED — `[LT-8]`–`[LT-13]`:** the received H3 file says these
> `std.borrow.with_views2/3/4` rules were recovered, but it contains no definition of any of the six
> rules. Their exact text must be recovered from an authoritative predecessor or supplied by the
> owner before this target can be adopted as the normative repository specification. No implementation agent may infer the missing
> signatures, mutability combinations, callback-result constraints, allocation guarantees, or
> escape behavior. Because this H4 identity is frozen, that later source repair must be issued as
> `0.9.5_Hardened_5`. See `docs/OWNER-QUEUE.md` ODR-004.
...
9. `[LT-8]`–`[LT-13]` are source-recovery blockers for the `std.borrow.with_views2/3/4` helper surface: the supplied lineage describes their intent but contains none of their definitions; this gap does not restore the former single-region `@view struct` restriction, which `0.9.5_Hardened_1` independently supersedes;
...
exclusivity. The surviving record says its `std.borrow.with_views2/3/4` helpers remain valid and
useful, but their exact `[LT-8]`–`[LT-13]` definitions are missing and are not reconstructed here.
...
The supplied lineage intends `std.borrow.with_views2/3/4` to remain conforming library APIs and
describes them as a predecessor mechanism for independently-lived views. However, the definitions
of `[LT-8]`–`[LT-13]` are absent. Until those definitions are recovered, this section records intent
and provenance only; it cannot establish the helpers' exact API or semantics. `0.9.5_Hardened_1`
separately adds inferred multi-region `@view struct` semantics below. Once recovered, the helpers
MUST NOT be used as a reason to reject a conforming multi-region `@view struct`.
...
| AUD-014-03 | High | The historical record says independent view lifetimes remained forbidden in storable `@view struct`s while canonical `std.borrow.with_views` helpers provided allocation-free late-bound callback composition; the exact helper definitions are the H4 source-recovery gap. |
...
tooling-structure, and inherited editorial repairs. The only unresolved item is the exact normative
text of LT-8 through LT-13; it is not supplied by this amendment.

### 0.9.5_Hardened_4 language-conformance condition

`0.9.5_Hardened_4` MUST NOT be declared conformant unless the definitions of `[LT-8]`–`[LT-13]`
have first been recovered; `[LT-14]`–`[LT-43]` including `[LT-31a]`, `[MIR-REG-1]`,
...
This revision incorporates the available Hardened_14 hardening record except for the explicitly
missing `[LT-8]`–`[LT-13]` source, the accepted 0.9.5 multi-region-view language revision, and the
...
`0.9.5_Hardened_4` MUST NOT be declared clean unless:
...
3. the active revision lineage ends at `0.9.5_Hardened_4`;
4. the current consistency checklist targets `0.9.5_Hardened_4`;
...
10. the source-recovery blocker for `[LT-8]`–`[LT-13]` is closed without inferred semantics.

---

## 0.9.5_Hardened_4 Evidence Status — No Implementation Claim
...
## 0.9.5_Hardened_4 consolidation record
...
blocks conformance. It MUST NOT be fabricated from another language's behavior. LT-8 through
LT-13 are the current source-recovery gap.
...
# Appendix B — 0.9.5_Hardened_4 Consistency Checklist

The following checks are mandatory CI checks for the consolidated specification itself:

1. Exactly one current version declaration exists and names `0.9.5_Hardened_4`.
2. The immediate predecessor is `0.9.5_Hardened_3`.
...
12. `[LT-8]`–`[LT-13]` are present as recovered canonical library rules before this target is adopted and have regression coverage obligations.
