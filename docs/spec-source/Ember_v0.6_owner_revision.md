...
**Version:** 0.6 (supersedes 0.5; see the change log at the end of Part 0)
...
| 2 | **Contracts.** `@requires`, `@ensures`, `@invariant`, `@decreases` as attributes, checked at run time under a policy flag. | X.2a, `[CTR-1..8]` |
| 3 | **Verification.** `@verified` asks for a function's obligations to be discharged by an external solver. Proved, disproved, unknown; unknown is never success. | X.2b, `[PRV-1..10]` |
| 4 | **Proof manifest.** A machine-readable record of what was proved, by which solver, under which assumptions. | `[PRV-9]` |
| 5 | **Trusted-base report with graded claims.** Every foreign fact is *asserted*, *checked*, *instrumented* or *proven*, and every unproven assumption is listed. | XIX.6a, `[TCB-1..4]` |
...
1. **Contracts are attributes, not clauses.** `requires`, `ensures`, `invariant`
   and `decreases` are ordinary identifiers in 0.5 and appear in neither list in
   `[LEX-15]`. Clause syntax would break any existing program using one as a
   name; `@requires(...)` needs no lexical change and sits beside `@borrows`,
   which is already a contract in all but name.
...
3. **Contract arithmetic is always checked, in every profile** (`[CTR-4]`).
   Without this, `@ensures(result > x)` is true under `debug`'s panicking
   arithmetic and false under `release`'s wrapping, and `[PRF-1]` — one source,
   one verdict in every profile — becomes false.
...
* `[RNG-2]` Two range types are distinct types even when representation and
  ...
  implicitly; the reverse requires `[RNG-3]`.
...
* `[RNG-4]` The compiler tracks a known range for every numeric expression it
  ...
  never derived from inside a `@fastmath` function.
...
The compiler infers, for every function, an **effect set** ⊆ `{Alloc, Sync, Lock, Io, Panic, Unsafe, FFI, Block, RuntimeCheck(k)}`. Effects are orthogonal: one operation MAY introduce several effects at once.
...
* `[EFF-18]` **Effects are orthogonal.** `Io` (file, socket, console) and `Lock`
  ...
  effect model so a single operation may report all applicable effects.
...
A contract states what a function needs from its caller and what it promises in
return, where the compiler can see it. Contracts are **attributes**: `requires`,
`ensures`, `invariant` and `decreases` are ordinary identifiers under `[LEX-15]`
and spelling them as clauses would break existing programs.
...
* `[CTR-1]` `@requires(expr)` is a precondition, evaluated in the **caller's**
  frame immediately before the call so that a failure names the call site.
  `@ensures(expr)` is a postcondition, evaluated in the callee before each
  return. Either may appear more than once; the meaning is their conjunction.
* `[CTR-2]` Within `@ensures`, the name `result` denotes the returned value and
  `old(place)` denotes the value a place held on entry. Both are bound only
  inside that attribute and are ordinary identifiers everywhere else; `[LEX-15]`
  is unchanged and no keyword is added.
* `[CTR-3]` A contract expression MUST be free of the `Alloc`, `Block`, `Io`,
  `Sync`, `Lock`, `FFI` and `Panic(Explicit)` effects (`E4050`). It may read
  `let` fields, call functions whose effect set is empty, and index views. A
  contract that could allocate or wait cannot be checked on a frame path, which
  is where contracts are worth the most.
* `[CTR-4]` **Contract arithmetic is checked in every profile.** An overflow
  inside a contract expression is a contract failure, never a wrap, whatever
  `[TYP-8]`'s policy is for ordinary code. Without this rule `@ensures(result >
  x)` holds under `debug` and fails under `release`, and `[PRF-1]` — one source,
  one verdict in every profile — would be false.
* `[CTR-5]` `@invariant(expr)` on a `struct` or `class` MUST hold after every
  constructor and after every method taking `mut self`; on a loop it MUST hold
  on entry and after each iteration.
* `[CTR-6]` `@decreases(expr)` names a quantity that strictly decreases and is
  bounded below, on a loop or a recursive function. It is what allows `[PRV-1]`
  to say anything about a loop at all; without one, every loop is an unknown.
* `[CTR-7]` Contracts are part of a function's public interface. `[EFF-4]`'s
  caching rule applies. An `override` of a `virtual` method MUST NOT strengthen
  `@requires` nor weaken `@ensures` — the inheritance direction `[EFF-8]` uses
  for effects (`E4052`).
* `[CTR-8]` `--contracts=off|debug|all` selects where **runtime contract checks**
  are emitted. The default is `debug` for debug builds, `debug` for release builds,
  and `off` for shipping builds. This flag is an **independent instrumentation policy**,
  not a profile and not part of `[PRF-1]`'s semantic exceptions. A contract remains
  part of the program's declared interface and is still available to static analysis
  and `[PRV-*]` verification even when its runtime check is omitted. `--contracts=off`
  therefore means "do not dynamically instrument the contract", never "the contract
  does not exist". A contract failure is a bug in every setting. A contract discharged
  by `[PRV-1]` MAY omit its runtime check in every setting. `--contracts=debug` emits
  checks only for debug-oriented execution; `--contracts=all` emits checks in all build
  profiles. Runtime contract instrumentation MUST NOT alter accepted source, the
  meaning of non-failing execution, or the result of ordinary expressions.

* `[CTR-9]` **Dynamic contract dispatch.** For an interface, virtual, `dyn`, or
  function-value call, the caller checks the contract attached to the statically
  visible callable type. An overriding implementation MUST accept every state
  accepted by the visible precondition and MUST satisfy the visible postcondition;
  it MUST NOT strengthen `@requires` or weaken `@ensures`. An implementation MAY
  establish stronger postconditions internally, but those are not additional
  obligations on callers. This rule applies uniformly to direct interface dispatch,
  virtual dispatch, `dyn` calls and function values. The selected implementation is
  resolved before its body is analyzed, while the caller's safety obligation comes
  from the visible callable contract.

Diagnostics: `E4050` a contract expression with a forbidden effect; `E4051` a
contract that cannot be evaluated at its check point; `E4052` an `override`
strengthening a precondition or weakening a postcondition; `E4053` `@decreases`
on something that is neither a loop nor a recursive function; `E4054` a dynamically
dispatched callable whose implementation contract is incompatible with the visible
contract.

## X.2b Verification

`@verified` asks for a function's obligations to be discharged by proof before
the program runs. Where a proof succeeds the check is not emitted, which makes
this a performance rule as much as a safety one: `[OPT-2]` already removes the
bounds check from a counted loop whose shape the compiler recognises, and
verification generalises that to the loops it cannot see through, with checks
still present in a shipping build wherever nothing was proved.
...
* `[PRV-1]` `@verified` on a function submits its contracts, its `[TYP-8]`
  overflow checks, its bounds checks and its `[RNG-3]` constructions as proof
  obligations. Each obligation has exactly one outcome: **proved**, **disproved**
  or **unknown**. A solver timeout is unknown.
* `[PRV-2]` **Unknown is never success.** In a build that requires proof an
  unknown obligation fails the build (`E4061`). An unproven obligation MUST NOT
  be reported as proved and MUST NOT be silently downgraded to a runtime check.
* `[PRV-3]` Proof is **modular**: a function is proved against the declared
  contracts of its callees, never by inlining them. A call to a function with no
  contract contributes no facts.
* `[PRV-4]` Verification is an additional layer. The borrow checker, effect
  checker, move analysis, exclusivity analysis and definite-initialisation
  analysis run unchanged and no proof weakens any of them.
* `[PRV-5]` A proved obligation MAY be omitted from the emitted code. An
  obligation proved under an assumption (`[TCB-3]`) MAY be omitted only when the
  assumption appears in the manifest.
* `[PRV-6]` Obligations are emitted in SMT-LIB form for an external solver; the
  ...
  configuration are part of the build identity (`[BLD-2]`) and of the manifest,
  so the same source, compiler and solver yield the same verdict on another day
  and another machine.
* `[PRV-7]` A failed proof reports the obligation's source, the counterexample
  where the solver supplies one, and which of four things is responsible: the
  code, an insufficient contract on a callee, an explicit assumption, or the
  solver giving up.
* `[PRV-8]` `--verify=off|check|prove`. **Verification is not a profile axis and is
  independent of `--contracts`.** `off` performs no proof search. `check` runs the
  verifier and reports `proved`, `disproved` and `unknown` outcomes but does not gate
  compilation on those outcomes and MUST NOT turn unproved obligations into runtime
  checks. `prove` runs the verifier and requires every obligation attached to
  `@verified` to be `proved`; `disproved` and `unknown` fail the build. Verification
  never changes which branch is taken or what an expression evaluates to. Runtime
  contract instrumentation is controlled only by `[CTR-8]`; formal verification is
  evidence generation, not runtime instrumentation.
* `[PRV-9]` A build with `--verify=prove` writes `target/<profile>/verify.json`:
  ...
  version, the content hash of every module, one entry per obligation with its
  id, source span, outcome and elapsed time, every assumption relied upon, and
  the trusted components of `[TCB-4]`.
* `[PRV-10]` The manifest is evidence and a reproducibility record. The
  toolchain MUST NOT describe it as a certification artefact, and MUST NOT imply
  that a proved build is certified for any purpose.

Diagnostics: `E4060` an obligation disproved, with counterexample; `E4061` an
obligation unknown in a build requiring proof; `E4062` `@verified` on a
construct the obligation generator does not support — reported as unsupported,
never as proved.
...
* `[STD-7]` `core` provides `FixedArray[T, N]`, `FixedString[N]` and
  ...
  require const generics, which v1 does not have; a v1 implementation MAY
  provide them for a fixed set of capacities and MUST NOT pretend the general
  form exists.
...
* `[STD-7a]` General const-generic support for integer capacity parameters is a
  ...
  **const-generic milestone before the fixed-capacity container milestone**. A
  ...
  compile-time `N` is accepted. `[CG-1]` through `[CG-4]` govern parsing, constant
  evaluation, monomorphisation and identity of const-generic arguments.
...
* `[FFI-34]` **Every fact in an overlay carries a grade** from `[TCB-1]`, written
  ...
  analysed. Claiming a grade whose evidence is absent is `E5050`.
* `[FFI-35]` **A foreign reference or raw pointer imports unsafe.** `T&`,
  `const T&` and `T*` produce a foreign pointer requiring an `unsafe` block to
  use, until an overlay supplies the lifetime and aliasing facts that make it a
  `ref T`/`ref mut T`. A C++ header does not record how long a returned reference
  lives or who else may hold it, and mapping one to a safe borrow on the strength
  of its spelling is how a dangling pointer acquires the borrow checker's
  endorsement. This **narrows** `[FFI-11]`'s `borrowed`: that contract is now the
  promotion a human writes, not the default the importer applies (`E5051`).
* `[FFI-36]` **Ownership transfer is written at the call site.** A foreign call
  yielding an object Ember must destroy returns the existing raw/opaque foreign
  pointer representation (`*T`, `*mut T` or an `extern type` handle), not a second
  owned-wrapper type. That handle becomes an owned Ember value only through
  `adopt(handle)`, which produces `ForeignBox[T]`. The overlay declares whether
  adoption is permitted and which destructor releases the native object; it does not
  perform the transfer invisibly. Adopting a handle the overlay does not permit, or
  adopting the same native object twice, is `E5052`. `ForeignBox[T]` is the sole
  standard owned representation for a foreign object. There is no `Foreign[T]` type.
...
## XIX.6a Trusted base and audit reporting
...
* `[TCB-1]` Every foreign fact carries one of four **grades**: **asserted** (a
  ...
  **proven** (discharged by analysis of the adapter). A fact with no grade is
  asserted.
...
* `[TCB-3]` Every assumption a proof relied on and did not discharge appears in
  the report and in `[PRV-9]`'s manifest, with its source and its grade.
* `[TCB-4]` Entries are categorised: language, compiler, runtime, standard
  ...
  toolchain stands outside the question.
* `[TCB-5]` An `instrumented` fact is valid only for the exact foreign identity and
  ...
  `target/<profile>/ffi-evidence/` and is referenced by content hash.
* `[TCB-6]` Instrumented evidence becomes `stale` when any identity or input recorded
  by `[TCB-5]` changes. A stale record MUST NOT satisfy an `instrumented` claim,
  a verified build requirement, or `[FFI-37]`; the report MUST mark it `stale` and
  tell the user to rerun `[CLI-14]`. Evidence expiry is therefore content-based,
  not solely time-based.
...
* `[BLD-11]` A package declares which `[STD-6]` layers it links. Omitting `alloc`
  ...
  available symbol set; the resolver owns the user-facing missing-symbol diagnostic.
