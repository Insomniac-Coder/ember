...
* `[STD-21]` *(new in 0.9.9)* `std.math` provides `PI`, `TAU`, `E`; `sin`, `cos`, `tan`, `asin`, `acos`,
  ...
  `hypot`, `lerp`, `smoothstep`, generic over `f32` and `f64` (each `fn f[T: Float](…)`, `[STD-27]`);
  and the vector types `Vec2`, `Vec3`,
  ...
  type a use is `f64`.
* `[STD-27]` *(new in 0.9.9)* `std.math.Float` is the bound of the generic float functions. It is
  implemented by `f32` and `f64` and by no other type: `implements Float` anywhere else is `E2042`.
  A bound `T: Float` provides, inside a generic body, `Copy`, `Eq`, `Ord` (`[TYP-37]`) and `Default`;
  ...
  (`x.sqrt()`, `y.atan2(x)`, `a.hypot(b)`). Each of those free functions is its method: `math.sqrt(x)`
  is `x.sqrt()`; `rsqrt`, `lerp` and `smoothstep` are free functions only. A method is the C
...
| `[STD-27]` | XV | `std.math.Float` is the bound of the generic float functions. It is … |
