...
| `[TYP-13]` | IV | `Option[T]` has the size of `T` when `T` has a niche: a class handle, … | compliant (partial) | `NonZero[T]`, class and class-interface handles, `Box`, ordinary and dynamic `ref`, `bool`, `char`, unit enums with spare discriminants, integer and float ranges with a niche, `Span`, `str`, and C-ABI function pointers use payload-sized representations. `TYP-13/` and `RNG-7/` check layout and Some/None behavior. Other foreign ABIs remain for Phase 5. |
...
| `[FN-6]` | V | Callable types. A function is a value. A callable type is written … | compliant (partial) | A capture-free named function or lambda with FFI-safe parameter and result types coerces to a distinct `extern "C" fn` value; a capturing closure is rejected. `FN-6/` checks both. Borrowed non-`Copy` aggregates in extern function pointers are `E0900` until the C by-value ABI and Ember borrow semantics are reconciled; other foreign ABIs and owned callable values remain. |
...
| `[CLS-2]` | V | `fn init(self, …)` is the constructor. Before any `init` body runs, … | compliant (partial) | `class_inherited_defaults.em` and a side-effecting base/derived default probe confirm base-first default evaluation before either `init`, including a virtual call from base `init` that reads the derived default. Other constructor shapes remain to probe. |
...
| `[CLS-7]` | V | Inside a class method, `self` is a handle. Any method may read and … | fixed | D-362 lets a plain `self` method write its fields, with per-field checks for non-`Copy` fields; `CLS-7/` covers assignments, mutation, and an alias conflict. |
| `[CLS-8]` | V | A class is `Sync` only as `[THR-1]` allows; every field of a `Sync` … | compliant (partial) | Explicit `@sync` class fields are checked for `Send` and `Sync`, including generic instances; `THR-1/` covers rejected non-shareable fields, including a `let Cell`. Thread-boundary transfer is not yet implemented. |
...
| `[CTL-1]` | VI | `for pattern in e:` iterates: * a place `e` whose type is `Iterable`: … | partly | `Array`, `[T; N]`, `Span`, ranges, `Map`, `Set` and `str`/`String` (by `char`) iterate; `for x in owned e:` consumes an `Array`, `Set` or `Map` through `IntoIterator` (2026-09-26); generators do not exist yet |
...
| `[RC-4]` | VIII | A non-`Sync` class's counts, and a `Shared`'s, use plain loads and … | compliant (partial) | `@sync` class type info now selects the runtime's atomic count path; other class and `Shared` counts remain plain. `THR-1/accept_sync_class_uses_atomic_reference_counts.em` checks the emitted flag, but concurrent retain/release is not yet probed. |
...
| `[THR-1]` | XI | A class is `Sync` only when it is declared `@sync class`. Every other … | compliant (partial) | Explicit marker, field `Send`/`Sync` validation, immutable fields after `init`, prohibition of `mut self` methods, and matching base/derived markers are checked in `THR-1/`; thread spawn and synchronization primitives remain to implement. |
...
| `[FFI-1]` | XVI | Every foreign declaration has a contract: for each pointer-typed … | partial | ODR-073/Hardened_30 defines hand-declaration syntax and a safe mapped surface. Borrowed `one` covers shared and exclusive references, including nullable ones. ODR-074/Hardened_31 adds hand-declared `count(n)` for `Span[T]` and `MutSpan[T]`, with `n` hidden from callers and supplied from length. Other ownership, count, lifetime, and result forms remain explicit gaps. |
| `[FFI-2]` | XVI | A call to a foreign function whose contract contains an `unknown` … | partial | Hand-declared functions without `safe fn` require an enclosing `unsafe:` or `unsafe fn`. The four `TODO(axis)` facts are accepted on unsafe declarations, including several on one slot, and each unknown fact rejects a `safe fn` with `E5002`. A complete borrowed-one contract permits a safe C call; a raw pointer remains unsafe to expose. Header overlays remain. |
| `[FFI-10]` | XVI | An `unsafe extern "C":` block declares foreign functions, statics and … | partial | Functions are module items and link through the C ABI, including safe scalar calls, borrowed-one/nullable references, counted spans, unsafe calls, and capture-free function values. A counted safe declaration emits a wrapper and C prototype in source ABI order. Duplicate names, non-unsafe blocks, unsupported ABIs, missing or mismatched clauses, and borrowed aggregate ABI mismatches are rejected. Foreign statics/types, other contracts, arbitrary linker aliases, and header import remain. |
| `[FFI-49]` | XVI | `@ffi(link_name="sym")` binds a declaration to a differently named symbol. | partial | A hand-declared C function can bind a valid C identifier; emitted C and the module interface use the same symbol, including when `link_name` shares an `@ffi` with a pointer clause. Assembler-only linker names remain `E0900`. `FFI-49/` checks accepted and rejected forms. |
| `[FFI-8]` | XVI | Type mapping. | not yet probed | |
| `[FFI-11]` | XVI | Contract vocabulary. A pointer contract has five axes; mutability … | partial | Borrowed `one` covers shared/exclusive single places and nullable references; `aliased` is accepted for shared places. Borrowed `count(n)` covers `Span[T]`/`MutSpan[T]`: an integer ABI witness is erased from the callable signature, and the wrapper checks narrowing and shared-witness length equality before C. `E5012` identifies missing counts; four `TODO(axis)` words retain unsafe calls. 128-bit witnesses, nullable counted spans, other ownership/count forms, results, and header overlays remain gaps. |
...
|---|---|---|---|
| direct `unsafe fn` calls | `[UNS-1]`: safe code requires an `unsafe:` block, while an `unsafe fn` body permits the call | ordinary, method, and generic calls lacked one consistent gate | fixed for direct calls: `E3100` outside unsafe, with an `unsafe fn` body and an `unsafe:` block accepted; an unsafe function cannot coerce to a safe callable value (`E0900`). `UNS-1/` and `FFI-10/` cover these paths. |
| `@safety` on an unsafe function | `[UNS-7]`: public unsafe functions state caller obligations; missing text is `L3015`, placeholder `TODO` is `L3016` | attribute rejected as unimplemented | partial: checked on functions and methods, including foreign declarations. A missing public obligation warns; a `TODO` placeholder warns; malformed or misplaced text errors. Obligation reporting through `ember tcb` remains. `UNS-7/` covers these forms. |
