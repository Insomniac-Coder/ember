...
| `[TYP-13]` | IV | `Option[T]` has the size of `T` when `T` has a niche: a class handle, … | compliant (partial) | `NonZero[T]`, class and class-interface handles, `Box`, ordinary `ref`, `char`, and `bool` use one payload-sized representation; `TYP-13/` checks size and Some/None behavior. `Span`, `str`, extern functions, ranges and enum discriminants remain to implement or probe. |
