...
| `[GRM-4]` | III | `x = e` where no `x` is in scope declares `x` with the type of `e`; … | gap | probed 2026-09-24: redeclaration in one block compiled (D-240, fixed: `E1020`); a name declared in every arm is not hoisted (`[CTL-10]`) |
...
| `[GRM-16]` | III | `return`, `break` and `continue` are expressions of type `Never`; … | gap | probed 2026-09-24: `1 + return 2` is `E0107` as it should be, but a jump as a conditional-expression branch (`v = 1 if b else return 7`) is refused too |
