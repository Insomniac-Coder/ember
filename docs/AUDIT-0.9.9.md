...
| a bound's method in a generic body | `[TYP-17]`: only what the bound provides | an instantiation re-checks the body on the concrete type, where an inherent method of the name comes first | fixed for generic functions (D-245); a generic type's methods are duck-typed (D-248, open) |
