...
| `[FFI-1]` | XVI | Every foreign declaration has a contract: for each pointer-typed … | partial | ODR-073/Hardened_30 defines hand-declaration syntax and requires a safe mapped surface. Borrowed `one` covers shared `ref T`, exclusive `mut p: T`, and nullable `Option[ref T]` / `Option[ref mut T]`, with explicit `aliased` on shared references. Other ownership, count, lifetime, and result forms remain explicit `E0900` gaps. |
...
| `[FFI-10]` | XVI | An `unsafe extern "C":` block declares foreign functions, statics and … | partial | Functions are module items and link through the C ABI, including safe scalar calls, contracted borrowed-one and nullable reference calls, unsafe calls, and capture-free function values. Duplicate names, non-unsafe blocks, unsupported ABIs, missing or mismatched pointer clauses, and borrowed aggregate ABI mismatches are rejected. Foreign statics, opaque types, other pointer contract shapes, arbitrary linker-symbol aliases, and importer integration remain. |
...
| `[FFI-11]` | XVI | Contract vocabulary. A pointer contract has five axes; mutability … | partial | ODR-073/Hardened_30 names pointer slots with `@ffi(param(name, words…))`. Borrowed `one` covers shared and exclusive single places, including nullable references; `aliased` is accepted for shared places. `E5012` identifies a missing count and suggests length-like siblings. All four `TODO(axis)` words retain unsafe calls. Other ownership and count forms, result clauses, and header overlays remain `E0900`. |
