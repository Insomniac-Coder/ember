...
| `[FFI-1]` | XVI | Every foreign declaration has a contract: for each pointer-typed … | partial | ODR-073/Hardened_30 defines hand-declaration syntax and requires a safe mapped surface. `@ffi(param(p, borrowed, one, exclusive))` on `mut p: T` and `@ffi(param(p, borrowed, one))` on `p: ref T` are built, including multiple named clauses. Other ownership, count, nullability, lifetime, and result forms remain explicit `E0900` gaps. |
| `[FFI-2]` | XVI | A call to a foreign function whose contract contains an `unknown` … | partial | Hand-declared functions without `safe fn` require an enclosing `unsafe:` or `unsafe fn`. A `safe fn` with the implemented borrowed-one contract has a safe C call; a raw pointer remains unsafe to expose. Full pointer contracts and header overlays remain. `FFI-10/` checks the gate. |
| `[FFI-10]` | XVI | An `unsafe extern "C":` block declares foreign functions, statics and … | partial | Functions are module items and link through the C ABI, including safe scalar calls, contracted borrowed-one calls, unsafe calls, and capture-free function values. Duplicate names, non-unsafe blocks, unsupported ABIs, missing or mismatched pointer clauses, and borrowed aggregate ABI mismatches are rejected. Foreign statics, opaque types, other pointer contract shapes, arbitrary linker-symbol aliases, and importer integration remain. |
...
| `[FFI-11]` | XVI | Contract vocabulary. A pointer contract has five axes; mutability … | partial | ODR-073/Hardened_30 names pointer slots with `@ffi(param(name, words…))`; borrowed `one` is built for shared `ref T` and `exclusive` `mut p: T`. Other words and result clauses remain `E0900`. |
