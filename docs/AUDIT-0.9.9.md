...
| inherent methods on another package's type | `[IFC-2]`: `E2120` | accepted (`extend i64: fn twice(self)`) | defect D-243 (open) |
| a bound's method in a generic body | `[TYP-17]`: only what the bound provides | an instantiation re-checks the body on the concrete type, where an inherent method of the name comes first | defect D-245 (open) |
