...
| `[TYP-13]` | IV | `Option[T]` has the size of `T` when `T` has a niche: a class handle, … | compliant (partial) | `NonZero[T]`, class and class-interface handles, `Box`, ordinary `ref`, `bool`, `char`, unit enums with spare discriminants, integer and float ranges with a niche, `Span`, and `str` use payload-sized representations. `TYP-13/` and `RNG-7/` check layout and Some/None behavior. Extern function pointers and `ref dyn` remain to implement or probe. |
