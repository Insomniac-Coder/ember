...
/* `[RNG-7]`: no valid range value is NaN. Test the bits so a compiler's
 * floating-point comparison mode cannot hide Option's NaN niche. */
static inline bool ember_f16_is_nan(uint16_t bits) {
    return (bits & UINT16_C(0x7C00)) == UINT16_C(0x7C00)
        && (bits & UINT16_C(0x03FF)) != 0;
}

static inline bool ember_f32_is_nan(float value) {
    uint32_t bits;
...
static inline bool ember_f64_is_nan(double value) {
    uint64_t bits;
    memcpy(&bits, &value, sizeof bits);
...
/* `[OBJ-1]`: a foreign build must fail rather than silently use a different
 * object ABI. C++ embedders include this header too. */
#if defined(__cplusplus)
#define EMBER_ABI_ASSERT(condition, message) static_assert((condition), message)
#else
#define EMBER_ABI_ASSERT(condition, message) _Static_assert((condition), message)
#endif
#if UINTPTR_MAX == UINT64_MAX
EMBER_ABI_ASSERT(sizeof(ember_obj_header) == 24, "Ember object header must be 24 bytes");
EMBER_ABI_ASSERT(offsetof(ember_obj_header, strong) == 0, "Ember strong count offset changed");
EMBER_ABI_ASSERT(offsetof(ember_obj_header, weak) == 4, "Ember weak count offset changed");
EMBER_ABI_ASSERT(offsetof(ember_obj_header, access) == 8, "Ember access offset changed");
EMBER_ABI_ASSERT(offsetof(ember_obj_header, flags) == 12, "Ember flags offset changed");
EMBER_ABI_ASSERT(offsetof(ember_obj_header, ti) == 16, "Ember type-info offset changed");
#endif
#undef EMBER_ABI_ASSERT
...
/* `[EXC-6]`: debug builds remember where each live field access began. */
...
EMBER_NORETURN void ember_panic_field_exclusivity_debug(
    bool writing, uint32_t active, const uint32_t* word, const char* what, ember_loc loc);
#define EMBER_FIELD_CONFLICT(writing, word, what, loc) \
    ember_panic_field_exclusivity_debug((writing), *(word), (word), (what), (loc))
#define EMBER_FIELD_TRACK_BEGIN(word, writing, loc) ember_debug_field_begin((word), (writing), (loc))
#define EMBER_FIELD_TRACK_END(word, writing, loc) ember_debug_field_end((word), (writing), (loc))
#else
#define EMBER_FIELD_CONFLICT(writing, word, what, loc) \
    ember_panic_field_exclusivity((writing), *(word), (what), (loc))
#define EMBER_FIELD_TRACK_BEGIN(word, writing, loc) ((void)0)
#define EMBER_FIELD_TRACK_END(word, writing, loc) ((void)0)
#endif
