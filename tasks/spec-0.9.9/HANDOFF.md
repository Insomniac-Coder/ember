...
* **LOCATION (2026-09-23):** this folder is committed on `main` (`Code/ember/tasks/spec-0.9.9/`); build hardenings to `Code/ember/docs/spec-source/`. H13 is the latest (ODR-032 to ODR-036, the `Map`/`Set` rulings, 2026-09-25; H12 carried ODR-031, H11 ODR-030, H10 ODR-029, H9 ODR-028); `parts/` is the working source for H14, and `parts-h13/` is frozen. To cut the next: add the ODR row to `ODRS` in `tools/appx_h.py`, run the rebuild recipe above with `cat parts/p*.md > ../../docs/spec-source/Ember_v0.9.9_Hardened_N.md`, copy `parts/` to `parts-hN/`, and repoint `docs/spec-source/development-target.json` (its SHA-256 is of the file's LF bytes).
