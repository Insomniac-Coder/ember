...
* **LOCATION (2026-09-23):** this folder is committed on `main` (`Code/ember/tasks/spec-0.9.9/`); build hardenings to `Code/ember/docs/spec-source/`. H6 is the latest (ODR-025, 2026-09-24); `parts/` is the working source for H7, and `parts-h6/` is frozen. To cut the next: add the ODR row to `ODRS` in `tools/appx_h.py`, run the rebuild recipe above with `cat parts/p*.md > ../../docs/spec-source/Ember_v0.9.9_Hardened_N.md`, copy `parts/` to `parts-hN/`, and repoint `docs/spec-source/development-target.json` (its SHA-256 is of the file's LF bytes).
